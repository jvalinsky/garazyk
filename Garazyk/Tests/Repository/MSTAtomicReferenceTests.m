...
- (void)testInitWithMST {
    ...
    ATProtoMST *snapshot = [ref currentSnapshot];
...
- (void)testSwapMST {
    ATProtoMST *mst1 = [[ATProtoMST alloc] init];
    ATProtoMST *mst2 = [[ATProtoMST alloc] init];
...
- (void)testConcurrentReadWriteStress {
    ...
    // Create a set of ATProtoMST objects to swap in
    NSMutableArray<ATProtoMST *> *msts = [NSMutableArray arrayWithCapacity:10];
    for (NSUInteger i = 0; i < 10; i++) {
        [msts addObject:[[ATProtoMST alloc] init]];
    }
    ...
    // Writer: swap ATProtoMST objects in a loop
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        for (NSUInteger i = 0; i < 100000; i++) {
            ATProtoMST *mst = msts[i % msts.count];
            [ref swapMST:mst];
        }
        // Final clear to nil
        [ref clear];
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            for (NSUInteger i = 0; i < 10000; i++) {
                @autoreleasepool {
                    ATProtoMST *snapshot = [ref currentSnapshot];
                    if (snapshot) {
                        // Verify the snapshot is a valid ATProtoMST object
                        @synchronized(msts) {
...
- (void)testConcurrentSwapNoLostMST {
    ...
    // Create distinct ATProtoMST objects with identifiable rootCIDs
    NSMutableArray<ATProtoMST *> *msts = [NSMutableArray arrayWithCapacity:20];
    for (NSUInteger i = 0; i < 20; i++) {
        [msts addObject:[[ATProtoMST alloc] init]];
    }

    __block NSMutableSet<ATProtoMST *> *seenMSTs = [NSMutableSet set];
...
- (void)testDeallocWithPendingReads {
    for (NSUInteger i = 0; i < 1000; i++) {
        @autoreleasepool {
            ATProtoMSTAtomicReference *ref = [[ATProtoMSTAtomicReference alloc] initWithMST:[[ATProtoMST alloc] init]];
            ATProtoMST *snapshot = [ref currentSnapshot];
