...
- (void)testEncode {
    ...
    NSString *encoded = [ATProtoBase58 encode:data];
    XCTAssertEqualObjects(encoded, @"StV1DL6CwTryKyV");
    
    NSData *empty = [NSData data];
    XCTAssertEqualObjects([ATProtoBase58 encode:empty], @"");
}

- (void)testDecode {
    NSString *string = @"StV1DL6CwTryKyV";
    NSData *decoded = [ATProtoBase58 decode:string];
    ...
    XCTAssertEqualObjects([ATProtoBase58 decode:@""], [NSData data]);
}

- (void)testInvalidDecode {
    XCTAssertNil([ATProtoBase58 decode:@"0"]); // '0' is not in alphabet
    XCTAssertNil([ATProtoBase58 decode:@"I"]); // 'I' is not in alphabet
    XCTAssertNil([ATProtoBase58 decode:@"l"]); // 'l' is not in alphabet
    XCTAssertNil([ATProtoBase58 decode:@"O"]); // 'O' is not in alphabet
}
...
- (void)testEncodeDecodeMatchExpectedValuesForLeadingZeros {
    ...
    // Leading zeros -> '1' in ATProtoBase58
    // 0xff -> '5Q' (Wait, is it?)
    // 255 = 4 * 58 + 23 ('Q') -> 4 ('5')
    ...
    NSData *decoded = [ATProtoBase58 decode:@"115Q"];
...
- (void)testLargeInputRejection {
    ...
    XCTAssertNil([ATProtoBase58 encode:largeData], @"Should reject input > 64KB");
}
...
- (void)testLargeStringRejection {
    ...
    XCTAssertNil([ATProtoBase58 decode:largeString], @"Should reject string > 64KB");
}
...
- (void)testMultiByteUTF8Rejected {
    ...
    XCTAssertNil([ATProtoBase58 decode:@"€"], @"Standalone BMP multi-byte char should be rejected"); // '€', 1 UTF-16 unit, 3 UTF-8 bytes
    XCTAssertNil([ATProtoBase58 decode:@"\U0001F600"], @"Supplementary-plane char should be rejected"); // emoji, 2 UTF-16 units, 4 UTF-8 bytes
    ...
    XCTAssertNil([ATProtoBase58 decode:prefixed], @"Trailing multi-byte char should be rejected");
}

- (void)testMaxValidInput {
    // Use a reasonable size that completes quickly (ATProtoBase58 is O(n²) in worst case)
    NSMutableData *maxData = [NSMutableData dataWithLength:1024];
    memset(maxData.mutableBytes, 0x41, 1024);
    NSString *encoded = [ATProtoBase58 encode:maxData];
    XCTAssertNotNil(encoded, @"Should accept 1KB input");
    XCTAssertGreaterThan(encoded.length, (NSUInteger)0);
    
    NSData *decoded = [ATProtoBase58 decode:encoded];
