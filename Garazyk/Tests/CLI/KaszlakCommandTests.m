...
@interface KaszlakCommandResult : NSObject
...
@implementation KaszlakCommandResult
@end

static NSString *KaszlakStringFromFileDescriptor(int descriptor) {
...
@interface KaszlakCommandTests : XCTestCase
@end

@implementation KaszlakCommandTests

- (NSString *)kaszlakExecutablePath {
    ...
    return [[buildDirectory stringByAppendingPathComponent:@"bin"]
        stringByAppendingPathComponent:@"kaszlak"];
}

- (nullable KaszlakCommandResult *)runKaszlakWithArguments:(NSArray<NSString *> *)arguments {
    NSString *executable = [self kaszlakExecutablePath];
    ...
    NSString *standardOutput = KaszlakStringFromFileDescriptor(stdoutPipe[0]);
    NSString *standardError = KaszlakStringFromFileDescriptor(stderrPipe[0]);
    close(stdoutPipe[0]);
    close(stderrPipe[0]);

    if (timedOut) {
        XCTFail(@"kaszlak did not exit within the bounded process-test timeout");
        return nil;
    }
    XCTAssertTrue(WIFEXITED(waitStatus), @"kaszlak terminated unexpectedly: %@", standardError);
    if (!WIFEXITED(waitStatus)) {
        return nil;
    }

    KaszlakCommandResult *result = [[KaszlakCommandResult alloc] init];
...
- (void)assertUsageOutput:(NSString *)output {
    ...
    XCTAssertTrue([output containsString:@"A command-line interface for managing kaszlak (ATProto PDS).\n"]);
    XCTAssertTrue([output containsString:@"Commands:\n"]);
    XCTAssertTrue([output containsString:@"  serve       Start the PDS server\n"]);
}

- (void)testNoArgumentsPrintsUsageAndReturnsInvalidArguments {
    KaszlakCommandResult *result = [self runKaszlakWithArguments:@[]];
    XCTAssertNotNil(result);
    if (!result) return;

    XCTAssertEqual(result.exitStatus, PDSCLIExitCodeInvalidArguments);
    [self assertUsageOutput:result.standardOutput];
    XCTAssertTrue([result.standardError containsString:@"Error: Missing command\n\n"]);
}

- (void)testFlagsBeforeCommandPrintsErrorAndReturnsInvalidArguments {
    KaszlakCommandResult *result = [self runKaszlakWithArguments:@[@"--verbose", @"serve"]];
    XCTAssertNotNil(result);
    if (!result) return;

    XCTAssertEqual(result.exitStatus, PDSCLIExitCodeInvalidArguments);
    [self assertUsageOutput:result.standardOutput];
    XCTAssertTrue([result.standardError containsString:@"Error: Flags must follow the command name\n\n"]);
}

- (void)testHelpCommandPrintsUsageAndReturnsZero {
    KaszlakCommandResult *result = [self runKaszlakWithArguments:@[@"help"]];
    XCTAssertNotNil(result);
    if (!result) return;

    XCTAssertEqual(result.exitStatus, PDSCLIExitCodeSuccess);
}

- (void)testUnknownCommandReturnsNotFound {
    KaszlakCommandResult *result = [self runKaszlakWithArguments:@[@"non-existent-cmd"]];
    XCTAssertNotNil(result);
    if (!result) return;

    XCTAssertEqual(result.exitStatus, PDSCLIExitCodeNotFound);
}
