...
@property (nonatomic, strong) ATProtoHttpRetryPolicy *policy;
@end
...
- (void)setUp {
    [super setUp];
    self.policy = [[ATProtoHttpRetryPolicy alloc] init];
}
...
- (void)testSucceedsOn200MatchesDecisionSucceed {
    ATProtoHttpRetryResult *result = [self.policy evaluateStatusCode:200 networkError:nil attemptNumber:0];
...
- (void)testFailsImmediatelyOn404MatchesDecisionFail {
    ATProtoHttpRetryResult *result = [self.policy evaluateStatusCode:404 networkError:nil attemptNumber:0];
...
- (void)testRetriesOn500WithExponentialBackoff {
    // Attempt 0
    ATProtoHttpRetryResult *result0 = [self.policy evaluateStatusCode:500 networkError:nil attemptNumber:0];
    XCTAssertEqual(result0.decision, HttpRetryDecisionRetryAfter);
    XCTAssertEqualWithAccuracy(result0.retryDelay, 0.5, 0.01);

    // Attempt 1
    ATProtoHttpRetryResult *result1 = [self.policy evaluateStatusCode:500 networkError:nil attemptNumber:1];
    XCTAssertEqual(result1.decision, HttpRetryDecisionRetryAfter);
    XCTAssertEqualWithAccuracy(result1.retryDelay, 1.0, 0.01);

    // Attempt 2
    ATProtoHttpRetryResult *result2 = [self.policy evaluateStatusCode:502 networkError:nil attemptNumber:2];
    ...
    ATProtoHttpRetryResult *result3 = [self.policy evaluateStatusCode:503 networkError:nil attemptNumber:3];
...
- (void)testRetriesOnNetworkError {
    ...
    // Attempt 0
    ATProtoHttpRetryResult *result0 = [self.policy evaluateStatusCode:0 networkError:error attemptNumber:0];
    XCTAssertEqual(result0.decision, HttpRetryDecisionRetryAfter);
    XCTAssertEqualWithAccuracy(result0.retryDelay, 0.5, 0.01);
    
    // Attempt 3 (Max retries reached)
    ATProtoHttpRetryResult *result3 = [self.policy evaluateStatusCode:0 networkError:error attemptNumber:3];
    XCTAssertEqual(result3.decision, HttpRetryDecisionFail);
}
...
- (void)testCustomPolicySettings {
    ...
    ATProtoHttpRetryResult *result5 = [self.policy evaluateStatusCode:500 networkError:nil attemptNumber:5];
