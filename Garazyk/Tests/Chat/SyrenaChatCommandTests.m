...
@interface SyrenaChatCommandResult : NSObject
...
@implementation SyrenaChatCommandResult
@end

static NSString *SyrenaChatStringFromFileDescriptor(int descriptor) {
...
@interface SyrenaChatCommandTests : XCTestCase
@end

@implementation SyrenaChatCommandTests

- (NSString *)syrenaChatExecutablePath {
    ...
    return [[buildDirectory stringByAppendingPathComponent:@"bin"]
        stringByAppendingPathComponent:@"syrena-chat"];
}

- (nullable SyrenaChatCommandResult *)runSyrenaChatWithArguments:(NSArray<NSString *> *)arguments {
    NSString *executable = [self syrenaChatExecutablePath];
    ...
    NSString *standardOutput = SyrenaChatStringFromFileDescriptor(stdoutPipe[0]);
    NSString *standardError = SyrenaChatStringFromFileDescriptor(stderrPipe[0]);
    close(stdoutPipe[0]);
    close(stderrPipe[0]);

    if (timedOut) {
        XCTFail(@"syrena-chat did not exit within the bounded process-test timeout");
        return nil;
    }
    XCTAssertTrue(WIFEXITED(waitStatus), @"syrena-chat terminated unexpectedly: %@", standardError);
    if (!WIFEXITED(waitStatus)) {
        return nil;
    }

    SyrenaChatCommandResult *result = [[SyrenaChatCommandResult alloc] init];
...
- (void)assertUsageOutput:(NSString *)output {
    XCTAssertTrue([output containsString:@"Usage: syrena-chat <command> [options]\n\n"]);
    XCTAssertTrue([output containsString:@"Syrena Chat - Standalone AT Protocol Chat Service\n\n"]);
    XCTAssertTrue([output containsString:@"  serve        Start Chat server\n"]);
    XCTAssertTrue([output containsString:@"  version      Show version info\n"]);
    XCTAssertTrue([output containsString:@"  help         Show this help\n\n"]);
}

- (void)testNoArgumentsPrintsUsageAndReturnsTwo {
    SyrenaChatCommandResult *result = [self runSyrenaChatWithArguments:@[]];
...
- (void)testHelpPrintsUsageAndReturnsZero {
    ...
    for (NSString *flag in helpFlags) {
        SyrenaChatCommandResult *result = [self runSyrenaChatWithArguments:@[flag]];
...
- (void)testVersionPrintsVersionAndReturnsZero {
    SyrenaChatCommandResult *result = [self runSyrenaChatWithArguments:@[@"version"]];
    XCTAssertNotNil(result);
    if (!result) return;

    XCTAssertEqual(result.exitStatus, 0);
    XCTAssertTrue([result.standardOutput containsString:@"Syrena Chat 0.2.0 (AT Protocol Chat Service)"]);
}

- (void)testUnknownCommandPrintsErrorAndReturnsTwo {
    SyrenaChatCommandResult *result = [self runSyrenaChatWithArguments:@[@"unknown-command"]];
    XCTAssertNotNil(result);
    if (!result) return;
...
- (void)testServeBindFailureReturnsOne {
    ...
    SyrenaChatCommandResult *result = [self runSyrenaChatWithArguments:@[@"serve", @"--port", @"19878"]];
    close(listenSocket);
