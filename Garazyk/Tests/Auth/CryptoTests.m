...
- (void)testSHA256MatchesExpectedHex {
    ...
    NSData *hash = [ATProtoCryptoUtils sha256:input];
    NSString *hex = [ATProtoCryptoUtils hexStringFromData:hash];
...
- (void)testHMACSHA1MatchesExpectedHex {
    ...
    NSString *hex = [ATProtoCryptoUtils hexStringFromData:hmac];
...
- (void)testRandomBytes {
    NSData *r1 = [ATProtoCryptoUtils randomBytes:16];
    NSData *r2 = [ATProtoCryptoUtils randomBytes:16];
...
- (void)testConstantTimeCompareEqual {
    XCTAssertTrue([ATProtoCryptoUtils constantTimeCompare:@"abc123xyz" to:@"abc123xyz"]);
    XCTAssertTrue([ATProtoCryptoUtils constantTimeCompare:@"" to:@""]);
    XCTAssertTrue([ATProtoCryptoUtils constantTimeCompare:@"a" to:@"a"]);
    XCTAssertTrue([ATProtoCryptoUtils constantTimeCompare:@"dpop_thumbprint_12345" to:@"dpop_thumbprint_12345"]);
}

- (void)testConstantTimeCompareNotEqual {
    XCTAssertFalse([ATProtoCryptoUtils constantTimeCompare:@"abc123xyz" to:@"abc123xya"]);
    XCTAssertFalse([ATProtoCryptoUtils constantTimeCompare:@"abc123xyz" to:@"xbc123xyz"]);
    XCTAssertFalse([ATProtoCryptoUtils constantTimeCompare:@"abc123xyz" to:@"abc123xyZ"]);
    XCTAssertFalse([ATProtoCryptoUtils constantTimeCompare:@"abc" to:@"abd"]);
}

- (void)testConstantTimeCompareDifferentLength {
    XCTAssertFalse([ATProtoCryptoUtils constantTimeCompare:@"abc" to:@"abcd"]);
    XCTAssertFalse([ATProtoCryptoUtils constantTimeCompare:@"abcd" to:@"abc"]);
    XCTAssertFalse([ATProtoCryptoUtils constantTimeCompare:@"" to:@"a"]);
    XCTAssertFalse([ATProtoCryptoUtils constantTimeCompare:@"a" to:@""]);
}

- (void)testConstantTimeCompareNilHandling {
    XCTAssertFalse([ATProtoCryptoUtils constantTimeCompare:nil to:@"abc"]);
    XCTAssertFalse([ATProtoCryptoUtils constantTimeCompare:@"abc" to:nil]);
    XCTAssertTrue([ATProtoCryptoUtils constantTimeCompare:nil to:nil]);
}

- (void)testConstantTimeCompareSpecialCharacters {
    XCTAssertTrue([ATProtoCryptoUtils constantTimeCompare:@"base64/url+encoded==" to:@"base64/url+encoded=="]);
    XCTAssertFalse([ATProtoCryptoUtils constantTimeCompare:@"base64/url+encoded==" to:@"base64/url+encoded=!"]);
    XCTAssertTrue([ATProtoCryptoUtils constantTimeCompare:@"did:plc:abc123" to:@"did:plc:abc123"]);
    XCTAssertFalse([ATProtoCryptoUtils constantTimeCompare:@"did:plc:abc123" to:@"did:plc:abc124"]);
}

#pragma mark - SHA-256 Edge Cases

- (void)testSHA256NilInput {
    NSData *result = [ATProtoCryptoUtils sha256:nil];
    XCTAssertNil(result);
}

- (void)testSHA256EmptyData {
    NSData *empty = [NSData data];
    NSData *hash = [ATProtoCryptoUtils sha256:empty];
...
- (void)testHMACSHA1NilKey {
    NSData *data = [@"data" dataUsingEncoding:NSUTF8StringEncoding];
    NSData *result = [ATProtoCryptoUtils hmacSHA1WithKey:nil data:data];
    XCTAssertNil(result);
}

- (void)testHMACSHA1NilData {
    NSData *key = [@"key" dataUsingEncoding:NSUTF8StringEncoding];
    NSData *result = [ATProtoCryptoUtils hmacSHA1WithKey:key data:nil];
    XCTAssertNil(result);
}

- (void)testHMACSHA256NilKey {
    NSData *data = [@"data" dataUsingEncoding:NSUTF8StringEncoding];
    NSData *result = [ATProtoCryptoUtils hmacSHA256WithKey:nil data:data];
    XCTAssertNil(result);
}

- (void)testHMACSHA256NilData {
    NSData *key = [@"key" dataUsingEncoding:NSUTF8StringEncoding];
    NSData *result = [ATProtoCryptoUtils hmacSHA256WithKey:key data:nil];
    XCTAssertNil(result);
}
...
- (void)testRandomBytesZeroLength {
    NSData *result = [ATProtoCryptoUtils randomBytes:0];
...
- (void)testHexStringFromDataEmpty {
    NSData *empty = [NSData data];
    NSString *hex = [ATProtoCryptoUtils hexStringFromData:empty];
...
- (void)testHexStringFromDataKnownValue {
    ...
    NSString *hex = [ATProtoCryptoUtils hexStringFromData:data];
...
- (void)testBase64URLEncodeDecodeRoundTrip {
    for (NSUInteger len = 1; len < 64; len++) {
        ...
        NSString *encoded = [ATProtoCryptoUtils base64URLEncode:data];
        NSData *decoded = [ATProtoCryptoUtils base64URLDecode:encoded];
...
- (void)testEncryptWithInvalidKeySize {
    ...
    NSData *result = [ATProtoCryptoUtils encryptData:plaintext withKey:shortKey];
...
- (void)testDecryptWithInvalidKeySize {
    ...
    NSData *result = [ATProtoCryptoUtils decryptData:fakeCipher withKey:shortKey];
...
- (void)testDecryptWithTooShortData {
    ...
    NSData *result = [ATProtoCryptoUtils decryptData:shortData withKey:key];
...
- (void)testEncryptDecryptEmptyData {
    NSData *key = [ATProtoCryptoUtils randomBytes:32];
    NSData *empty = [NSData data];
    NSData *encrypted = [ATProtoCryptoUtils encryptData:empty withKey:key];
    XCTAssertNotNil(encrypted);
    NSData *decrypted = [ATProtoCryptoUtils decryptData:encrypted withKey:key];
    XCTAssertNotNil(decrypted);
...
- (void)testDeriveKeyFromPassword {
    ...
    NSData *key = [ATProtoCryptoUtils deriveKeyFromPassword:@"testpassword" salt:salt];
    XCTAssertNotNil(key);
...
- (void)testDeriveKeyDifferentPasswords {
    NSData *salt = [@"fixed-salt-1234" dataUsingEncoding:NSUTF8StringEncoding];
    NSData *key1 = [ATProtoCryptoUtils deriveKeyFromPassword:@"password1" salt:salt];
    NSData *key2 = [ATProtoCryptoUtils deriveKeyFromPassword:@"password2" salt:salt];
...
- (void)testDeriveKeyDifferentSalts {
    ...
    NSData *key1 = [ATProtoCryptoUtils deriveKeyFromPassword:@"password" salt:salt1];
    NSData *key2 = [ATProtoCryptoUtils deriveKeyFromPassword:@"password" salt:salt2];
