...
- (void)testThumbprint_EC_P256_Valid {
    ...
    NSError *error = nil;
    NSString *thumbprint = [ATProtoAuthCryptoJWK thumbprint:jwk error:&error];
...
- (void)testThumbprint_EC_NilError_DoesNotCrash {
    ...
    NSString *thumbprint = [ATProtoAuthCryptoJWK thumbprint:jwk error:nil];
    XCTAssertNotNil(thumbprint);
}
...
- (void)testThumbprint_NilJWK_ReturnsNil {
    ...
    NSString *thumbprint = [ATProtoAuthCryptoJWK thumbprint:@{} error:&error];
    XCTAssertNil(thumbprint);
    XCTAssertNotNil(error);
}
...
- (void)testThumbprint_DeterministicForSameKey {
    ...
    NSString *tp1 = [ATProtoAuthCryptoJWK thumbprint:jwk error:&error1];
    NSString *tp2 = [ATProtoAuthCryptoJWK thumbprint:jwk error:&error2];
    XCTAssertNotNil(tp1);
    XCTAssertNotNil(tp2);
...
- (void)testPublicKeyFromJWK_NonEC_ReturnsNilError {
    ...
    NSError *error = nil;
    id key = [ATProtoAuthCryptoJWK publicKeyFromJWK:jwk error:&error];
    XCTAssertNil(key);
    XCTAssertNotNil(error);
...
- (void)testPrivateKeyFromJWK_NonEC_ReturnsNilError {
    NSDictionary *jwk = @{@"kty": @"RSA", @"n": @"modulus", @"e": @"AQAB"};
    NSError *error = nil;
    id key = [ATProtoAuthCryptoJWK privateKeyFromJWK:jwk error:&error];
    XCTAssertNil(key);
    XCTAssertNotNil(error);
}
...
- (void)testPublicJWKFromKey_NilKey_ReturnsNilError {
    NSError *error = nil;
    NSDictionary *result = [ATProtoAuthCryptoJWK publicJWKFromKey:(id<PDSKeyProtocol>)nil error:&error];
    XCTAssertNil(result);
    XCTAssertNotNil(error);
}
...
- (void)testJWKFromKey_NilKey_ReturnsNilError {
    NSError *error = nil;
    NSDictionary *result = [ATProtoAuthCryptoJWK jwkFromKey:(id<PDSKeyProtocol>)nil error:&error];
    XCTAssertNil(result);
    XCTAssertNotNil(error);
}
...
- (void)testThumbprintForKey_NilKey_ReturnsNilError {
    NSError *error = nil;
    NSString *result = [ATProtoAuthCryptoJWK thumbprintForKey:(id<PDSKeyProtocol>)nil error:&error];
    XCTAssertNil(result);
    XCTAssertNotNil(error);
}
...
- (void)testThumbprintAndPublicJWKConsistency {
    ...
    NSDictionary *publicJWK = [ATProtoAuthCryptoJWK publicJWKFromJWK:privateJWK];

    NSError *err1 = nil, *err2 = nil;
    NSString *tpPrivate = [ATProtoAuthCryptoJWK thumbprint:privateJWK error:&err1];
    NSString *tpPublic = [ATProtoAuthCryptoJWK thumbprint:publicJWK error:&err2];
