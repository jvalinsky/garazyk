...
async function fetchWithDPoPRetry(url, init, dpopKeyPair, options = {}) {
  // First attempt
  const dpopProof = await createDPoPProof(dpopKeyPair, init.method || "GET", url, {
    ath: options.ath,
  });
  const headers = {
    ...init.headers,
...
async function handleCallback() {
  try {
    ...
    const dpopProof = await createDPoPProofFromJWK(
      dpopPrivateJWK,
      "POST",
      new URL("/oauth/token", LAB_CONFIG.pdsUrl),
    );
...
async function updateHandleFlow() {
  try {
    ...
    if (response.ok) {
      ...
      resultDiv.innerHTML =
        '<div class="alert alert-destructive">Error: <span class="error-text"></span></div>';
      resultDiv.querySelector(".error-text").textContent = errorData.error ||
