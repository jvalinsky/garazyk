...
+ (NSString *)renderAppViewMetricsPartial:(NSDictionary *)result {
    ...
    [html appendFormat:@"<div class=\"metric\"><span class=\"metric-label\">Queue Depth</span><span class=\"metric-value\">%@</span></div>", GZAdminUIEscaped([backfill[@"queue_depth"] stringValue] ?: @"0")];
    [html appendFormat:@"<div class=\"metric\"><span class=\"metric-label\">Active Workers</span><span class=\"metric-value\">%@</span></div>", GZAdminUIEscaped([backfill[@"active_workers"] stringValue] ?: @"0")];
    [html appendFormat:@"<div class=\"metric\"><span class=\"metric-label\">Relays</span><span class=\"metric-value\">%@</span></div>", GZAdminUIEscaped([ingest[@"relays"] stringValue] ?: @"0")];
    [html appendFormat:@"<div class=\"metric\"><span class=\"metric-label\">Index Records</span><span class=\"metric-value\">%@</span></div>", GZAdminUIEscaped([index[@"total_records"] stringValue] ?: @"0")];
...
+ (NSString *)renderIngestHealthPartial:(NSDictionary *)result {
    ...
    for (NSDictionary *relay in relays) {
        NSString *url = GZAdminUIEscaped(relay[@"url"] ?: @"");
        NSString *lag = GZAdminUIEscaped([relay[@"lag"] stringValue] ?: @"0");
        NSString *tps = GZAdminUIEscaped([relay[@"throughput"] stringValue] ?: @"0");
...
+ (NSString *)renderBackfillQueuePartial:(NSDictionary *)result {
    ...
    for (NSDictionary *entry in entries) {
        NSString *did = GZAdminUIEscaped(entry[@"did"] ?: @"");
        NSString *status = GZAdminUIEscaped(entry[@"status"] ?: @"");
