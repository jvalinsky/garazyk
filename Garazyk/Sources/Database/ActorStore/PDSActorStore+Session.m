...
- (nullable NSDictionary *)sessionInfoForRefreshToken:(NSString *)token error:(NSError **)error {
    if (!token) return nil;
    NSString *sql = @"SELECT account_did, session_id, expires_at, next_token FROM refresh_tokens WHERE token = ? AND expires_at > ?";
...
- (nullable NSString *)accountDidForRefreshToken:(NSString *)token error:(NSError **)error {
    if (!token) return nil;
    NSString *sql = @"SELECT account_did FROM refresh_tokens WHERE token = ? AND expires_at > ? AND next_token IS NULL";
    NSArray *results = [self.database executeParameterizedQuery:sql params:@[token, @([[NSDate date] timeIntervalSince1970])] error:error];
    if (results.count > 0) {
...
- (BOOL)markRefreshTokenRotated:(NSString *)token nextToken:(NSString *)nextToken graceExpiresAt:(NSDate *)graceExpiresAt error:(NSError **)error {
    if (!token || !nextToken || !graceExpiresAt) return NO;
    NSString *sql = @"UPDATE refresh_tokens SET next_token = ?, expires_at = ? WHERE token = ?";
    return [self.database executeParameterizedUpdate:sql params:@[nextToken, @(graceExpiresAt.timeIntervalSince1970), token] error:error];
}
