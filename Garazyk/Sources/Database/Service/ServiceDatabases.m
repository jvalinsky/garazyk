...
- (BOOL)markRefreshTokenRotated:(NSString *)token nextToken:(NSString *)nextToken graceExpiresAt:(NSDate *)graceExpiresAt error:(NSError **)error {
    ...
    [self.servicePool transactWithDid:@"__service__" block:^(id<PDSActorStoreTransactor> transactor, NSError **innerError) {
        PDSActorStore *store = (PDSActorStore *)transactor;
        success = [store markRefreshTokenRotated:token nextToken:nextToken graceExpiresAt:graceExpiresAt error:innerError];
    } error:error];
    return success;
}
...
- (NSInteger)pruneExpiredPasswordResetTokensBefore:(NSDate *)date
    ...
    [self.servicePool transactWithDid:@"__service__"
        ...
        BOOL updated = [store.database
            executeParameterizedUpdate:@"DELETE FROM password_reset_tokens WHERE expires_at < ?"
