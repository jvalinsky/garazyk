...
async function loadDID(did) {
  ...
  try {
    ...
    summaryContent.innerHTML =
      '<p class="error">Failed to load DID: <span class="error-text"></span></p>';
    ...
    handles.length
      ? `
                <ul class="did-handle-list">
                    ${
        handles.map((h) =>
          `<li class="did-handle-item"><strong>${h.replace("at://", "@")}</strong></li>`
        ).join("")
      }
                </ul>
            `
      ...
      ? `
        ...
        services.map((s) => `
                        <tr>
                            ...
                            <td>${s.type}</td>
                            <td><a href="${
          typeof s.serviceEndpoint === "string" ? s.serviceEndpoint : s.serviceEndpoint.uri
        }" target="_blank">${
          typeof s.serviceEndpoint === "string" ? s.serviceEndpoint : s.serviceEndpoint.uri
        }</a></td>
                        </tr>
...
function computeDiff(oldOp, newOp) {
  ...
  if (JSON.stringify(oldSvc) !== JSON.stringify(newSvc)) {
    ...
    allKeys.forEach((k) => {
      ...
      else if (JSON.stringify(oldSvc[k]) !== JSON.stringify(newSvc[k])) {
        svcChanges.push({ action: "updated", key: k, val: newSvc[k], oldVal: oldSvc[k] });
      }
    });
  ...
  if (JSON.stringify(oldVM) !== JSON.stringify(newVM)) {
    ...
    all.forEach((k) => {
      if (!oldVM[k]) vmChanges.push({ action: "added", key: k, val: newVM[k] });
        ...
        vmChanges.push({ action: "updated", key: k, val: newVM[k] });
      }
    });
      ...
      diffHtml = `
                <div class="diff-block">
                    ...
                    <div class="diff-group">
                        <div>Handles: <span class="added">${
        (op.alsoKnownAs || []).join(", ") || "None"
      }</span></div>
                        <div>Services: <span class="added">${
        Object.keys(op.services || {}).join(", ") || "None"
      }</span></div>
                        <div>Keys: <span class="added">${
        (op.rotationKeys || []).length
      } keys</span></div>
                    </div>
                </div>
             `;
            ...
            c.items.forEach((item) => {
              ...
              diffHtml += `
                                <div class="diff-item diff-item-${item.action}">
                                    ...
                                    ${
                item.action === "updated" && item.val.endpoint
                  ? `<br><span class="diff-endpoint">-> ${item.val.endpoint}</span>`
                  : ""
              }
                                </div>
                            `;
...
function renderGraph(doc) {
  ...
  methods.forEach((m, i) => {
    ...
    nodesHtml += `<div class="node key" title="${
      m.publicKeyMultibase || ""
    }" data-top="${top}" data-left="30">Key: ${label}</div>`;

    services.forEach((s, j) => {
      const sTop = 50 + (j * 90);
      linesHtml += `<line x1="130" y1="${top + 15}" x2="350" y2="${
        sTop + 15
      }" stroke="var(--separator-color)" stroke-width="1" stroke-dasharray="4" />`;
    });
  });

  services.forEach((s, i) => {
    const top = 50 + (i * 90);
    const url = typeof s.serviceEndpoint === "string"
      ? s.serviceEndpoint
      : (s.serviceEndpoint?.uri || "N/A");
    nodesHtml +=
      `<div class="node service" title="${url}" data-top="${top}" data-left="350">Service: ${s.type}</div>`;
  });
...
function renderMetricsDashboard(text) {
  ...
  const diskBar = "█".repeat(Math.round(diskRatio / 5)) +
    "░".repeat(20 - Math.round(diskRatio / 5));
  ...
  container.innerHTML = `
        ...
        <table class="param-table mt-sm">
            ...
            <tr><td>L1 Memory</td><td><code>${memBar}</code> ${memRatio}% (${memHits}/${
    memHits + memMisses
  })</td></tr>
            <tr><td>L2 Disk</td><td><code>${diskBar}</code> ${diskRatio}% (${diskHits}/${
    diskHits + diskMisses
  })</td></tr>
        </table>
        <table class="param-table mt-sm">
