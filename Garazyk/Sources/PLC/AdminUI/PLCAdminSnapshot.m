...
- (instancetype)initWithStore:(id<PLCStore>)store syncEngine:(PLCSyncEngine *)syncEngine {
    self = [super init];
    if (self) {
        _store = store;
...
- (NSDictionary<NSString *, id> *)snapshot {
    ...
    dispatch_sync(self.snapshotQueue, ^{
        ...
        NSDictionary *metrics = [[PLCMetrics sharedMetrics] snapshot];
...
- (NSDictionary<NSString *, id> *)directoryEntryForDID:(NSString *)did {
    ...
    dispatch_sync(self.snapshotQueue, ^{
        ...
        PLCOperation *latest = [self.store getLatestOperationForDID:did error:&latestError];
        ...
        result = @{
            ...
            @"verification": @{ @"status": @"recorded", @"failures": [[PLCMetrics sharedMetrics] snapshot][@"verificationFailures"] ?: @0 },
        };
    });
    return result;
}
