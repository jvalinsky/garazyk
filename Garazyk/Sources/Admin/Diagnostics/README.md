...
The System Diagnostics Dashboard provides comprehensive monitoring and management capabilities for the AT Protocol PDS, covering three major areas: Sequencer Health, Blob Storage Audits, and Rate Limit Management.
...
#### Purpose
Monitor real-time and historical performance of the event sequencer, including throughput, subscriber count, and backpressure metrics.

#### Implementation
- **PDSSequencerAnalyticsCollector**: Runs a GCD dispatch timer that wakes every 60 seconds to collect metrics
- Metrics collected:
...
**GET /admin/api/diagnostics/sequencer/stats**
...
**GET /admin/api/diagnostics/sequencer/history?hours=24**
Returns historical data for specified period (default: 24 hours, max: 720 hours / 30 days).
...
- **Initialization**: Created in `PDSApplication.initializeServices()` after `_subscribeReposHandler`
...
#### Purpose
Perform deep integrity checks on blob storage to detect orphaned files, CID mismatches, missing files, and unreferenced blobs. All operations support dry-run mode.
...
**POST /admin/api/diagnostics/blobs/audit**
...
#### Purpose
Query current rate limit status, view top limited users, and perform admin overrides with immutable audit trail.
...
**POST /admin/api/diagnostics/ratelimits/query**
...
**GET /admin/api/diagnostics/ratelimits/top?limit=20**
...
**POST /admin/api/diagnostics/ratelimits/clear**
Clear rate limit with admin override.
