...
- (NSString *)usage {
    return @"kaszlak repo <subcommand> [options]";
}

- (NSString *)helpText {
    return @"Inspect user repositories.\n\n"
           @"Usage: kaszlak repo <subcommand> [options]\n\n"
           @"Subcommands:\n"
           ...
           @"Examples:\n"
           @"  kaszlak repo list did:plc:abc123           # List all records for user\n"
           @"  kaszlak repo get did:plc:abc123 at://did:plc:abc123/app.bsky.feed.post/abc\n"
           @"  kaszlak repo root did:plc:abc123           # Get current repo root";
}
...
- (void)executeCreateRecordWithArgs:(NSArray<NSString *> *)args context:(PDSCLICommandContext *)context {
    if (args.count < 3) {
        [context printError:@"Usage: kaszlak repo create-record <did> <collection> [rkey] <json_value>"];
        return;
    }
...
- (void)executeDeleteRecordWithArgs:(NSArray<NSString *> *)args context:(PDSCLICommandContext *)context {
    if (args.count < 3) {
        [context printError:@"Usage: kaszlak repo delete-record <did> <collection> <rkey>"];
        return;
    }
...
- (void)executeGetWithArgs:(NSArray<NSString *> *)args context:(PDSCLICommandContext *)context {
    if (args.count < 2) {
        [context printError:@"Usage: kaszlak repo get <did> <uri>"];
        return;
    }
...
- (void)executeRepairWithArgs:(NSArray<NSString *> *)args context:(PDSCLICommandContext *)context {
    if (args.count < 1) {
        [context printError:@"Usage: kaszlak repo repair <did>"];
        return;
    }
