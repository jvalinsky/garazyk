...
- (NSString *)helpText {
    return @"Interactive REPL (Read-Eval-Print Loop) mode for kaszlak CLI.\n\n"
           ...
           @"Examples:\n"
           @"  kaszlak repl                           # Start interactive REPL\n"
           @"  kaszlak repl --data-dir /var/lib/kaszlak   # REPL with custom data dir";
}
...
- (int)executeWithArguments:(NSArray<NSString *> *)args context:(PDSCLICommandContext *)context {
    ...
    while (1) {
        ...
        if ([trimmed isEqualToString:@".help"]) {
            ...
            printf("\nAll kaszlak commands are available (serve, account, invite, etc.)\n");
            printf("Use 'kaszlak <command> --help' for command-specific help.\n\n");
            free(line);
            continue;
        }
