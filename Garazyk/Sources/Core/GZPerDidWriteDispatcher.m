...
/*!
 @file GZPerDidWriteDispatcher.m
 ...
 */
@interface GZPerDidWriteState : NSObject {
@public
...
@property (nonatomic, strong) NSMutableArray<GZWriteBlock> *pendingWork;
...
@implementation GZPerDidWriteState

+ (instancetype)stateForDid:(NSString *)did {
    GZPerDidWriteState *state = [[self alloc] init];
    if (state) {
        state->_did = [did copy];
...
/*! Per-DID write state. Key: DID string, Value: GZPerDidWriteState. */
@property (nonatomic, strong) NSMutableDictionary<NSString *, GZPerDidWriteState *> *didStateMap;
...
@implementation GZPerDidWriteDispatcher
...
- (void)_stopInternal {
    ...
    for (GZPerDidWriteState *state in self.didStateMap.allValues) {
        [state destroyMutex];
    }
...
- (void)dispatchWriteForDid:(NSString *)did block:(GZWriteBlock)block {
...
- (void)_processNextPendingForDid:(NSString *)did
                            state:(GZPerDidWriteState *)state
    ...
    if (state.pendingWork.count > 0) {
        GZWriteBlock nextBlock = state.pendingWork[0];
...
- (void)_evictIdleStates {
    ...
    for (NSString *did in self.didStateMap) {
        GZPerDidWriteState *state = self.didStateMap[did];
        pthread_mutex_lock(&state->_mutex);
