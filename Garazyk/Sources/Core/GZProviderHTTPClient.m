...
NSString *const GZProviderHTTPClientErrorDomain = @"com.atproto.pds.providerhttpclient";

@interface GZProviderHTTPClient ()
@property (nonatomic, strong, readwrite) NSURL *baseURL;
@property (nonatomic, copy, readwrite) NSString *authHeader;
@property (nonatomic, strong) id safeHTTPClient;
@end

@implementation GZProviderHTTPClient
...
- (nullable NSDictionary *)postPath:(NSString *)path
    ...
    @try {
        ...
        if (error) {
            *error = [NSError errorWithDomain:GZProviderHTTPClientErrorDomain
                                         code:GZProviderHTTPClientErrorSerializationFailed
                                     userInfo:userInfo];
        }
        return nil;
    }
    if (jsonError || !jsonData) {
        if (error) {
            *error = jsonError ?: [NSError errorWithDomain:GZProviderHTTPClientErrorDomain
                                                      code:GZProviderHTTPClientErrorSerializationFailed
                                                  userInfo:@{NSLocalizedDescriptionKey: @"Failed to serialize JSON body"}];
        }
        return nil;
...
- (nullable NSDictionary *)executeRequestWithRetry:(NSURLRequest *)request
    ...
    while (attempt <= self.maxRetries && !success) {
        ...
        [self.safeHTTPClient performSafeDataTaskWithRequest:request
            ...
            if (taskError) {
                ...
                requestError = [NSError errorWithDomain:GZProviderHTTPClientErrorDomain
                                                    code:GZProviderHTTPClientErrorHTTPError
                                                userInfo:userInfo];
            }
        ...
        if (requestError && [requestError.domain isEqualToString:GZProviderHTTPClientErrorDomain]) {
            if (requestError.code == GZProviderHTTPClientErrorHTTPError) {
    ...
    if (!success && attempt > self.maxRetries) {
        if (error && !requestError) {
            *error = [NSError errorWithDomain:GZProviderHTTPClientErrorDomain
                                          code:GZProviderHTTPClientErrorMaxRetriesExceeded
