...
NSString * const PDSLogComponentHTTP = @"HTTP";
NSString * const PDSLogComponentAdmin = @"Admin";
NSString * const PDSLogComponentService = @"Service";
NSString * const PDSLogComponentCore = @"Core";
NSString * const PDSLogComponentBlob = @"Blob";
NSString * const PDSLogComponentSync = @"Sync";
NSString * const PDSLogComponentExplore = @"Explore";
NSString * const PDSLogComponentCLI = @"CLI";
...
- (void)logWithLevel:(PDSLogLevel)level
    ...
    dispatch_async(self.logQueue, ^{
        ...
        // Text format
        if (self.logFormat == PDSLogFormatText || self.logFormat == PDSLogFormatBoth) {
            ...
            if (self.asyncLogging) {
                dispatch_async(self.ioQueue, ^{
                    ...
                    if (self.logBuffer.count >= 100 || level == PDSLogLevelError) {
        ...
        // JSON format
        if (self.logFormat == PDSLogFormatJSON || self.logFormat == PDSLogFormatBoth) {
            ...
            if (jsonData) {
                ...
                if (self.printToStdout && self.logFormat == PDSLogFormatJSON) {
...
- (void)setCorrelationID:(NSString *)correlationID {
    if (correlationID) {
        [[NSThread currentThread] threadDictionary][@"PDSLoggerCorrelationID"] = correlationID;
    } else {
        [[NSThread currentThread] threadDictionary] [@"PDSLoggerCorrelationID"] = nil;
    }
}
...
- (NSString *)correlationID {
    return [[NSThread currentThread] threadDictionary][@"PDSLoggerCorrelationID"];
}
