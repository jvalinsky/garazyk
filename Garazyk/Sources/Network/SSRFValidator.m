...
#if defined(__APPLE__)
// Apple implementation using CFHost (CFNetwork)
...
#else
// GNUstep/Linux implementation using getaddrinfo()
+ (BOOL)validateHostResolvesToPublicIP:(NSString *)hostname error:(NSError **)error {
    if (hostname.length == 0) {
        if (error) {
            *error = [NSError errorWithDomain:SSRFValidatorErrorDomain
    ...
    if (gaiError != 0 || result == NULL) {
        if (error) {
            NSString *message = gaiError != 0 ?
                [NSString stringWithUTF8String:gai_strerror(gaiError)] :
                @"No addresses found";
            *error = [NSError errorWithDomain:SSRFValidatorErrorDomain
                                         code:SSRFValidatorErrorResolutionFailed
                                     userInfo:@{NSLocalizedDescriptionKey: message ?: @"Failed to resolve hostname"}];
        }
        return NO;
    }
