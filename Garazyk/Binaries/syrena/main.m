...
/// Writes a crash report to /tmp/syrena-crash.log using async-signal-safe calls.
static void crash_signal_handler(int sig) {
    ...
    int len = snprintf(buf, sizeof(buf),
        "\n=== FATAL SIGNAL %s (%d) in syrena ===\n", signame, sig);
    write(STDERR_FILENO, buf, (size_t)len);

    int fd = open("/tmp/syrena-crash.log",
...
/// Installs crash signal handlers using sigaction and the uncaught exception handler.
