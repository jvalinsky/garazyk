...
| `Session` (`PDSSQLiteSessionStorage`) | **Migrated 2026-07-11** | The SQLite session storage behind the `PDSSessionStorage` protocol (memory sessions use `PDSMemorySessionStorage`). Plan premise corrected: the SQLite class had **no** hand-rolled queue (that was the memory sibling), so the migration *adds* serialization via ConnectionManagerSerial. It was covered by one persistence test → added 7 file-backed characterization tests first, then migrated raw `sqlite3` (save/get/revoke/list) onto ConnectionManagerSerial + QueryRunner. Verified: SessionStore 24/24, OAuth2 76/76, OAuthDPoP 13/13 |
| `PDSReplayCache` | **Migrated 2026-07-11** | First Tier-2 migration done: characterization strengthened to 10 tests, then migrated off its hand-rolled dispatch queue + raw `sqlite3` onto ConnectionManagerSerial + QueryRunner. The atomic check-and-add became a `performWriteTransaction:` (Transactor `executeQuery` + `executeUpdate`) — the first real adopter of the step-2/3 Transactor. Incidentally removed a latent self-deadlock (cleanup timer now on a dedicated queue, not the db queue) |
...
| `PLCPersistentStore` | Defer (Tier 3) — **foundation landed 2026-07-11** | Config feasibility confirmed and landed: `configureDatabase` now uses shared `ATProtoDBConfigurePragmas` + a `PLCPersistentStoreDBConfig` that exactly replicates the tuned pragmas (foreign_keys, 64 MB cache, WAL, synchronous NORMAL, busy_timeout 5s, wal_autocheckpoint 1000) — the value to hand ConnectionManagerSerial. Remaining connection/query migration is **atomic**: `PLCReplicaStore` uses the parent's `db`/`transactionQueue`/`prepareStatement` directly, so it must migrate in the same change (no reads-only partial). Accepted tradeoff: drop the statement cache (per-call prepare). 932-line parent + ~150-line transactional `appendOperation` + subclass |
