...
| `PLCReplicaStore` | **Migrated 2026-07-12** | Migrated onto inherited `queryRunner` (`executeUpdate:`/`executeQuery:`). All 9 characterization tests (`PLCReplicaStoreTests`) verified green. |
| `PLCPersistentStore` | **Migrated 2026-07-12** | Migrated off hand-rolled `db`/`transactionQueue`/`stmtCache` onto `ATProtoConnectionManagerSerial` + `ATProtoDatabaseQueryRunner`. Core methods run inside serial connection manager blocks; subclass `PLCReplicaStore` routes via `queryRunner`. Verified across 61 PLC tests (`PLCStoreTests` 11/11, `PLCReplicaStoreTests` 9/9, `PLCAuditorTests` 11/11, etc.). |
