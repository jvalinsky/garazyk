...
function initAdminPanel(bridge) {
  ...
  function renderServiceCards(cards) {
    ...
    grid.style.cssText =
      "display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:var(--space-md);margin-bottom:var(--space-xl);";
      ...
      el.innerHTML = `
        ...
        card.detail
          ? `<div style="font-size:var(--font-size-xs);color:var(--color-text-tertiary);margin-top:2px;">${
            escapeHtml(card.detail.substring(0, 80))
          }</div>`
          : ""
      }
                </div>
  ...
  async function doAccountSearch() {
    ...
    if (resp.ok && resp.data) {
      ...
      let html =
        '<table class="skylab-admin-table" style="width:100%;font-size:var(--font-size-xs);border-collapse:collapse;">';
      html +=
        "<thead><tr><th>Handle</th><th>DID</th><th>Email</th><th>Status</th></tr></thead><tbody>";
      for (const info of infos) {
        html += `<tr>
                    <td style="padding:var(--space-xs);">${escapeHtml(info.handle || "—")}</td>
                    <td style="padding:var(--space-xs);font-family:var(--font-mono);">${
          escapeHtml(info.did || "—")
        }</td>
      ...
      resultsEl.innerHTML = `<div class="skylab-empty-state">Search failed: ${
        escapeHtml(resp.data?.error || "unknown")
      }</div>`;
    }
  }
  ...
  async function loadAppViewStatus() {
    ...
    if (resp.ok && resp.data) {
      el.innerHTML =
        `<pre style="font-size:var(--font-size-xs);color:var(--color-text-secondary);overflow:auto;max-height:200px;">${
          escapeHtml(JSON.stringify(resp.data, null, 2))
        }</pre>`;
    } else {
