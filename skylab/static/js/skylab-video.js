...
(function () {
  ...
  function getVideoBaseUrl(bridge) {
    return bridge?.services?.video ||
      (typeof bridge?.serviceUrl === "function" ? bridge.serviceUrl("video") : null);
  }
  ...
  async function getVideoServiceToken(bridge) {
    ...
    if (authResp.ok) {
      return authResp.data?.token || authResp.data?.serviceJwt || authResp.data?.jwt ||
        authResp.data?.accessJwt || null;
    }
  ...
  async function getVideoJobStatus(bridge, jobId, token = null) {
    ...
    const headers = {};
    if (token || bridge?.auth?.accessJwt) {
      headers.Authorization = `Bearer ${token || bridge.auth.accessJwt}`;
    }
  ...
  function initVideoPanel(bridge) {
    ...
    if (
      !panel || !uploadCard || !fileInput || !uploadBtn || !progressEl || !progressFill ||
      !progressText || !jobsEl || !bridge
    ) {
      return null;
    }
    ...
    function normalizeJobState(value) {
      ...
      if (
        upper.includes("PROCESS") || upper.includes("RUN") || upper.includes("UPLOAD") ||
    ...
    function normalizeJob(payload, fallbackJobId = null) {
      ...
      const stateClass = normalizeJobState(
        data.state || data.status || payload?.state || payload?.status,
      );
      const progress = toPercent(
        data.progress ?? data.percent ?? data.completion ?? data.completedPercent ??
          payload?.progress ?? payload?.percent,
        stateClass,
      );
      ...
      const cid = data.cid || data.blobCid || data.videoCid || blob?.ref?.$link || blob?.cid ||
        payload?.cid || null;
      const playlistUrl = data.playlistUrl || data.hlsUrl || payload?.playlistUrl ||
        payload?.hlsUrl || null;
      ...
      return {
        jobId,
        rawState: String(
          data.state || data.status || payload?.state || payload?.status || "pending",
        ),
        state: stateClass,
    ...
    function renderJobs() {
      ...
      for (const job of jobs) {
        ...
        if (job.state === "completed" && job.blobUrl) {
          ...
          const cdnPlaylistUrl = directVideoUrl(
            bridge,
            `watch/${job.did}/${job.cid}/playlist.m3u8`,
          );
          const previewLabel = document.createElement("div");
          previewLabel.style.marginTop = "var(--space-sm)";
          previewLabel.style.fontSize = "var(--font-size-xs)";
    ...
    async function refreshUploadLimits() {
      ...
      try {
        ...
        const remaining = data.remaining ?? data.remainingDaily ?? data.remainingUploads ??
          data.uploadsRemaining ?? data.available ?? null;
        const limit = data.limit ?? data.dailyLimit ?? data.maxUploads ?? data.maxDailyUploads ??
          null;
        ...
        limitsEl.textContent = `Remaining daily uploads: unavailable (${
