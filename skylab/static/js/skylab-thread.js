(function(global) {
    ...
    function initThreadPanel(bridge, router) {
        ...
        function showLoading() { container.innerHTML = '<div class="skylab-empty-state">Loading thread\u2026</div>'; }
        ...
        function renderThread(did, rkey) {
            ...
            bridge.xrpc('app.bsky.feed.getPostThread', { uri: 'at://' + did + '/app.bsky.feed.post/' + rkey, depth: 10 },
                null, { service: 'appview', auth: !!bridge.auth }
            ).then(resp => {
                if (!resp.ok) { showError('Thread not found'); return; }
                const thread = resp.data?.thread;
                if (!thread) { showError('Thread not found'); return; }
