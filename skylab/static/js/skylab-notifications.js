(function(global) {
    ...
    function initNotificationsPanel(bridge) {
        ...
        function showLoading() { container.innerHTML = '<div class="skylab-empty-state">Loading notifications\u2026</div>'; }

        function showEmpty() { container.innerHTML = '<div class="skylab-empty-state">No notifications</div>'; }

        function reasonIcon(reason) {
            const icons = { like: '\u2764', follow: '\u2795', reply: '\u21A9', repost: '\u267B', quote: '\u201C' };
            return icons[reason] || '\u25CF';
        }
        ...
        async function load() {
            showLoading();
            const resp = await bridge.xrpc('app.bsky.notification.listNotifications', { limit: 30 }, null, { service: 'appview', auth: true });
            if (resp.ok) {
