...
function initChatPanel(bridge) {
    ...
    function renderConversationList() {
        ...
        for (const convo of conversations) {
            ...
            item.innerHTML = `
                <div style="font-weight:600;font-size:var(--font-size-sm);">${escapeHtml(members)}</div>
                <div style="font-size:var(--font-size-xs);color:var(--color-text-secondary);margin-top:2px;">${escapeHtml(preview)}</div>
            `;
    ...
    async function selectConversation(convoId) {
        ...
        const resp = await bridge.xrpc('chat.bsky.convo.getMessages', {
            convoId: convoId, limit: 50,
        }, null, { service: 'chat', auth: true });
    ...
    async function sendMessage(input) {
        ...
        if (currentMode === 'e2ee') {
            ...
            appendSystemMessage('E2EE messaging requires Germ SDK integration. Falling back to plaintext for now.');
        }
    ...
    function appendSystemMessage(text) {
        ...
        const el = document.createElement('div');
        el.style.cssText = 'text-align:center;color:var(--color-text-tertiary);font-size:var(--font-size-xs);padding:var(--space-sm);';
        el.textContent = text;
