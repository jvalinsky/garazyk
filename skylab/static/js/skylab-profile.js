(function(global) {
    ...
    function initProfilePanel(bridge, router) {
        ...
        function showLoading() { container.innerHTML = '<div class="skylab-empty-state">Loading profile\u2026</div>'; }

        function showError(msg) { container.innerHTML = '<div class="skylab-empty-state">' + msg + '</div>'; }
        ...
        function renderProfile(actor) {
            ...
            Promise.all([
                bridge.xrpc('app.bsky.actor.getProfile', { actor: actor }, null, { service: 'appview', auth: !!bridge.auth }),
                bridge.xrpc('app.bsky.feed.getAuthorFeed', { actor: actor, limit: 25 }, null, { service: 'appview', auth: !!bridge.auth }),
                bridge.xrpc('app.bsky.graph.getFollows', { actor: actor, limit: 50 }, null, { service: 'appview', auth: !!bridge.auth }),
                bridge.xrpc('app.bsky.graph.getFollowers', { actor: actor, limit: 50 }, null, { service: 'appview', auth: !!bridge.auth }),
            ]).then(([profileResp, feedResp, followsResp, followersResp]) => {
                if (!profileResp.ok) { showError('Profile not found'); return; }
                ...
                const followers = Array.isArray(followersResp.data?.followers) ? followersResp.data.followers : [];
        ...
        function buildTabs(feed, follows, followers) {
            ...
            function showTab(tabId) {
                activeTab = tabId;
                tabBar.querySelectorAll('.skylab-tab').forEach(t => t.classList.toggle('active', t.dataset.tab === tabId));
                content.innerHTML = '';
