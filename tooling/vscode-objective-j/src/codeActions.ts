 ...
 */
export class ObjJCodeActionProvider implements vscode.CodeActionProvider {
  ...
  private createAddImportAction(
    ...
    if (classDef.uri.toString() === document.uri.toString()) return undefined;
  ...
  private createMethodStubAction(
    ...
    const match = diagnostic.message.match(
      /Class '(\w+)' declares conformance to <(\w+)> but does not implement '([^']+)'/
    );
    ...
    const cls = file.result.classes.find(
      (c) => c.name === className && !c.category
    );
    ...
    if (method.params.length > 0) {
      const paramParts = method.params.map(
        (p) => `${p.label}:(${p.type})${p.name}`
      );
