 ...
 */
function isMethodContinuationLine(trimmed: string): boolean {
...
function formatLines(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (braceDepth > 0) {
      ...
      if (closingFirst) level = braceDepth - 1;
  ...
  function computeIndentLevel(): number {
    if (!inImplOrProto) return 0;
    if (inIvarBlock) return 1;
 ...
 */
export class ObjJDocumentFormattingEditProvider
 ...
 */
export class ObjJDocumentRangeFormattingEditProvider
