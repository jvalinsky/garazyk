...
func (r *NameAssertionAlignmentRule) normalizeTestName(name string) string {
	...
	// Handle "testWhen*Then*" pattern - keep both parts
...
func (r *NameAssertionAlignmentRule) keywordsMatch(keyword1, keyword2 string) bool {
	...
	synonyms := map[string][]string{
		...
		"auth":       {"oauth", "token", "authorization", "authentication"},
		"token":      {"oauth", "jwt", "auth", "bearer"},
		"validation": {"validate", "valid", "verify", "check"},
		"error":      {"fail", "exception", "throw", "invalid", "reject"},
		"parse":      {"parser", "parsing", "decode", "deserialize"},
		"serialize":  {"serializer", "encode", "encoding"},
		"did":        {"identifier", "identity", "decentralized"},
		"handle":     {"username", "name", "identifier"},
		"reject":     {"invalid", "error", "fail", "throw"},
		"accept":     {"valid", "success", "allow"},
	}
...
func (r *NameAssertionAlignmentRule) determineSeverity(score float64) Severity {
	if score < 0.3 {
		return CRITICAL // Likely false positive
	} else if score < 0.5 {
...
func (r *NameAssertionAlignmentRule) generateMessage(testName string, claimedKeywords, assertionSemantics []string, score float64) string {
	return fmt.Sprintf(
		"Test name '%s' claims to test [%s] but assertions validate [%s]. Alignment score: %.2f (threshold: 0.50)",
		testName,
