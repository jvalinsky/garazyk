	...
	for _, fixturePath := range fixturePaths {
		if !r.fixturePathExists(fixturePath, ctx.TestFile.Path) {
			findings = append(findings, Finding{
				...
				Recommendation: "Verify the fixture path is correct or create the missing fixture file in ATProtoPDS/Tests/fixtures/",
				Confidence:     0.8,
			})
		}
	}
...
func (r *TestFixtureRule) fixturePathExists(fixturePath, testFilePath string) bool {
	...
	// Try relative to ATProtoPDS/Tests/fixtures/
	// Extract the base path (assuming testFilePath contains ATProtoPDS/Tests/)
	if idx := strings.Index(testFilePath, "ATProtoPDS/Tests/"); idx != -1 {
		basePath := testFilePath[:idx]
		fixturesPath := filepath.Join(basePath, "ATProtoPDS/Tests/fixtures", fixturePath)
