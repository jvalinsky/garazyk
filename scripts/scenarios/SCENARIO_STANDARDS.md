...
All scenarios in `scripts/scenarios/scenarios/*.ts` must follow this documentation template to ensure clarity for developers and automated consistency.
...
1.  **Module Header:** Top-level comment block with `@module`, a brief scenario description, the behavioral steps, and expectations.
2.  **Function Documentation:** Exactly one JSDoc block immediately before the exported `run` function.
