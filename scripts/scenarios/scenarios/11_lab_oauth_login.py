...
from scripts.lib.atproto import ScenarioResult
...
def run() -> ScenarioResult:
    ...
    try:
        ...
        if health_resp.status_code == 200:
            ...
            result.step_failed("UI Server health check",
    ...
    try:
        ...
        body = lab_resp.text
        if lab_resp.status_code == 200 and content_type.lower().startswith("text/html") and "lab-login-section" in body:
            result.step_passed("Lab page loads", f"content_type={content_type!r}")
        else:
            result.step_failed("Lab page loads",
                               f"status={lab_resp.status_code} content_type={content_type!r}")
    except Exception as exc:
        result.step_failed("Lab page loads", str(exc))

    try:
        meta_resp = requests.get(f"{ui_url}/lab/client-metadata.json",
        ...
        checks = [
            ...
            isinstance(metadata.get("grant_types"), list) and "authorization_code" in metadata.get("grant_types", []),
            isinstance(metadata.get("response_types"), list) and "code" in metadata.get("response_types", []),
        ...
        if all(checks):
            result.step_passed("Lab client metadata valid",
                               f"client_id={metadata.get('client_id')!r}")
        else:
            result.step_failed("Lab client metadata valid",
                               json.dumps({"status": meta_resp.status_code, "content_type": content_type,
                                           "missing_keys": missing_keys, "client_id": metadata.get("client_id"),
    ...
    try:
        login_resp = requests.post(f"{ui_url}/admin/login",
                                   json={"password": admin_password},
                                   headers={"Content-Type": "application/json"},
                                   timeout=10)
        if login_resp.status_code != 200:
            result.step_failed("Admin login flow", f"status={login_resp.status_code}")
        else:
            ...
            if admin_token:
                ...
                result.step_failed("Admin login flow",
    ...
    if admin_cookie_header:
        try:
            auth_admin_resp = requests.get(f"{ui_url}/admin",
            ...
            if auth_admin_resp.status_code == 200:
                ...
                result.step_failed("Admin authenticated access",
    ...
    if admin_cookie_header:
        try:
            htmx_auth_resp = requests.get(f"{ui_url}/admin/partials/overview",
                                          headers={"HX-Request": "true", "Cookie": admin_cookie_header},
                                          allow_redirects=False, timeout=10)
            if htmx_auth_resp.status_code == 200:
                result.step_passed("Admin HTMX with auth", "HTMX request with cookie returned 200")
            else:
                result.step_failed("Admin HTMX with auth",
    ...
    if admin_cookie_header:
        try:
            logout_resp = requests.post(f"{ui_url}/admin/logout",
                                        headers={"Cookie": admin_cookie_header},
                                        timeout=10)
            if logout_resp.status_code != 200:
                result.step_failed("Admin logout",
                                   f"POST /admin/logout returned status={logout_resp.status_code}")
            else:
                post_logout_resp = requests.get(f"{ui_url}/admin",
                                                headers={"Cookie": admin_cookie_header},
                                                allow_redirects=False, timeout=10)
                if post_logout_resp.status_code == 302:
                    result.step_passed("Admin logout",
