...
async function follow(client: XrpcClient, followerName: string, targetName: string, result: ScenarioResult) {
...
export async function run(): Promise<ScenarioResult> {
  ...
  if (active.length < 4) {
    result.stepFailed("Account creation", `Only ${active.length} accounts created, need at least 4`);
    result.finish();
    return result;
  }
  ...
  await timedCall(
    ...
    async () => {
      const res = await client.raw.get("app.bsky.graph.getFollows", { actor: marcus.did }, marcus.accessJwt);
      return res;
    },
  ...
  await timedCall(
    ...
    async () => {
      const res = await client.raw.get("app.bsky.graph.getFollowers", { actor: luna.did }, luna.accessJwt);
      return res;
    },
  ...
  await timedCall(
    ...
    async () => {
      const res = await client.raw.get("app.bsky.actor.getProfile", { actor: marcus.did }, marcus.accessJwt);
      return res;
    },
  ...
  if (!luna.did || !luna.accessJwt) {
    ...
    await timedCall(
      ...
      async () => {
        const res = await client.raw.get("app.bsky.actor.searchActors", { q: "Luna" }, luna.accessJwt);
        return res;
      },
