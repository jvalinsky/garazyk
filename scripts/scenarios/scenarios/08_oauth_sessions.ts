...
export async function run(): Promise<ScenarioResult> {
  ...
  try {
    const authUrl = `${PDS1}/oauth/authorize?client_id=scenario-test-client&redirect_uri=${PDS1}/oauth/callback&response_type=code&scope=atproto&state=test-state-123`;
    ...
    if (authResp.status === 400 && body.error === "invalid_request") {
      ...
      result.stepFailed("OAuth authorize enforces PAR", `status=${authResp.status} body=${JSON.stringify(body)}`);
    }
  } catch (exc: any) {
  ...
  try {
    await client.raw.post("com.atproto.server.deleteSession", {}, marcus.refreshJwt || marcus.accessJwt);
  ...
  if (marcusRefreshJwt) {
    ...
    result.stepSkipped("Refresh after deleteSession fails", "no refreshJwt returned by createSession");
  }
