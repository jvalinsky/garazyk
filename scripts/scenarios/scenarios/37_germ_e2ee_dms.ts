...
export async function run(): Promise<ScenarioResult> {
  ...
  for (const char of [luna, marcus]) {
    const session = await client.accounts.createAccount(char.handle, char.email, char.password).catch(() => 
  ...
  const convo = await timedCall(result, "Vanilla: Get convo", async () => {
    return await client.raw.xrpcGet("chat.bsky.convo.getConvoForMembers", { members: [luna.did, marcus.did] }, luna.accessJwt);
  });
  ...
  if (convoId) {
    await timedCall(result, "Vanilla: Send plaintext", async () => {
      return await client.raw.xrpcPost("chat.bsky.convo.sendMessage", {
        convoId, message: { $type: "chat.bsky.convo.message", text: plaintext, createdAt: now() }
      }, luna.accessJwt);
    });
  ...
  if (germHealthy) {
    ...
    const claim = await timedCall(result, "Germ: Luna claims addresses", async () => {
      return await germPost("com.germnetwork.mailbox.claimAddresses", { agentRef: "luna-1", count: 3 }, luna.accessJwt);
    });

    const mClaim = await germPost("com.germnetwork.mailbox.claimAddresses", { agentRef: "marcus-1", count: 3 }, marcus.accessJwt);
    ...
    if (mClaim?.addresses?.length > 0) {
      ...
      await timedCall(result, "Germ: Deliver ciphertext", async () => {
        return await germPost("com.germnetwork.mailbox.deliver", {
          address: mClaim.addresses[0], ciphertext: { $bytes: ctB64 }
        }, luna.accessJwt);
      });
