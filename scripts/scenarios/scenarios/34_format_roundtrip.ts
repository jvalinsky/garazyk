...
export async function run(ctx: ScenarioContext): Promise<ScenarioResult> {
  ...
  const [s1, ct1, body1] = await client.raw.xrpcGetBinary(
  ...
  const [s2, ct2, body2] = await client.raw.xrpcGetBinary(
    "com.atproto.sync.getRepo",
    {
      params: { did: luna.did },
  ...
  const head = await client.raw.xrpcGet("com.atproto.sync.getHead", {
    did: luna.did,
  });
  result.stepPassed(
  ...
  const records = await client.records.listRecords(
    luna.did,
    "app.bsky.feed.post",
  );
