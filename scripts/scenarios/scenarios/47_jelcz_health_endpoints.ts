/**
 * @module scenarios/47_jelcz_health_endpoints
 *
...
export async function run(): Promise<ScenarioResult> {
  ...
  // Check that /_health returns 200 via XRPC client
  await timedCall(result, "Jelcz health via XRPC client", async () => {
    const health = await videoClient.raw.xrpcGet(
      "app.bsky.video.getJobStatus",
      { jobId: "_health_check" },
      luna.accessJwt,
    );
    // This should fail gracefully (no such job) rather than crash
    assert(health !== undefined, "Expected a response (error or valid) from getJobStatus");
  });
