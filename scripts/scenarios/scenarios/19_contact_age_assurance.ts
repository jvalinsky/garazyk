...
export async function run(ctx: ScenarioContext): Promise<ScenarioResult> {
  ...
  try {
    ...
    if (marcus.accessJwt) {
      await timedCall(
        result,
        "Marcus begins age assurance",
        async () => {
          return await client.ageAssurance.beginAgeAssurance(
            marcus.email,
            "en",
            "US",
            { regionCode: "CA", token: marcus.accessJwt },
          );
        },
      );
      ...
      await timedCall(
        result,
        "Marcus age assurance state",
        async () => {
          return await client.ageAssurance.getAgeAssuranceState("US", {
            regionCode: "CA",
