...
find "$DATA_DIR" -name "data.sqlite" -not -path "*/service.sqlite" 2>/dev/null | while read -r db_path; do
    # Preserve directory structure relative to DATA_DIR
    rel_path="${db_path#$DATA_DIR/}"
    dest_path="$BACKUP_DEST/$rel_path"
    ...
    # Extract DID directory name for labeling
    did_dir=$(basename "$(dirname "$db_path")")
    backup_db "$db_path" "$dest_path" "user/$did_dir"
    USER_COUNT=$((USER_COUNT + 1))
done
...
echo "Pruning backups older than $RETENTION_DAYS days..."
PRUNED=0
find "$BACKUP_DIR" -name "pds-backup-*.tar.gz" -mtime +"$RETENTION_DAYS" -print -delete | while read -r old; do
    echo "  Removed: $(basename "$old")"
    PRUNED=$((PRUNED + 1))
done
...
if [ "$ERRORS" -gt 0 ]; then
    ...
    echo "Status: All databases backed up successfully"
fi
