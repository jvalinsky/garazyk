...
echo "--- Setting default hostname in config ---"
echo '{"pds": {"hostname": "test.com"}}' > "$PROJECT_ROOT/config.json"

echo "--- Creating accounts ---"
"$CLI" account create --email alice@test.com --handle alice --data-dir "$DATA" --config "$PROJECT_ROOT/config.json"
...
"$CLI" repo create-record "$ALICE" "app.bsky.feed.post" "post1" \
    ...
    --data-dir "$DATA" --config "$PROJECT_ROOT/config.json"
...
POST1_JSON=$("$CLI" repo get "$ALICE" "at://$ALICE/app.bsky.feed.post/post1" --data-dir "$DATA" --config "$PROJECT_ROOT/config.json" --json)
...
"$CLI" repo list "$ALICE" --data-dir "$DATA" --config "$PROJECT_ROOT/config.json"
echo ""
echo "--- Verification: Bob's Repo ---"
"$CLI" repo list "$BOB" --data-dir "$DATA" --config "$PROJECT_ROOT/config.json"
