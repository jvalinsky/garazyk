...
import {
  getRotationKeys, verifyOperationSignature,
  verifyOperationSignatureWithPrev,
  fetchExportBatch, DIDStore,
} from './lib/plc.mjs'

// ── Option definitions ────────────────────────────────────────────

const OPTIONS = [
  option({ name: 'mode',      flag: '--mode',       type: 'string',  default: 'sequential',  env: 'PLC_SYNC_MODE',  description: 'Processing mode: sequential or concurrent' }),
  ...
  option({ name: 'count',     flag: '--count',      type: 'int',     default: 1000,          env: 'PLC_COUNT',      description: 'Total operations to process' }),
  ...
  option({ name: 'verbose',   flag: '--verbose',    short: '-v', type: 'boolean', default: false,          description: 'Print per-batch details' }),
]
  ...
  while (totalOps < args.count) {
    ...
    if (args.verbose || result.failed > 0) {
      const failStr = result.failed > 0
        ? ` ${result.failed} failed (${Object.entries(result.reasons).map(([r, c]) => `${c}x ${r}`).join(', ')})`
        : ''
      console.log(`  Batch ${String(batchNum).padStart(4)}: ${result.passed} passed${failStr}  cursor=${cursor}`)
    }
