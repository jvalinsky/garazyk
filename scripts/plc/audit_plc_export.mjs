...
import {
  getRotationKeys, classifyDidKey, validateFields,
  fetchExportBatch,
} from './lib/plc.mjs'
...
const OPTIONS = [
  option({ name: 'after',     flag: '--after',      type: 'string', default: null,          env: 'PLC_AFTER',      description: 'Start from this export cursor (ISO timestamp)' }),
  option({ name: 'count',     flag: '--count',      type: 'int',    default: 1000,          env: 'PLC_COUNT',      description: 'Total operations to analyze' }),
  option({ name: 'batchSize', flag: '--batch-size',  type: 'int',    default: 100,           env: 'PLC_BATCH_SIZE', description: 'Operations per batch' }),
  option({ name: 'server',    flag: '--server',     type: 'string', default: 'https://plc.directory', env: 'PLC_SERVER', description: 'PLC directory URL' }),
]
...
async function main() {
  ...
  console.log(`Genesis (prev=null): ${genesisCount} (${(genesisCount/totalOps*100).toFixed(1)}%)`)
  console.log(`Non-genesis:         ${nonGenesisCount} (${(nonGenesisCount/totalOps*100).toFixed(1)}%)`)
  console.log(`Nullified:           ${nullifiedCount}`)

  console.log('\nOperation types:')
  for (const [type, count] of Object.entries(typeCounts).sort((a, b) => b[1] - a[1])) {
    console.log(`  ${String(count).padStart(6)}x  ${type} (${(count/totalOps*100).toFixed(1)}%)`)
  }
  ...
  if (batchStats.length > 1) {
    ...
    console.log(`  Batch 0:    ${first.genesis} genesis, ${first.nonGenesis} non-genesis  cursor=${first.cursor}`)
    if (batchStats.length > 2) {
      console.log('  ...')
    }
    console.log(`  Batch ${batchStats.length - 1}:  ${last.genesis} genesis, ${last.nonGenesis} non-genesis  cursor=${last.cursor}`)
    ...
    console.log(`  Average:    ${avgGenesis.toFixed(1)} genesis, ${avgNonGenesis.toFixed(1)} non-genesis per batch`)
  }
}
