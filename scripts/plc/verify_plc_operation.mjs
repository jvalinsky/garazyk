...
import {
  bytesToHex, encodeUnsigned,
  cborRoundTripCheck, deriveDID, calculateCID,
...
const OPTIONS = [
  ...
  option({ name: 'json',     flag: '--json',    type: 'string', default: null,                     description: 'Raw JSON operation to verify' }),
  option({ name: 'stdin',    flag: '--stdin',   type: 'boolean', default: false,                    description: 'Read JSON operation from stdin' }),
  option({ name: 'verbose',  flag: '--verbose', short: '-v', type: 'boolean', default: false,       description: 'Print CBOR hex, hash hex, and CID' }),
]
...
async function verifyOperation(op, did, cid, verbose) {
  ...
  if (verbose) {
    ...
    console.log(`${ok ? 'PASS' : 'FAIL'}  ${did || derivedDid}  type=${op.type}  sig=${sigValid ? 'valid' : 'INVALID'}`)
  }
