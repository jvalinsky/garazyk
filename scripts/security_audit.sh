...
done < <(find "$PROJECT_ROOT/ATProtoPDS/Sources/" -name "*.m")
...
if grep -rn "CC_SHA256.*password" --include="*.m" "$PROJECT_ROOT/ATProtoPDS/Sources/" | grep -v "Legacy" | grep -v "legacy" | grep -v "//"; then
...
if grep -rn "600000" --include="*.m" "$PROJECT_ROOT/ATProtoPDS/Sources/" | grep -i "iteration"; then
    pass "PBKDF2 using recommended 600,000 iterations"
elif grep -rn "CCKeyDerivationPBKDF" --include="*.m" "$PROJECT_ROOT/ATProtoPDS/Sources/"; then
    # Found PBKDF2 but not 600k iterations
    ITER_COUNT=$(grep -rn "iterations.*=" --include="*.m" "$PROJECT_ROOT/ATProtoPDS/Sources/" | grep -oE "[0-9]+" | sort -n | tail -1)
...
if grep -rn "skipSSRFCheck.*=.*YES" --include="*.m" "$PROJECT_ROOT/ATProtoPDS/Sources/" | grep -v "//"; then
    fail "Found code that sets skipSSRFCheck = YES"
    echo "   SSRF protection should not be disabled in production"
elif grep -rn "skipSSRFCheck.*=" --include="*.m" "$PROJECT_ROOT/ATProtoPDS/Sources/" | grep -v "//"; then
...
if grep -rn "readonly.*skipSSRFCheck" --include="*.h" "$PROJECT_ROOT/ATProtoPDS/Sources/"; then
...
if grep -rn '"http://' --include="*.m" "$PROJECT_ROOT/ATProtoPDS/Sources/Database/" | grep redirect_uri | grep -v "localhost" | grep -v "127.0.0.1"; then
...
for range in "${REQUIRED_RANGES[@]}"; do
    if ! grep -q "$range" "$PROJECT_ROOT/ATProtoPDS/Sources/Identity/HandleResolver.m"; then
...
if grep -q "IPv4-mapped" "$PROJECT_ROOT/ATProtoPDS/Sources/Identity/HandleResolver.m"; then
    if grep -q "::ffff:0:0/96" "$PROJECT_ROOT/ATProtoPDS/Sources/Identity/HandleResolver.m"; then
...
if grep -B3 "seedTestClient" "$PROJECT_ROOT/ATProtoPDS/Sources/Auth/OAuth2Handler.m" | grep -q "#ifdef DEBUG"; then
...
if grep -A5 -- "- (BOOL)seedTestClient" "$PROJECT_ROOT/ATProtoPDS/Sources/Database/PDSDatabase.m" | grep -q "#ifndef DEBUG"; then
...
if grep -q "url.scheme.*https" "$PROJECT_ROOT/ATProtoPDS/Sources/Auth/OAuth2Handler.m"; then
