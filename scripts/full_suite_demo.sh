...
ensure_process_running() {
    ...
    log_error "$label process exited during startup"
    if [[ -f "$log_path" ]]; then
        tail -40 "$log_path" >&2 || true
    fi
    error_exit "$label failed to start"
}
...
rm -rf "$DEMO_ROOT"
mkdir -p "$DEMO_ROOT"/{plc,pds,relay,appview,chat,video,ui,config}
mkdir -p "$LOG_DIR"
> "$PID_FILE"

PDS_CONFIG_PATH="$DEMO_ROOT/config/pds-config.json"
cat > "$PDS_CONFIG_PATH" <<EOF
{
  "server": {
    "host": "127.0.0.1",
    "port": $SERVICE_PORT_PDS,
    "data_dir": "$DEMO_ROOT/pds",
    "issuer": "$PDS_URL",
  ...
  "database": {
    "service_pool_max_size": 10,
    "user_pool_max_size": 50
  },
  "logging": {
    "format": "text",
    "level": "info"
  ...
  "auth": {
    "master_secret": "$PDS_MASTER_SECRET"
  }
}
EOF
...
PDS_PLC_KEYS_DIR="$DEMO_ROOT/pds/keys" \
    "$BUILD_DIR/kaszlak" serve --config "$PDS_CONFIG_PATH" --port "$SERVICE_PORT_PDS" --data-dir "$DEMO_ROOT/pds" --foreground \
    > "$LOG_DIR/pds.log" 2>&1 &
echo "PID=$!" >> "$PID_FILE"
...
sleep 1
ENCODED_URL=$(UPSTREAM_URL="ws://127.0.0.1:$SERVICE_PORT_PDS/xrpc/com.atproto.sync.subscribeRepos" deno eval 'console.log(encodeURIComponent(Deno.env.get("UPSTREAM_URL") ?? ""))' 2>/dev/null || echo "ws%3A%2F%2F127.0.0.1%3A$SERVICE_PORT_PDS%2Fxrpc%2Fcom.atproto.sync.subscribeRepos")
...
APPVIEW_PID=$!
echo "PID=$APPVIEW_PID" >> "$PID_FILE"

log_info "Waiting for AppView backfill status endpoint..."
wait_for_service appview 60
ensure_process_running "$APPVIEW_PID" "AppView" "$LOG_DIR/appview.log"
...
if [[ "$SKIP_SEED" != "true" ]]; then
    ...
    if [[ -f "$SCRIPT_DIR/seed_full_suite.ts" ]]; then
        deno run -A "$SCRIPT_DIR/seed_full_suite.ts"
        log_ok "Seeding completed"
    else
        error_exit "seed_full_suite.ts not found"
    fi
