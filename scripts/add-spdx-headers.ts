...
const SPDX_BLOCK_2024 = `// SPDX-FileCopyrightText: 2024-2026 Jack Valinsky\n${SPDX_LICENSE}\n`;
...
async function* walk(dir: string): AsyncGenerator<string> {
  for await (const entry of Deno.readDir(dir)) {
    ...
    if (entry.isDirectory) {
      ...
      yield* walk(path);
    } else if (entry.isFile && [".h", ".m", ".c"].some((ext) => entry.name.endsWith(ext))) {
      yield path;
    }
  }
}

async function processFile(root: string, path: string): Promise<"added" | "skip-existing"> {
  const content = await Deno.readTextFile(path);
  if (hasSpdx(content)) return "skip-existing";

  const rel = relative(root, path).replaceAll("\\", "/");
  const spdx = FILES_2024.has(rel) ? SPDX_BLOCK_2024 : SPDX_BLOCK;
  const extra = path.includes("MSTWalker") ? MSTWALKER_ATTRIBUTION : "";
  await Deno.writeTextFile(path, `${spdx}${extra}${content}`);
  return "added";
}
...
async function main() {
  ...
  const stats: Record<string, number> = { added: 0, "skip-existing": 0, error: 0 };
