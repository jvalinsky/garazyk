...
describe("Property Test: Git History Preservation", () => {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      try {
        ...
        assert.strictEqual(
          ...
          `Iteration ${
            i + 1
          }: Expected ${commitCount} commits before move, got ${logBefore.length}`,
        );
        ...
        assert.ok(
          ...
          `Iteration ${
            i + 1
          }: Expected at least ${commitCount} commits after move, got ${logAfter.length}`,
        );
  ...
  it("should preserve history across nested directory moves", async () => {
    ...
    try {
      ...
      assert.ok(
        verification.preserved,
        "History should be preserved across nested directory moves",
      );
      assert.ok(
        verification.commitCount >= commitCount,
  ...
  it("should preserve history for files with special characters", async () => {
    ...
    try {
      ...
      assert.ok(
        verification.preserved,
        "History should be preserved for files with special characters",
      );
      assert.ok(
        verification.commitCount >= commitCount,
        `Should have at least ${commitCount} commits`,
