...
type Count = { total: number; documented: number };
type Counts = Record<Bucket, Count>;
type MissingCounts = Record<Bucket, number>;
type Subsystem =
  | "Core"
  | "Database"
  | "Blob"
...
function missingCounts(counts: Counts): MissingCounts {
...
function summarize(counts: Counts): Count & { percent: number } {
...
function subsystemForPath(path: string): Subsystem {
...
function parseSubsystemThreshold(value: string | undefined): [Subsystem, number] {
  if (value === undefined) {
  ...
  if (!subsystem || !thresholdText || !isSubsystem(subsystem)) {
    throw new Error(`--min-subsystem must use a known subsystem, got: ${value}`);
  }
    ...
    switch (arg) {
      ...
      case "--subsystem": {
        ...
        if (subsystem === undefined || !isSubsystem(subsystem)) {
          throw new Error(`--subsystem must be one of: ${subsystems.join(", ")}`);
        }
...
Examples:
  deno run -A scripts/docs/doc-coverage.ts Garazyk/Sources --files
  deno run -A scripts/docs/doc-coverage.ts Garazyk/Sources --subsystem Chat --files
  deno run -A scripts/docs/doc-coverage.ts Garazyk/Sources --min-subsystem Chat=60`);
}
...
function printCounts(title: string, counts: Counts, filesAnalyzed?: number): void {
  console.log(title);
  ...
  for (const bucket of buckets) {
    ...
    console.log(
      `${formatBucket(bucket)} ${String(documented).padStart(5)} / ${String(total).padEnd(5)} (${
...
function printFileReports(files: FileReport[]): void {
  const rows = files
    .filter((file) => missingTotal(file.missing) > 0)
    .sort((left, right) => missingTotal(right.missing) - missingTotal(left.missing));
...
function printTextReport(report: Report): void {
  ...
  if (!report.files || !report.files.every((file) => file.subsystem !== "Other")) {
  ...
  if (report.subsystems) {
    for (const subsystem of subsystems) {
      ...
      if (report.files && report.files.length > 0) {
        const hasMatchingFiles = report.files.some((file) => file.subsystem === subsystem);
...
function createSubsystemReport(): Record<Subsystem, {
  filesAnalyzed: number;
  totals: Counts;
  overall: Count & { percent: number };
}> {
...
async function buildReport(options: Options): Promise<Report> {
  ...
  const subsystemReport = (options.bySubsystem || options.minSubsystems.size > 0)
  ...
  if (subsystemReport) {
    for (const subsystem of subsystems) {
      subsystemReport[subsystem].overall = summarize(subsystemReport[subsystem].totals);
    }
  }
...
async function main() {
  try {
    ...
    if ((report.threshold && !report.threshold.passed) || failedSubsystemThreshold) {
      Deno.exit(1);
    }
  } catch (error) {
