"""Assertion helpers for ATProto scenario scripts.

The helpers raise AssertionError with messages suitable for ScenarioResult
details. They keep scenario files focused on protocol behavior instead of
repeating response-shape checks after every XRPC call.
"""
...
def assert_success(response: dict | Any, operation: str) -> dict:
    """Assert that an XRPC-style response has no top-level error field."""
...
def assert_contains(response: dict, key: str, expected: Any = None, operation: str = "") -> Any:
    """Assert that response contains key and optionally an expected value."""
...
def assert_status(response: dict, expected_status: int, operation: str = "") -> None:
    """Assert a raw HTTP-check response carries the expected status code."""
...
def assert_error(response: dict, expected_error: str, operation: str = "") -> None:
    """Assert a negative test returned the expected protocol error code."""
...
def assert_xrpc_raises(
    ...
    **kwargs,
    ...
    """Call func and assert it raises XrpcError.

    Returns the caught error for inspection so negative-path scenarios can also
    assert status codes or diagnostic messages.
