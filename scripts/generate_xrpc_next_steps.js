...
function lexiconPath(repoRoot, methodId) {
  ...
  return path.join(repoRoot, "ATProtoPDS", "Resources", "lexicons", ...parts, `${methodName}.json`);
}
...
function createIssueMarkdown(payload, topN) {
  ...
  payload.top_candidates.slice(0, topN).forEach((candidate, index) => {
    ...
    lines.push("  - `ATProtoPDS/Sources/Network/XrpcMethodRegistry.m`");
    lines.push("  - `ATProtoPDS/Sources/App/PDSController.m`");
    lines.push("  - `ATProtoPDS/Sources/App/Services/` (new or existing service)");
