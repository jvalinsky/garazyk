...
export function classifyChar(
  ...
  if (cp >= 0x25A0 && cp <= 0x25FF) {
    ...
    if (cp == 0x25C6 || cp == 0x25C7 || cp == 0x25AA || cp == 0x25AB) {
      return "bullet"; // ◆◇▪▫
    }
...
export function classifyBuffer(buffer: ScreenBuffer): CharToken[][] {
  ...
  for (let y = 0; y < buffer.height; y++) {
    ...
    for (let x = 0; x < buffer.width; x++) {
      ...
      const type = classifyChar(cell.char, cell.style);
...
export function findContainers(tokens: CharToken[][]): Rect[] {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if (
        tl === "corner_tl" || tl === "tee_l" || tl === "tee_d" || tl === "cross"
      ) {
        ...
        while (rightX < width) {
          ...
          if (
            t === "corner_tr" || t === "tee_d" || t === "cross" || t === "tee_r"
          ) {
            validRights.push(rightX);
          }
          if (
            t !== "edge_h" && t !== "tee_u" && t !== "tee_d" && t !== "cross" &&
            t !== "tee_r" && t !== "corner_tr"
          ) {
            break;
          }
        ...
        while (bottomY < height) {
          ...
          if (
            t === "corner_bl" || t === "tee_l" || t === "cross" ||
            t === "tee_d" || t === "tee_u"
          ) {
            validBottoms.push(bottomY);
          }
          if (
            t !== "edge_v" && t !== "tee_r" && t !== "tee_l" && t !== "cross" &&
            t !== "tee_d" && t !== "corner_bl" && t !== "tee_u"
          ) {
            break;
          }
          bottomY++;
        ...
        for (const rX of validRights) {
          for (const bY of validBottoms) {
            ...
            if (
              br === "corner_br" || br === "tee_l" || br === "tee_r" ||
              br === "tee_u" || br === "tee_d" || br === "cross"
            ) {
              rects.push({
                x,
                y,
...
function elementTypeForRole(role: ElementMeta["role"]): TuiElement["type"] {
  switch (role) {
    case "panel":
      return "container";
    case "service":
...
export function extractTree(
  ...
  if (metaMap) {
    for (const [ref, meta] of metaMap.entries()) {
      const el: TuiElement = {
        type: elementTypeForRole(meta.role),
        role: meta.role,
        bounds: meta.bounds,
  ...
  for (const c of containers) {
    ...
    for (const el of elementsByRef.values()) {
      if (
        el.bounds.x === c.x && el.bounds.y === c.y &&
        el.bounds.width === c.width && el.bounds.height === c.height
      ) {
        matched = true;
        break;
      }
    ...
    while (x < buffer.width - 2) {
      ...
      if (
        ...
        elementsByRef.set(id, {
          type: "checkbox",
          role: "checkbox",
          bounds: { x, y, width: 3, height: 1 },
          interactable: true,
          focused: false,
          actions: ["click" as ElementAction],
          id,
          states: [t2 === " " ? "unchecked" : "checked"],
          style: { bold: false, underline: false, inverse: false },
          children: [],
        });
        x += 3;
      ...
      if (t1 === "<") {
        ...
        if (
          endX < buffer.width && tokens[y]![endX]!.char === ">" &&
          ...
          elementsByRef.set(id, {
            type: "button",
            role: "button",
            bounds: { x, y, width: endX - x + 1, height: 1 },
            interactable: true,
            focused: false,
            actions: ["click" as ElementAction],
            id,
  ...
  for (const el of sortedRefs) {
    ...
    for (const potential of sortedRefs) {
      ...
      if (rectContains(potential.bounds, el.bounds)) {
        if (
          parent === root ||
          (potential.bounds.width * potential.bounds.height <
