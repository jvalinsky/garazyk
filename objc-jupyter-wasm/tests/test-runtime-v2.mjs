...
assert.ok(
  hostStreams.some((s) => s.text.includes("Autoreleasepool finished")),
  "Autoreleasepool should not crash",
);
hostStreams.length = 0;
...
assert.ok(
  hostStreams.some((s) => s.text.includes("does not respond to selector")),
  "Readonly setter should not exist",
);
hostStreams.length = 0;
...
assert.ok(
  uncaughtResult.evalue.includes("Fatal Error"),
  "Error message should contain exception value",
);
hostStreams.length = 0;
