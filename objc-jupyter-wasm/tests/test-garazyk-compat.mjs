...
import { createObjcKernel, classifySnippet, diagnoseUnsupportedApis, runHostBridgeSelfTest, streamText } from './objc-kernel-test-harness.mjs';
...
async function runCase(test, index) {
  ...
  if ((test.expectedStatus ?? 'ok') === 'host-check') {
    ...
    return {
      ...
      failure: check.passed && missing.length === 0 ? undefined : `Host bridge check failed: ${test.hostCheck}`,
    };
  }
...
if (jsonOutput) {
  ...
  for (const result of results) {
    ...
    console.log(`${icon} ${C.bold}${result.name}${C.reset}  ${C.dim}[${result.category}, ${result.supportClass}]${C.reset}`);
  ...
  console.log('');
  if (failed === 0) {
    console.log(`${C.green}${C.bold}Garazyk compatibility corpus passed${C.reset}  (${passed}/${results.length})`);
  } else {
    console.log(`${C.red}${C.bold}Garazyk compatibility corpus failed${C.reset}  (${failed} failed, ${passed} passed)`);
  }
}
