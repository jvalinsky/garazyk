...
1. **Message Forwarding**:
   ...
   - If `methodSignatureForSelector:` returns a non-nil value (simulated marker), create an
     `NSInvocation` marker and pass it to `forwardInvocation:`.
   - This allows for dynamic proxying and other advanced ObjC patterns.

2. **Key-Value Coding (KVC)**:
   - Implement `valueForKey:` and `setValue:forKey:` as built-in methods in
