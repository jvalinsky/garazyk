...
The Objective-C kernel's parser fails on valid C syntax that appears in notebook cells, preventing
10/85 cells from passing. Root cause: parser routing logic doesn't recognize storage qualifiers and
type modifiers, sending them to expression parser instead of type/variable declaration parser.
