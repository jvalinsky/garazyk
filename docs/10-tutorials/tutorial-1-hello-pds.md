...
3. You're connecting to the right port
4. Request path is exactly `/xrpc/com.atproto.server.describeServer`
...
Congratulations! You've built a working AT Protocol server from scratch. Let's review what you accomplished:

**Core Concepts Learned:**
- PDS initialization and lifecycle management
- HTTP server implementation using BSD sockets
- XRPC request routing and dispatch
- AT Protocol's `describeServer` endpoint
- Objective-C memory management with ARC
- Concurrent request handling with GCD

**What You Built:**
A minimal but functional PDS that listens on port 2583, accepts HTTP connections, routes XRPC requests, and responds with server metadata. This is the foundation that all PDS functionality builds upon.

**Key Takeaways:**
- Every PDS starts with configuration, initialization, and a run loop
- XRPC uses NSIDs to identify methods (like `com.atproto.server.describeServer`)
- Concurrency is essential—one slow client shouldn't block others
- Error handling is critical in server code

This tutorial simplified many aspects (no authentication, no database, minimal error handling), but you now understand the core architecture. The next tutorials will add the missing pieces: account management, data persistence, authentication, and more.
...
- [XRPC Specification](https://atproto.com/specs/xrpc) — Details on the RPC mechanism
- [September PDS Architecture](../01-getting-started/architecture-overview) — How the production server is structured
