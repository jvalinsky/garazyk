...
Production systems fail in interesting ways. Here's how to diagnose and fix common issues.

### Issue: Container Won't Start

**Symptoms**: `docker compose up` fails immediately, or the container starts and immediately exits.
...
# If you see permission errors, the volume might be owned by root
# Recreate the volume:
docker compose down
...
# Verify port binding
docker compose ps
# Should show "127.0.0.1:2583->2583/tcp"
...
# Should output: PDS_TRUST_PROXY_HEADERS=1
...
**1. `PDS_TRUST_PROXY_HEADERS` not set**
...
Verify `/etc/nginx/proxy_params_pds` includes:
```nginx
proxy_set_header X-Real-IP $remote_addr;
proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
```objc
...
Database corruption is serious. Restore from backup immediately:
...
# Find latest backup
ls -lh /var/backups/atprotopds/

# Extract backup
cd /var/backups/atprotopds
tar -xzf pds-backup-YYYYMMDD_HHMMSS.tar.gz

# Restore to volume
sudo rsync -av YYYYMMDD_HHMMSS/ /var/lib/docker/volumes/pds_pds_data/_data/
...
- Disk errors (monitor disk health with SMART)
...
Let's Encrypt needs port 80 for HTTP-01 challenge. Verify:
```bash
sudo ufw status
...
Verify `/etc/nginx/sites-available/pds` includes:
```nginx
location /.well-known/acme-challenge/ {
    root /var/www/html;
}
...
This is advanced—backup everything first!
...
# Add: {"data-root": "/mnt/larger-disk/docker"}
...
1. **Check the logs**: `docker compose logs -f pds` often reveals the issue
2. **Search GitHub issues**: Someone else has probably hit this issue
3. **Ask in Discord**: The AT Protocol community is helpful
4. **File an issue**: If you found a bug, report it with logs and reproduction steps

**When asking for help, include**:
- PDS version (`docker run --rm nspds:local --version`)
- Error messages from logs
- Steps to reproduce the issue
- Your configuration (redact secrets!)
- System information (OS, Docker version, available resources)
