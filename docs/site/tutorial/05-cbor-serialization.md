...
<script setup>
const integerEncoderCode = `#import <Foundation/Foundation.h>

void encodeUnsignedInteger(uint64_t value) {
    ...
    if (value < 24) {
        ...
        // Manual big-endian encoding for portability
        ...
        [data appendBytes:&marker length:1];
        uint8_t bytes[4];
        for (int i = 0; i < 4; i++) bytes[i] = (value >> (24 - i*8)) & 0xFF;
        [data appendBytes:bytes length:4];
    } else {
        uint8_t marker = 0x1B;
        [data appendBytes:&marker length:1];
        uint8_t bytes[8];
        for (int i = 0; i < 8; i++) bytes[i] = (value >> (56 - i*8)) & 0xFF;
        [data appendBytes:bytes length:8];
    }
    
    NSLog(@"Value: %llu -> Encoded: %@", value, data);
}

int main() {
    @autoreleasepool {
        encodeUnsignedInteger(10);
        encodeUnsignedInteger(100);
        encodeUnsignedInteger(300);
        encodeUnsignedInteger(100000);
    }
    return 0;
}`;

const mapSorterCode = `#import <Foundation/Foundation.h>

// Simplified Key-Value pair for sorting demonstration
@interface KV : NSObject
@property NSString *key;
@property NSData *encodedKey;
...
- (instancetype)initWithKey:(NSString *)key {
    self = [super init];
    _key = key;
    // Simple encoding: Major Type 3 (Text) + Length + UTF8
    NSData *utf8 = [key dataUsingEncoding:NSUTF8StringEncoding];
    uint8_t header = 0x60 | (uint8_t)utf8.length; // Assuming short keys < 23 chars
    NSMutableData *d = [NSMutableData dataWithBytes:&header length:1];
    [d appendData:utf8];
    _encodedKey = d;
    return self;
}
...
int main() {
    @autoreleasepool {
        NSArray *keys = @[
            [[KV alloc] initWithKey:@"name"],     // 5 bytes encoded
            [[KV alloc] initWithKey:@"id"],       // 3 bytes encoded
            [[KV alloc] initWithKey:@"created"]   // 9 bytes encoded
        ];
        
        NSLog(@"Original: %@", keys);
        
        NSArray *sorted = [keys sortedArrayUsingComparator:^NSComparisonResult(KV *a, KV *b) {
            NSData *d1 = a.encodedKey;
            NSData *d2 = b.encodedKey;
...
<ObjcRunner :initialCode="integerEncoderCode" />
```
...
<ObjcRunner :initialCode="mapSorterCode" />
```
