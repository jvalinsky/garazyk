...
Ready to try it out? You can write and run standard Objective-C code right here in your browser. We've set up a secure sandbox running GNUstep with ARC support.

<script setup>
const demoCode = `#import <Foundation/Foundation.h>
...
<ObjcRunner :initialCode="demoCode" />
...
### 7. The Delegate Pattern

Delegation is a core pattern in Objective-C where an object "delegates" responsibility/decisions to another object. It relies heavily on Protocols and weak properties.

**Scenario**: A `NetworkFetcher` needs to tell its owner when data arrives.

1. **Define the Protocol**:
   ```objc
   @protocol NetworkFetcherDelegate <NSObject>
   - (void)fetcher:(NetworkFetcher *)fetcher didFinishWithData:(NSData *)data;
   - (void)fetcher:(NetworkFetcher *)fetcher didFailWithError:(NSError *)error;
   @end
   ```
...
3. **Call the Delegate**:
   ```objc
   // In NetworkFetcher.m
   if ([self.delegate respondsToSelector:@selector(fetcher:didFinishWithData:)]) {
       [self.delegate fetcher:self didFinishWithData:downloadedData];
   }
   ```

This pattern decouples the fetcher from the controller that uses it.
