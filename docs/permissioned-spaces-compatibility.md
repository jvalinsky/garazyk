...
| OAuth federation scenario | Scenario 93 obtains PAR + PKCE + DPoP OAuth grants, exchanges signed delegation tokens for credentials, writes from PDS A to a space hosted by PDS B, reads with a member credential from independent PDS C, checks public repo isolation, and rejects a newly minted credential after membership removal. | **Runtime passes 2026-07-18** — scenario 93: 19/19; scenario 94: 25/25 in structured run `2026-07-18t2204z-20523` on the 3-PDS `--binary` topology. The consent redirect is regression-covered by `9000097ba`; pruned-oplog recovery and private-blob acceptance remain pending. |
