...
+ (NSString *)base32Encode:(NSData *)data {
    ...
    for (NSUInteger i = 0; i < length; i++) {
        ...
        buffer &= ((1ULL << bitsLeft) - 1);  // Keep remaining bits
    }
...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        if (argc < 2) {
            fprintf(stderr, "Usage: cid-tool <file>\n");
            return 1;
        }
        ...
        // Raw CID for file content
        CID *rawCID = [CID sha256:data];
        printf("Raw CID: %s\n", rawCID.stringValue.UTF8String);
        
        // DAG-CBOR CID (for structured data)
        NSData *digest = [CID rawSha256:data];
        CID *dagCID = [CID cidWithDigest:digest codec:0x71];
        printf("DAG-CBOR CID: %s\n", dagCID.stringValue.UTF8String);
    }
    return 0;
}
```
