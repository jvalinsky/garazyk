...
[server addMiddleware:^BOOL(HttpRequest *req, HttpResponse *resp) {
    NSLog(@"%@ %@ from %@", req.method, req.path, req.remoteAddress);
    return YES;  // Continue to handler
}];
```
...
Store middleware handlers in an array and execute them before route dispatch.
