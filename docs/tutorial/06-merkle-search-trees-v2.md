...
+ (uint32_t)keyDepth:(NSString *)key {
    ...
    for (int i = 0; i < CC_SHA256_DIGEST_LENGTH; i++) {
        ...
        if (byte == 0) {
            ...
            // But we count in "half-bytes" (nibbles) for finer granularity
            zeroCount += 4;  // 4 half-bytes = 2 nibbles
            continue;
        }

        // First non-zero byte: count leading zero bits
        if ((byte & 0xC0) != 0) break;      // 11xxxxxx → 0 leading zeros
        if ((byte & 0xFC) == 0) zeroCount += 3; // 000000xx → 6 leading zeros
        else if ((byte & 0xF0) == 0) zeroCount += 2; // 0000xxxx → 4 leading zeros
        else zeroCount += 1;                // 00xxxxxx → 2 leading zeros
