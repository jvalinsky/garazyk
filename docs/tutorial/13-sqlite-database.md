...
📝 **Exercise 1: Add Handle Update**

Implement a method to update an account's handle:
...
- (BOOL)updateHandle:(NSString *)newHandle
              forDID:(NSString *)did
               error:(NSError **)error {
    NSString *sql = @"UPDATE accounts SET handle = ? WHERE did = ?";
    sqlite3_stmt *stmt = [self prepareStatement:sql error:error];
    if (!stmt) return NO;
    
    sqlite3_bind_text(stmt, 1, newHandle.UTF8String, -1, SQLITE_TRANSIENT);
    sqlite3_bind_text(stmt, 2, did.UTF8String, -1, SQLITE_TRANSIENT);
...
- Hint: Use `SELECT COUNT(*) FROM blocks WHERE did = ?`
- Consider: What should you return on error?
...
- Hint: Use `LIMIT` and `WHERE rkey > cursor ORDER BY rkey`
- Challenge: Return records as dictionaries with `uri`, `cid`, `rkey`
