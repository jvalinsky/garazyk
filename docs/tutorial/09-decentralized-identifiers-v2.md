...
2. Base58-encode the result (you can use an online Base58 encoder)
...
- Full format: `did:key:z<base58-encoded-data>`
...
**Step 1:** Add multicodec
```
0xe7 0x01 0x02 0xb1 0xf4 ... (35 bytes total)
```

**Step 2:** Base58-encode
```
Q3shokFTS3brHcDQrn82RUDfCZESWL1ZdCEJwekUDPQiYBme
```
...
3. Read the first byte (multicodec type)
4. Look up in multicodec table

<details>
<summary>Hint</summary>

- First DID: multicodec starts with `0xe7` → secp256k1-pub
- Second DID: multicodec starts with `0xed` → ed25519-pub
- Count bytes to verify expected key length

</details>

### 📝 Exercise 3: Design a Key Rotation Strategy

**Scenario:** You're building a PDS and need to help users rotate their signing keys (e.g., if a key is compromised).

**Design:**
1. What fields need to change in an update operation?
2. How do you ensure the old key can't be used after rotation?
3. What happens to signatures made with the old key?

**Consider:**
- The recovery key should remain unchanged (why?)
- Old signatures should still be verifiable (why?)
- The operation must be signed (by which key?)
