---
title: Workstream 01 — Completed Items (Archive)
status: archived
last_verified: 2026-08-05
---

# Workstream 01 — Completed Items (Archive)

Verbatim detail for the nineteen closed items of
[Security and Protocol Correctness](../../plans/workstreams/01-security-and-protocol-correctness.md).
Moved here on 2026-08-05 so the active workstream carries only open scope; the
text below is unchanged from the original, which Git also retains.

**This file is history, not backlog.** Nothing here is actionable. The active
workstream holds the summary table, the two remaining open items, and the
pointers back into this file. Durable design decisions from these items live in
`docs/adr/`; commit hashes cited below are the primary evidence.

Item S6 (published-spec conformance matrix) is deliberately *not* archived: its
gap G3 is still open, so it remains in the active workstream in full.
...
[Workstream 10](../../plans/workstreams/10-dasl-conformance.md) Phase 3 migrated `CAR.m`'s header
