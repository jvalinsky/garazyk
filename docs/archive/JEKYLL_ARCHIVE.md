...
**Backup created**: `docs/archive/jekyll-backup/` (local backup, not committed)

To create a backup before removing Jekyll files:

```bash
# Create backup directory
mkdir -p docs/archive/jekyll-backup

# Copy Jekyll-specific files
cp docs/_config.yml docs/archive/jekyll-backup/ 2>/dev/null || true
cp docs/Gemfile docs/archive/jekyll-backup/ 2>/dev/null || true
cp docs/Gemfile.lock docs/archive/jekyll-backup/ 2>/dev/null || true

# Archive with timestamp
tar -czf docs/archive/jekyll-backup-$(date +%Y%m%d).tar.gz docs/archive/jekyll-backup/

echo "Jekyll documentation archived to docs/archive/jekyll-backup-$(date +%Y%m%d).tar.gz"
```
...
```bash
# Extract backup
tar -xzf docs/archive/jekyll-backup-YYYYMMDD.tar.gz

# Restore files
cp docs/archive/jekyll-backup/_config.yml docs/
cp docs/archive/jekyll-backup/Gemfile docs/ 2>/dev/null || true
cp docs/archive/jekyll-backup/Gemfile.lock docs/ 2>/dev/null || true
```
