...
The oplog optimizes transport; it does not represent a committed data structure. The proposal states:
...
> the repo's structure (paths to CIDs) with `com.atproto.space.listRecords`
...
The server and lexicons require no changes. The `prev` field on
oplog entries already provides the chain linkage. A future enhancement may add a `SinceNotFound` error to `listRepoOps`, though correctness does not require it.
...
1. **Local repo is empty** (first sync, or space deletion recovery):
   Fetch the full CAR via `getRepo`.

2. **Local repo exists, gap detected**:
   a. Fetch the lightweight listing via `listRecords(excludeValues=true)`
      with pagination. This transfers only collection, rkey, and cid data.
   b. Build remote `{path to CID}` map.
   c. Get local `{path to CID}` map from `PDSSpaceStore`.
   d. Compute diff: records to add (in remote, not local), records to
      update (in both, CID differs), records to delete (in local, not
      remote).
   e. If the total changed records fall below the threshold, fetch each record via `getRecord` and apply locally.
   f. If the total changed records exceed the threshold, fetch the full CAR via `getRepo`.
...
The lightweight path costs `ceil(R/100) + C` round-trips (listing pages plus getRecord calls). The full CAR costs 1 round-trip but transfers all record data. The lightweight path saves bandwidth when `C` is small relative to `R`. The full CAR performs better when `C` is large.
...
This process replaces the full state rather than merging records. The CAR is authoritative
from the repo host.
...
The proposal allows a repo host to compact or drop the oplog. The implementation:
...
This retains the last N distinct revisions. The `NOT IN` subquery selects the
revisions to keep, and the outer query deletes everything else.

**Design choices:**
- Count-based pruning avoids parsing timestamps from monotonic TIDs.
- It provides a predictable bound on oplog size.
- A count-based window satisfies the proposal's backfill window requirement.
- The system can add time-based pruning later as a secondary policy.

**Execution:**
- A background timer decouples pruning from the write path to avoid latency.
- The configurable timer defaults to disabled. When enabled, it runs on a serial queue.
...
field in the output schema. The handlers must return the cursor.
...
- `PDSSpaceStore` gains:
  ...
  - `recordIndexForSpace:author:error:` — local `{path to CID}` map for diffing.
