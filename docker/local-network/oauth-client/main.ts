...
const loggingFetch: typeof globalThis.fetch = async (input, init) => {
  ...
  try {
    ...
    console.log(
      `[OAUTH-FETCH] <-- ${response.status} ${response.statusText} ${url} (${elapsed}ms)`,
    );
    console.log(
      `[OAUTH-FETCH]     response headers:`,
      JSON.stringify(Object.fromEntries(response.headers.entries())),
    );

    // Clone and log the body for metadata endpoints (not for large responses)
    if (
      url && typeof url === "string" && url.includes("/.well-known/") && response.status === 200
    ) {
