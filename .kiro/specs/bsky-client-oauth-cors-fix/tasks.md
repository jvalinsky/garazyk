...
- [ ] 4. Integration testing on live PDS
  ...
  **Remote VM Access**: The production PDS runs on the VM at `DEPLOY_HOST`. Execute commands on the remote VM using SSH:
  ```bash
  ssh DEPLOY_HOST "<command>"
  ```
  
  **Common Remote Commands**:
  - View PDS logs: `ssh DEPLOY_HOST "docker compose logs -f pds"`
  - Query database: `ssh DEPLOY_HOST "docker exec nspds sqlite3 /data/pds.db 'SELECT client_id FROM oauth_clients;'"`
  - Restart PDS: `ssh DEPLOY_HOST "cd DEPLOY_DIR/objpds/docker/pds && docker compose restart pds"`
  - Check container status: `ssh DEPLOY_HOST "docker ps"`
  ...
  - [ ] 4.1 Test bsky.app OAuth flow on pds.garazyk.xyz
    ...
    - **Troubleshooting**:
      - Check PDS logs: `ssh DEPLOY_HOST "docker compose logs -f pds"`
  ...
  - [ ] 4.4 Verify existing registered clients still work on pds.garazyk.xyz
    ...
    - **Prerequisites**: 
      ...
      - If none exist, register a test client: `ssh DEPLOY_HOST "docker exec nspds kaszlak oauth register-client --client-id test-client --redirect-uri https://example.com/callback"`
    - **Test Steps**:
      1. Query database to list registered clients: `ssh DEPLOY_HOST "docker exec nspds sqlite3 /data/pds.db 'SELECT client_id FROM oauth_clients;'"`
