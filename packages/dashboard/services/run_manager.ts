...
import type { Run, RunConfig } from "./types.ts";
import { fetchRun } from "../db/queries.ts";
import { importRunReports } from "./report_scanner.ts";
import { getDashboardPaths } from "../paths.ts";

function reportsRootDir(): string {
  return getDashboardPaths().reportsDir;
}

function activeRunLockPath(): string {
  return getDashboardPaths().activeRunLockPath;
}
...
export interface RunManager {
  ...
  restartRun(runId: string): Promise<{ newRunId: string } | { error: string }>;
}
...
class RunManagerImpl implements RunManager {
  ...
  async #startRunInner(
    ...
    const runDir = join(reportsRootDir(), "runs", runId);
    const runReportsDir = join(runDir, "reports");
    const logPath = join(runDir, `run.log`);

    await Deno.mkdir(runReportsDir, { recursive: true });
  ...
  async recover(): Promise<void> {
    try {
      const lockData = await Deno.readTextFile(activeRunLockPath());
  ...
  private async writeLockFile(run: Run) {
    ...
    const lockPath = activeRunLockPath();
    const tempPath = lockPath + ".tmp";
    await Deno.writeTextFile(tempPath, JSON.stringify(run, null, 2));
    await Deno.rename(tempPath, lockPath);
  }

  private async readLockFile(): Promise<Run | undefined> {
    try {
      return JSON.parse(await Deno.readTextFile(activeRunLockPath())) as Run;
    } catch {
      return undefined;
    }
  }
  ...
  private async clearLockFile() {
    try {
      await Deno.remove(activeRunLockPath());
    } catch {
      // Ignore
    }
  }
  ...
  private async spawnRunner(run: Run) {
    const args = [
      "run",
      "-A",
      getDashboardPaths().runScenariosScript,
      "--run-id",
      run.id,
      "--topology",
      run.topology!,
  ...
  async restartRun(
    ...
    const config: RunConfig = {
      topology: run.topology || "garazyk-default",
...
export const runManager: RunManager = new RunManagerImpl();
