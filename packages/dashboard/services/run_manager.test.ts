...
Deno.test({
  ...
  async fn() {
    ...
    // Check DB entry
    const row = db.prepare("SELECT * FROM runs WHERE id = ?").get(
      runId,
    ) as RunStatusRow | undefined;
    assertExists(row);
    ...
    const rowAfterStop = db.prepare("SELECT * FROM runs WHERE id = ?").get(
      runId,
    ) as RunStatusRow | undefined;
    assertExists(rowAfterStop);
