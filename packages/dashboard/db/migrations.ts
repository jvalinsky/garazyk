...
export function runMigrations(db: Database): void {
  ...
  if (currentVersion < 3) {
    console.log(
      "[db] Applying migration v3: Ensure scenario_params_json exists...",
    );
    try {
      addColumns(db, "runs", [
        "scenario_params_json TEXT",
...
function getCurrentVersion(db: Database): number {
  try {
    const row = db.prepare("SELECT MAX(version) as v FROM schema_migrations")
      .get() as {
        v: number | null;
      };
    return row?.v || 0;
...
function addColumns(db: Database, table: "runs", columns: string[]): void {
