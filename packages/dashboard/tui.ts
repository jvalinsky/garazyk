...
import { readKeys, isKey, isCtrl, isQuit, Keys } from "./tui/input.ts";
import type { Key } from "./tui/input.ts";
import { computeLayout, PANEL_IDS, type PanelId } from "./tui/layout.ts";
import { panelContentArea } from "./tui/layout.ts";
import { FocusRing } from "./tui/focus.ts";
import {
  createPanelStates,
  moveCursorUp,
  moveCursorDown,
...
/** Options for the terminal dashboard. */
export interface DashboardTuiOptions extends DashboardPathOptions {
  /** Refresh interval in milliseconds. Defaults to 2000. */
  intervalMs?: number;
...
export async function collectTuiSnapshot(
  ...
  configureDashboardPaths(options);
  const paths = await import("./paths.ts").then((m) => m.getDashboardPaths());
