...
function usage(): never {
  console.error(
    "Usage: deno run -A packages/narzedzia/tsdoc_coverage.ts [--json] " +
...
function hasTypeParamDoc(jsDoc: JsDoc | undefined, name: string): boolean {
  return (jsDoc?.tags ?? []).some((tag) =>
    (tag.kind === "template" || tag.kind === "typeParam" ||
      tag.kind === "typeparam") &&
    tag.name === name &&
...
function isSourceFile(path: string): boolean {
  if (
    path.endsWith(".d.ts") || path.endsWith(".test.ts") ||
...
export async function loadDocJson(
  ...
  const sourceArg = relative(cwd, path);
  ...
  if (stdout.length > 0) {
    try {
      ...
      return {
        ...
        warning: `${
          displayPath(path)
        }: failed to parse deno doc JSON: ${error}`,
      };
    }
  }
...
export function addTopLevelNode(
  ...
  if (kind === "class") {
    addTypeParams(
      ...
      name,
      jsDoc,
      typeParamsFrom(asObject(node.classDef)),
      location,
    );
    ...
    addTypeParams(
      ...
      name,
      jsDoc,
      typeParamsFrom(asObject(node.interfaceDef)),
      location,
    );
    ...
    addTypeParams(
      ...
      name,
      jsDoc,
      typeParamsFrom(asObject(node.functionDef)),
      location,
    );
    ...
    addTypeParams(
      ...
      name,
      jsDoc,
      typeParamsFrom(asObject(node.typeAliasDef)),
      location,
    );
  }
}
...
export function addClassMembers(
  ...
  for (const method of asArray(classDef?.methods)) {
    ...
    addTypeParams(
      ...
      symbol,
      jsDoc,
      typeParamsFrom(asObject(method.functionDef)),
      location,
    );
  }
}
...
export function buildReport(
  ...
  const byKind = Object.fromEntries(
    symbolKinds.map((
      kind,
    ) => [kind, bucketFor(items.filter((item) => item.kind === kind))]),
  ...
  const byFile = Object.fromEntries(
    files.map((
      file,
    ) => [file, bucketFor(items.filter((item) => item.file === file))]),
  );
...
export function printReport(report: CoverageReport): void {
  ...
  for (const kind of symbolKinds) {
    ...
    console.log(
      `  ${kind}: ${bucket.percent}% (${bucket.documented}/${bucket.total})`,
    );
  }
...
export async function main(): Promise<void> {
  ...
  const sourceFiles = [
    ...new Set(
      (await Promise.all(paths.map((path) => collectSourceFiles(path))))
        .flat(),
    ),
  ].sort();
