...
const DEFAULT_HEADER = "// SPDX-FileCopyrightText: 2025-2026 Jack Valinsky\n" +
...
Deno.test("hasSpdx returns true when SPDX line is present", () => {
  assert(
    hasSpdx("// SPDX-License-Identifier: Unlicense OR CC0-1.0\nconst x = 1;"),
  );
});
...
Deno.test("addSpdxHeader preserves multiline content after the header", () => {
  ...
  assertEquals(
    addSpdxHeader(content, "packages/narzedzia/example.ts"),
...
Deno.test("addSpdxHeader handles empty content", () => {
  assertEquals(
    addSpdxHeader("", "Garazyk/Sources/Other/File.m"),
    DEFAULT_HEADER,
  );
});

Deno.test("addSpdxHeader prepends a header even when content already has one", () => {
  const existing = "// SPDX-FileCopyrightText: 2025-2026 Jack Valinsky\n" +
    "// SPDX-License-Identifier: Unlicense OR CC0-1.0\n" +
    "const value = 1;\n";
...
Deno.test("addSpdxHeader preserves the original SPDX header text verbatim", () => {
  const existing = "// SPDX-FileCopyrightText: 2024-2026 Jack Valinsky\n" +
    "// SPDX-License-Identifier: Unlicense OR CC0-1.0\n";
