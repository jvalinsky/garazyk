...
function buildTree(
  ...
  function serviceNameFor(role: string): string {
    const known =
      DEFAULT_SERVICE_NAMES[role as keyof typeof DEFAULT_SERVICE_NAMES];
...
function renderChildren(
  ...
  for (let i = 0; i < children.length; i++) {
    ...
    lines.push(
      `${prefix}${connector}${edgeLabel}${formatNodeLine(node, verbose)}`,
    );
...
function mermaidNodeDef(node: TopologyNode): string {
  ...
  const label =
    `${node.role.toUpperCase()}<br/>${node.binary} :${node.port}<br/>${
      mermaidStatusEmoji(node)
    } ${mermaidStatusLabel(node)}`;
...
function collectEdges(
  ...
  for (const child of children) {
    ...
    edges.push(
      ...collectEdges(child.node.children, mermaidNodeId(child.node.role)),
    );
  }
  return edges;
}
...
function latexChildLines(
  ...
  lines.push(
    `${indent}  edge from parent node[midway, font=\\tiny] {${edgeLabel}}`,
  );
  lines.push(`${indent}}`);
  return lines;
}
...
export function renderLatex(tree: TopologyTree): string[] {
  ...
  lines.push(
    "%% LaTeX document generated by `hamownia service topology --format latex`",
  );
  ...
  lines.push("");
  lines.push(
    "\\tikzstyle{node-healthy}=[rectangle, rounded corners, fill=green!10, draw=green!50!black, text centered, minimum width=2.2cm]",
  );
  lines.push(
    "\\tikzstyle{node-unhealthy}=[rectangle, rounded corners, fill=yellow!10, draw=orange!50!black, text centered, minimum width=2.2cm]",
  );
  lines.push(
    "\\tikzstyle{node-stopped}=[rectangle, rounded corners, fill=red!10, draw=red!50!black, text centered, minimum width=2.2cm]",
  );
  lines.push(
    "\\tikzstyle{node-unknown}=[rectangle, rounded corners, fill=gray!10, draw=gray!50!black, text centered, minimum width=2.2cm]",
  );
  lines.push("");
  ...
  if (roots.length === 1) {
    ...
    for (let i = 0; i < roots.length; i++) {
      ...
      lines.push(
        `  \\node[${latexStyle(root)}${pos}] (${root.role}) {${
          latexNodeLabel(root)
        }};`,
      );
    }
