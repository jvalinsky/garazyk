...
import { initLogger, loadTopologyManifest, TopologyRegistry } from "@garazyk/schemat";
import {
  buildBinaryTopology,
...
export const topologyCommand = new Command()
  ...
  .option("-f, --format <fmt:format>", "Output format", { default: "text" as const })
  .action(async ({ verbose, quiet, preset, manifest, format = "text" }: TopologyOptions) => {
