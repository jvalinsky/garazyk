...
const clientFlowType = new EnumType(["none", "smoke", "login", "deep"] as const);
...
export const runCommand = new Command()
  ...
  .option("--collect-diagnostics",
    "Capture diagnostics for the current run and exit.")
  .option("--allow-hybrid-network",
  ...
  .action(async (
    ...
    const {
      verbose, list, setupOnly, setup, noSetup,
      teardown, teardownOnly, stop, binary, pds2,
      collectDiagnostics, allowHybridNetwork, keepRunning, noJson, otel,
      runId, diagnosticsDir, reportsDir, webClient,
      clientFlow, topology, runner, timeout,
    } = options;
