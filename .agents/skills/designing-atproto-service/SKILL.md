...
```text
packages/schemat/        Topology schemas, presets, manifests, and compiler
packages/gruszka/          Typed XRPC clients generated from lexicons
packages/laweta/           Docker and Docker Compose primitives
packages/hamownia/         Scenario execution, reports, timing, assertions
...
2. Add or update topology schemas and presets in `packages/schemat` if the service needs new configuration fields.
3. Add Docker Compose rendering or Docker client support through `packages/laweta` and topology compiler code.
4. Add typed client methods in `packages/gruszka` only through lexicon generation when lexicons change.
...
- Keep service configuration validated by Zod schemas in `packages/schemat`.
...
The XRPC client in `packages/gruszka/lexicons.ts` is generated from `lexicons/`.
...
- waits for service health through hamownia helpers,
