...
This skill provides comprehensive guidelines for writing production-ready bash scripts that are maintainable, secure, and efficient. Based on best practices from Google's Bash Style Guide, Greg's Wiki BashGuide, and other authoritative sources, it covers everything from script initialization to testing and deployment.

## When to Use

```dot
digraph bash_scripting_flow {
    "Need to write shell script?" [shape=diamond];
    "Complex automation task?" [shape=diamond];
    "Production environment?" [shape=diamond];
    "Apply professional bash skill" [shape=box];
    "Simple one-liner" [shape=box];

    "Need to write shell script?" -> "Complex automation task?" [label="yes"];
    "Complex automation task?" -> "Production environment?" [label="yes"];
    "Production environment?" -> "Apply professional bash skill" [label="yes"];
    "Production environment?" -> "Simple one-liner" [label="no"];
    "Complex automation task?" -> "Simple one-liner" [label="no"];
}
```

**Use this skill when:**
- Writing scripts for production systems or CI/CD pipelines
- Creating automation for complex multi-step processes
- Scripts will be maintained by a team or run on multiple systems
- Security, reliability, and maintainability are important
- Scripts handle user input, files, or network operations

**Do not use for:**
- Simple one-liners or aliases
- Personal automation scripts with single use
- When Python/Ruby would be more appropriate for complexity

## Prerequisites

- Basic bash knowledge (variables, loops, conditionals)
- Understanding of shell concepts (pipes, redirection, exit codes)
- Familiarity with common Unix tools (grep, sed, awk, find)
- Access to ShellCheck linter
- Understanding of POSIX shell standards
...
- [ ] Define script metadata (name, version, description)
- [ ] Handle script location/portability issues

### 2. Error Handling Setup
- [ ] Implement trap handlers for cleanup
- [ ] Use proper exit codes (0 for success, 1-255 for errors)
- [ ] Validate all external commands exist
...
- [ ] Use safe path handling (`realpath`, absolute paths)
...
- [ ] Validate on target systems
- [ ] Include error case testing
- [ ] Test color output (when applicable)
