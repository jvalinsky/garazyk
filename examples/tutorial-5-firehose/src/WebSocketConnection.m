...
#else
// No CommonCrypto on GNUstep — SHA-1 only used for WebSocket handshake
// which is handled in the server code, not here
#endif
...
- (void)sendMessage:(NSData *)message {
    ...
    dispatch_async(self.sendQueue, ^{
        ...
        // Thread-safe: pendingSendCount/Bytes only modified on sendQueue
        ...
        // Thread-safe: decrement on same queue
        self.pendingSendCount--;
        self.pendingSendBytes -= message.length;
...
- (void)receiveLoop {
    while (self.isOpen) {
        ...
        if (n <= 0) {
            if (n < 0) {
                NSLog(@"WebSocket recv error: %s", strerror(errno));
            }
            [self close];
            break;
        }
        ...
        if (payloadLen == 126) {
            uint16_t len16;
            ssize_t r = recv(self.socket, &len16, 2, 0);
            if (r <= 0) { [self close]; break; }
            payloadLen = ntohs(len16);
        } else if (payloadLen == 127) {
            uint64_t len64;
            ssize_t r = recv(self.socket, &len64, 8, 0);
            if (r <= 0) { [self close]; break; }
        ...
        if (masked) {
            ssize_t r = recv(self.socket, maskingKey, 4, 0);
            if (r <= 0) { [self close]; break; }
        }
