...
- (NSString *)signPayload:(NSDictionary *)payload error:(NSError **)error {
    ...
    NSString *signature = [self stubSignData:signingInput error:error];
...
- (NSString *)stubSignData:(NSString *)data error:(NSError **)error {
    NSData *dataBytes = [data dataUsingEncoding:NSUTF8StringEncoding];
    NSMutableData *signature = [NSMutableData dataWithLength:64];
    SecRandomCopyBytes(kSecRandomDefault, dataBytes.length, signature.mutableBytes);
    NSString *sigStr = [signature base64EncodedStringWithOptions:0];
    return [sigStr stringByReplacingOccurrencesOfString:@"+" withString:@"-"];
}
...
- (JWT *)mintAccessTokenForDID:(NSString *)did
    ...
    NSString *signature = [self stubSignData:[NSString stringWithFormat:@"%@.%@", [JWT base64URLEncodeData:[NSJSONSerialization dataWithJSONObject:[header toDictionary] options:0 error:error] error:error] ?: @"", [JWT base64URLEncodeData:[NSJSONSerialization dataWithJSONObject:[payload toDictionary] options:0 error:error] error:error] ?: @""] error:error];
...
- (JWT *)mintRefreshTokenForDID:(NSString *)did
    ...
    return [JWT jwtWithHeader:header payload:payload signature:@"stub-refresh" error:error];
}
