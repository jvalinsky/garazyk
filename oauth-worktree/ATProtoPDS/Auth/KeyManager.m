...
- (nullable NSData *)exportPublicKeyData:(SecKeyRef)key {
    ...
    NSData *keyData = CFBridgingRelease(SecKeyCopyExternalRepresentation(key, (__bridge CFDictionaryRef)options, &error));
    if (error) return nil;
...
- (nullable KeyPair *)generateKeyPairWithAlgorithm:(NSString *)algorithm
    ...
    SecKeyRef publicKey = SecKeyCopyPublicKey(privateKey, &pubError);
