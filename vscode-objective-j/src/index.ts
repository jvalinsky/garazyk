...
import { parseDocument, ObjJParseResult, ObjJClass, ObjJProtocol, ObjJForwardDecl, ObjJMethod } from "./parser";
 ...
 */
export class ObjJWorkspaceIndex {
   ...
   */
  allClasses(): { cls: ObjJClass; uri: vscode.Uri }[] {
    const result: { cls: ObjJClass; uri: vscode.Uri }[] = [];
   ...
   */
  allProtocols(): { proto: ObjJProtocol; uri: vscode.Uri }[] {
    const result: { proto: ObjJProtocol; uri: vscode.Uri }[] = [];
    for (const file of this.files.values()) {
   ...
   */
  allForwardDecls(): { decl: ObjJForwardDecl; uri: vscode.Uri }[] {
    const result: { decl: ObjJForwardDecl; uri: vscode.Uri }[] = [];
    for (const file of this.files.values()) {
   ...
   */
  findClass(name: string): { cls: ObjJClass; uri: vscode.Uri } | undefined {
    for (const file of this.files.values()) {
      for (const cls of file.result.classes) {
   ...
   */
  findCategories(className: string): { cls: ObjJClass; uri: vscode.Uri }[] {
    const result: { cls: ObjJClass; uri: vscode.Uri }[] = [];
    for (const file of this.files.values()) {
      for (const cls of file.result.classes) {
   ...
   */
  findProtocol(name: string): { proto: ObjJProtocol; uri: vscode.Uri } | undefined {
    for (const file of this.files.values()) {
      for (const proto of file.result.protocols) {
        if (proto.name === name) {
   ...
   */
  findMethodsBySelector(selector: string): { method: ObjJMethod; containerName: string; uri: vscode.Uri }[] {
    const result: { method: ObjJMethod; containerName: string; uri: vscode.Uri }[] = [];
    for (const file of this.files.values()) {
      for (const cls of file.result.classes) {
   ...
   */
  getSuperclassChain(className: string, maxDepth: number = 20): string[] {
    const chain: string[] = [];
    ...
    for (let i = 0; i < maxDepth; i++) {
      ...
      if (!cls || !cls.cls.superclass) break;

      const superName = cls.cls.superclass;
      if (visited.has(superName)) break; // cycle guard
   ...
   */
  getAllMethodsForClass(className: string): { method: import("./parser").ObjJMethod; source: string }[] {
    const results: { method: import("./parser").ObjJMethod; source: string }[] = [];
        ...
        if (displayName.toLowerCase().includes(lowerQuery)) {
          results.push(
            new vscode.SymbolInformation(
              ...
              cls.superclass || "",
              new vscode.Location(file.uri, cls.nameRange)
            )
          );
        }
        for (const method of cls.methods) {
          if (method.selector.toLowerCase().includes(lowerQuery)) {
            results.push(
              new vscode.SymbolInformation(
                `${method.isClassMethod ? "+" : "-"} ${method.selector}`,
                vscode.SymbolKind.Method,
                displayName,
                new vscode.Location(file.uri, method.selectorRange)
              )
            );
          }
        }
        ...
        if (proto.name.toLowerCase().includes(lowerQuery)) {
          results.push(
            new vscode.SymbolInformation(
              ...
              "",
              new vscode.Location(file.uri, proto.nameRange)
            )
          );
        }
        for (const method of proto.methods) {
        ...
        if (decl.name.toLowerCase().includes(lowerQuery)) {
          ...
          results.push(
            new vscode.SymbolInformation(
              ...
              kind,
              `@${decl.kind}`,
              new vscode.Location(file.uri, decl.nameRange)
            )
          );
        }
      }
