...
const cib_1 = require("./cib");
const OBJJ_SELECTOR = { language: "objective-j", scheme: "file" };
let workspaceIndex;
let diagnosticProvider;
let cibProvider;
function activate(context) {
    workspaceIndex = new index_1.ObjJWorkspaceIndex();
    diagnosticProvider = new diagnostics_1.ObjJDiagnosticProvider(workspaceIndex);
    cibProvider = new cib_1.ObjJCibProvider(workspaceIndex);
    ...
    vscode.languages.registerDocumentSymbolProvider(OBJJ_SELECTOR, new ObjJDocumentSymbolProvider()), vscode.languages.registerWorkspaceSymbolProvider(new ObjJWorkspaceSymbolProvider()), vscode.languages.registerDefinitionProvider(OBJJ_SELECTOR, new ObjJDefinitionProvider()), vscode.languages.registerReferenceProvider(OBJJ_SELECTOR, new ObjJReferenceProvider()), vscode.languages.registerHoverProvider(OBJJ_SELECTOR, new ObjJHoverProvider()), vscode.languages.registerCompletionItemProvider(OBJJ_SELECTOR, new ObjJCompletionProvider(), "[", " ", ":", '"', "<", "/"), 
    // Near-term: diagnostics, code actions, rename
    diagnosticProvider, vscode.languages.registerCodeActionsProvider(OBJJ_SELECTOR, new codeActions_1.ObjJCodeActionProvider(workspaceIndex), {
        providedCodeActionKinds: codeActions_1.ObjJCodeActionProvider.providedCodeActionKinds,
    }), vscode.languages.registerRenameProvider(OBJJ_SELECTOR, new rename_1.ObjJRenameProvider(workspaceIndex)), 
    // Medium-term: semantic tokens, signature help
    vscode.languages.registerDocumentSemanticTokensProvider(OBJJ_SELECTOR, new semanticTokens_1.ObjJSemanticTokenProvider(workspaceIndex), semanticTokens_1.LEGEND), vscode.languages.registerSignatureHelpProvider(OBJJ_SELECTOR, new signatureHelp_1.ObjJSignatureHelpProvider(workspaceIndex), ":", " "), 
    // Longer-term: folding, formatting, cib awareness
    vscode.languages.registerFoldingRangeProvider(OBJJ_SELECTOR, new folding_1.ObjJFoldingRangeProvider()), vscode.languages.registerDocumentFormattingEditProvider(OBJJ_SELECTOR, new formatter_1.ObjJDocumentFormattingEditProvider()), vscode.languages.registerDocumentRangeFormattingEditProvider(OBJJ_SELECTOR, new formatter_1.ObjJDocumentRangeFormattingEditProvider()), cibProvider);
}
function deactivate() { }
// ---------------------------------------------------------------------------
// Document Symbol Provider
// ---------------------------------------------------------------------------
class ObjJDocumentSymbolProvider {
    provideDocumentSymbols(document) {
        ...
        for (const decl of result.forwardDecls) {
            ...
            symbols.push(new vscode.DocumentSymbol(`@${decl.kind} ${decl.name}`, "", kind, decl.range, decl.nameRange));
        }
        return symbols;
    }
}
function classToSymbol(cls) {
    ...
    for (const ivar of cls.ivars) {
        sym.children.push(new vscode.DocumentSymbol(ivar.name, ivar.type, vscode.SymbolKind.Field, ivar.range, ivar.nameRange));
    }
    for (const method of cls.methods) {
        const prefix = method.isClassMethod ? "+" : "-";
        sym.children.push(new vscode.DocumentSymbol(`${prefix} ${method.selector}`, method.returnType, vscode.SymbolKind.Method, method.range, method.selectorRange));
    }
    return sym;
}
function protocolToSymbol(proto) {
...
// ---------------------------------------------------------------------------
class ObjJWorkspaceSymbolProvider {
...
class ObjJDefinitionProvider {
    provideDefinition(document, position) {
        ...
        if (/^[A-Z]/.test(word)) {
            ...
            if (found)
                return new vscode.Location(found.uri, found.cls.nameRange);
            const proto = workspaceIndex.findProtocol(word);
            if (proto)
                return new vscode.Location(proto.uri, proto.proto.nameRange);
        }
        ...
        if (selectorMatch) {
            ...
            if (methods.length > 0)
                return methods.map((m) => new vscode.Location(m.uri, m.method.selectorRange));
        }
    ...
    provideHover(document, position) {
        ...
        if (cibProvider) {
            ...
            for (const cls of result.classes) {
                if (position.line >= cls.range.start.line && position.line <= cls.range.end.line) {
                    const ivar = cls.ivars.find((iv) => iv.isOutlet && iv.nameRange.contains(position));
                    if (ivar) {
        ...
        if (/^[A-Z]/.test(word)) {
            ...
            if (found) {
                ...
                if (cls.superclass)
                    header += ` : ${cls.superclass}`;
                if (cls.protocols.length)
                    header += ` <${cls.protocols.join(", ")}>`;
                parts.push(header);
                ...
                if (allMethods.length > 0) {
                    ...
                    if (own.length > 10)
                        parts.push(`- ... and ${own.length - 10} more`);
                }
            ...
            if (proto) {
                ...
                if (p.parentProtocols.length)
                    header += ` <${p.parentProtocols.join(", ")}>`;
                const parts = [header];
...
class ObjJCompletionProvider {
    ...
    completeLocalImport(document, partial) {
        const items = [];
    ...
    completeFrameworkImport(partial) {
        const items = [];
    ...
    isInsideMessageSend(textBefore) {
        let depth = 0;
    ...
    methodToCompletion(m, containerName) {
        const prefix = m.isClassMethod ? "+" : "-";
...
function extractSelectorAtPosition(line, charPos) {
...
function extractSelectorFromMessageSend(line, charPos) {
    const parts = [];
