...
exports.ObjJSemanticTokenProvider = exports.LEGEND = exports.TOKEN_MODIFIERS = exports.TOKEN_TYPES = void 0;
...
class ObjJSemanticTokenProvider {
    constructor(index) {
        this.index = index;
    }
    provideDocumentSemanticTokens(document) {
        const builder = new vscode.SemanticTokensBuilder(exports.LEGEND);
        const result = (0, parser_1.parseDocument)(document);
        ...
        for (const cls of result.classes) {
            ...
            if (cls.superclass) {
                ...
                if (superIdx >= 0) {
                    builder.push(new vscode.Range(cls.range.start.line, superIdx, cls.range.start.line, superIdx + cls.superclass.length), "class", []);
                }
            }
            // Instance variables
            for (const ivar of cls.ivars) {
                ...
                const typeMatch = ivarLine.match(/(?:@outlet\s+)?([A-Z]\w*|id|BOOL|SEL|int|unsigned|float|double|char|void|long|short|signed)\s+/);
                if (typeMatch) {
                    const typeIdx = ivarLine.indexOf(typeMatch[1]);
                    if (typeIdx >= 0 && /^[A-Z]/.test(typeMatch[1])) {
                        builder.push(new vscode.Range(ivar.range.start.line, typeIdx, ivar.range.start.line, typeIdx + typeMatch[1].length), "type", this.isFrameworkType(typeMatch[1]) ? ["defaultLibrary"] : []);
                    }
                }
            }
            // Methods
                ...
                for (const param of method.params) {
                    ...
                    for (let li = method.range.start.line; li <= method.range.end.line && li < lines.length; li++) {
                        ...
                        if (paramIdx >= 0) {
                            ...
                            if (before.endsWith(")")) {
                                builder.push(new vscode.Range(li, paramIdx, li, paramIdx + param.name.length), "parameter", ["declaration"]);
                                break;
                            }
                        }
                    }
    ...
    isFrameworkType(name) {
        return /^(CP|CG)[A-Z]/.test(name);
    }
}
exports.ObjJSemanticTokenProvider = ObjJSemanticTokenProvider;
