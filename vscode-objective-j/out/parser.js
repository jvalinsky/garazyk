...
const RE_IMPLEMENTATION = /^@implementation\s+([a-zA-Z_]\w*)(?:\s*:\s*([a-zA-Z_]\w*))?(?:\s*<([^>]+)>)?(?:\s*\(([^)]+)\))?/;
 ...
 */
function parseMethodSignature(signatureStr) {
    const params = [];
...
function parseDocument(document) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (RE_END.test(trimmed)) {
            if (currentClass) {
                currentClass.range = new vscode.Range(currentClass.range.start.line, 0, i, line.length);
            ...
            if (currentProtocol) {
                currentProtocol.range = new vscode.Range(currentProtocol.range.start.line, 0, i, line.length);
        ...
        if (inIvarBlock && currentClass) {
            ...
            const ivarRegex = /(@outlet\s+)?([A-Z]\w*|id|BOOL|SEL|int|unsigned|float|double|char|void|long|short|signed)\s+([_a-zA-Z]\w*)/;
        ...
        if (container) {
            ...
            if (methodAccum) {
                ...
                if (trimmed.includes("{") || trimmed.endsWith(";")) {
                    ...
                    const selectorStart = lines[methodAccum.startLine].indexOf(selector.split(":")[0], lines[methodAccum.startLine].indexOf(")") + 1);
                    const method = {
                        selector,
                        ...
                        selectorRange: new vscode.Range(methodAccum.startLine, Math.max(0, selectorStart), methodAccum.startLine, Math.max(0, selectorStart) + selector.split(":")[0].length),
                    };
            ...
            if (methodMatch) {
                ...
                if (rest.includes("{") || rest.endsWith(";")) {
                    ...
                    const selectorStart = line.indexOf(selector.split(":")[0], line.indexOf(")") + 1);
                    const method = {
                        selector,
                        ...
                        params,
                        range: new vscode.Range(i, 0, i, line.length),
                        selectorRange: new vscode.Range(i, Math.max(0, selectorStart), i, Math.max(0, selectorStart) + selector.split(":")[0].length),
                    };
                    container.methods.push(method);
                }
                else {
    ...
    if (currentClass) {
        currentClass.range = new vscode.Range(currentClass.range.start.line, 0, lines.length - 1, lines[lines.length - 1].length);
        classes.push(currentClass);
    }
    if (currentProtocol) {
        currentProtocol.range = new vscode.Range(currentProtocol.range.start.line, 0, lines.length - 1, lines[lines.length - 1].length);
        protocols.push(currentProtocol);
    }
