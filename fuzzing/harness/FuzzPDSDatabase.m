...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    @autoreleasepool {
        ...
        @try {
            ...
            [gFuzzDatabase executeUnsafeRawSQL:@"SAVEPOINT fuzz" error:&error];

            // Test raw SQL path
            [gFuzzDatabase executeUnsafeRawSQL:sql error:&error];
            ...
            [gFuzzDatabase executeUnsafeRawSQL:@"ROLLBACK TO SAVEPOINT fuzz" error:&error];
        } @catch (NSException *exception) {
            // SQL parse exceptions are expected; rollback any partial state
            NSError *rollbackErr = nil;
            [gFuzzDatabase executeUnsafeRawSQL:@"ROLLBACK TO SAVEPOINT fuzz" error:&rollbackErr];
        }
    }
    return 0;
}
