...
static NSString *const kServiceAuthLxmCreateAccount =
    ...
    if (error) {
      *error = [NSError
          errorWithDomain:@"JWT"
                 ...
                 userInfo:@{
                   NSLocalizedDescriptionKey : @"Invalid JWT payload JSON"
                 }];
    }
    return nil;
  }
...
static NSString *didWebIdentifierFromIssuer(NSString *issuer,
                                            NSString *fallbackHost) {
  ...
  if (includePort) {
    return [NSString
        stringWithFormat:@"did:web:%@%%3A%lu", host, (unsigned long)port];
  }
  return [NSString stringWithFormat:@"did:web:%@", host];
}

static NSArray<NSString *> *
serviceAuthExpectedAudiences(PDSConfiguration *config) {
  ...
  if (canonicalHost.length > 0) {
    [audiences
        addObject:[NSString stringWithFormat:@"did:web:%@", canonicalHost]];
  }
...
static NSArray<NSString *> *jwtAllowedAlgorithmsForMinter(JWTMinter *minter) {
  ...
  NSMutableOrderedSet<NSString *> *algorithms =
...
static void
registerServerDescribeAndResolveLexiconMethods(XrpcDispatcher *dispatcher,
                                               PDSConfiguration *config) {
...
static NSArray<NSString *> *queryArrayValues(HttpRequest *request,
                                             NSString *key) {
  NSMutableArray<NSString *> *values = [NSMutableArray array];
  NSArray<NSString *> *pairs =
      [request.queryString componentsSeparatedByString:@"&"];
  for (NSString *pair in pairs) {
    if (pair.length == 0) {
      continue;
    }
    NSRange eqRange = [pair rangeOfString:@"="];
    NSString *rawKey = eqRange.location == NSNotFound
                           ? pair
                           : [pair substringToIndex:eqRange.location];
    NSString *rawValue = eqRange.location == NSNotFound
                             ? @""
                             : [pair substringFromIndex:eqRange.location + 1];

    NSString *decodedKey =
        [[rawKey stringByReplacingOccurrencesOfString:@"+" withString:@" "]
    ...
    for (NSString *component in
         [decodedValue componentsSeparatedByString:@","]) {
      NSString *trimmed =
  ...
  if (values.count == 0) {
    ...
    for (NSString *component in
         [singleValue componentsSeparatedByString:@","]) {
      NSString *trimmed =
          [component stringByTrimmingCharactersInSet:
                         [NSCharacterSet whitespaceAndNewlineCharacterSet]];
      if (trimmed.length > 0) {
...
static NSDictionary *adminInviteCodeViewFromRow(NSDictionary *row) {
  NSString *code =
      [row[@"code"] isKindOfClass:[NSString class]] ? row[@"code"] : @"";
  NSString *accountDid = [row[@"account_did"] isKindOfClass:[NSString class]]
                             ? row[@"account_did"]
                             : @"";
  NSInteger uses = [row[@"uses"] respondsToSelector:@selector(integerValue)]
                       ? [row[@"uses"] integerValue]
                       : 0;
  NSInteger maxUses =
      [row[@"max_uses"] respondsToSelector:@selector(integerValue)]
          ? [row[@"max_uses"] integerValue]
          : 1;
  if (maxUses < 0) {
    maxUses = 0;
  }
  ...
  BOOL disabled = [row[@"disabled"] respondsToSelector:@selector(boolValue)]
                      ? [row[@"disabled"] boolValue]
                      : NO;
  NSTimeInterval createdAt =
      [row[@"created_at"] respondsToSelector:@selector(doubleValue)]
          ? [row[@"created_at"] doubleValue]
          : 0;
...
static NSArray<NSDictionary *> *
loadAdminInviteCodeViews(PDSServiceDatabases *serviceDatabases, NSString *sort,
                         NSInteger limit, NSInteger offset, NSError **error) {
  PDSDatabase *db = [serviceDatabases serviceDatabaseWithError:error];
  if (!db) {
    return nil;
  }

  NSString *orderBy = [sort isEqualToString:@"usage"]
                          ? @"uses DESC, created_at DESC, code ASC"
                          : @"created_at DESC, code ASC";
  NSString *sql = [NSString
      stringWithFormat:
          @"SELECT code, account_did, created_at, uses, max_uses, disabled "
          @"FROM invite_codes ORDER BY %@ LIMIT ? OFFSET ?",
          orderBy];
  NSArray<NSDictionary *> *rows =
      [db executeParameterizedQuery:sql
                             params:@[ @(limit), @(offset) ]
                              error:error];
  [db close];
  if (!rows) {
  ...
  NSMutableArray<NSDictionary *> *codes =
...
static NSArray<NSString *> *
validatedUniqueStringArrayFromJSONValue(id value, NSString *fieldName,
                                        NSError **error) {
  if (!value) {
    return @[];
  }
  if (![value isKindOfClass:[NSArray class]]) {
    if (error) {
      *error = [NSError
          errorWithDomain:@"com.atproto.admin"
                     code:400
                 userInfo:@{
                   NSLocalizedDescriptionKey : [NSString
                       stringWithFormat:@"%@ must be an array of strings",
                                        fieldName ?: @"field"]
                 }];
    }
    return nil;
      ...
      if (error) {
        *error = [NSError
            errorWithDomain:@"com.atproto.admin"
                   ...
                   userInfo:@{
                     NSLocalizedDescriptionKey : [NSString
                         stringWithFormat:@"%@ must contain only strings",
                                          fieldName ?: @"field"]
                   }];
      }
      return nil;
      ...
      if (error) {
        *error = [NSError
            errorWithDomain:@"com.atproto.admin"
                   ...
                   userInfo:@{
                     NSLocalizedDescriptionKey : [NSString
                         stringWithFormat:@"%@ cannot contain empty strings",
                                          fieldName ?: @"field"]
                   }];
      }
      return nil;
...
static BOOL executeServiceUpdate(PDSDatabase *db, NSString *sql,
                                 NSArray *params, BOOL ignoreMissingTable,
                                 NSError **error) {
  NSError *updateError = nil;
  BOOL success =
      [db executeParameterizedUpdate:sql params:params error:&updateError];
  if (success || (ignoreMissingTable && isNoSuchTableError(updateError))) {
    return YES;
  }
  if (error) {
    *error = updateError
                 ?: [NSError errorWithDomain:@"com.atproto.admin"
                                        code:500
                                    userInfo:@{
                                      NSLocalizedDescriptionKey :
...
static NSData *pbkdf2HashPassword(NSString *password, NSData *salt,
                                  NSError **error) {
  ...
  int result = CCKeyDerivationPBKDF(kCCPBKDF2, password.UTF8String,
                                    (size_t)password.length, salt.bytes,
                                    (size_t)salt.length, kCCPRFHmacAlgSHA256,
                                    iterations, derivedKey, derivedKeyLength);
  if (result != kCCSuccess) {
    if (error) {
      *error = [NSError
          errorWithDomain:@"com.atproto.server"
                     code:500
                 userInfo:@{
                   NSLocalizedDescriptionKey : @"Failed to derive password hash"
                 }];
    }
    return nil;
  }
...
static BOOL updateAccountEmail(PDSServiceDatabases *serviceDatabases,
                               NSString *did, NSString *email,
                               NSError **error) {
  PDSDatabaseAccount *account =
      [serviceDatabases getAccountByDid:did error:error];
  if (!account) {
    return NO;
  }
  account.email = email;
...
static BOOL updateAccountPassword(PDSServiceDatabases *serviceDatabases,
                                  NSString *did, NSString *password,
                                  NSError **error) {
  PDSDatabaseAccount *account =
      [serviceDatabases getAccountByDid:did error:error];
  if (!account) {
...
static BOOL updateAccountSigningKey(PDSServiceDatabases *serviceDatabases,
                                    NSString *did, NSString *signingKey,
                                    NSError **error) {
  if (![signingKey hasPrefix:@"did:key:"]) {
    if (error) {
      *error = [NSError errorWithDomain:@"com.atproto.admin"
                               ...
                               userInfo:@{
                                 NSLocalizedDescriptionKey :
                                     @"signingKey must be a did:key identifier"
                               }];
    }
    return NO;
  }
  ...
  PDS_LOG_WARN(@"updateAccountSigningKey accepted but no DID document "
               @"persistence is configured for DID %@ (signingKey=%@)",
               did, signingKey);
  return YES;
}

static void registerAdminAccountMaintenanceMethods(
    XrpcDispatcher *dispatcher, PDSServiceDatabases *serviceDatabases,
    JWTMinter *jwtMinter, id<PDSAdminController> adminController) {
  [dispatcher registerComAtprotoAdminSearchAccounts:^(HttpRequest *request,
                                                      HttpResponse *response) {
    if (!authorizeAdminRequest(request, response, serviceDatabases, jwtMinter,
                               adminController)) {
      return;
    }
    if (request.method != HttpMethodGET) {
    ...
    if (limitParam.length > 0 &&
        (!parseStrictIntegerString(limitParam, &limit) || limit < 1 ||
         limit > 100)) {
      response.statusCode = HttpStatusBadRequest;
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
    ...
    if (cursorParam.length > 0 &&
        (!parseStrictIntegerString(cursorParam, &offset) || offset < 0)) {
      response.statusCode = HttpStatusBadRequest;
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
    ...
    NSString *emailQuery =
        [[request queryParamForKey:@"email"] lowercaseString];
    NSError *queryError = nil;
    NSArray<PDSDatabaseAccount *> *allAccounts =
        [serviceDatabases getAllAccountsWithError:&queryError];
    if (!allAccounts) {
      response.statusCode = HttpStatusInternalServerError;
      [response setJsonBody:@{
        @"error" : @"QueryFailed",
    ...
    NSMutableArray<PDSDatabaseAccount *> *filteredAccounts =
        [NSMutableArray arrayWithCapacity:allAccounts.count];
    for (PDSDatabaseAccount *account in allAccounts) {
      if (emailQuery.length > 0) {
        NSString *accountEmail = [account.email lowercaseString];
        if (accountEmail.length == 0 ||
            [accountEmail rangeOfString:emailQuery].location == NSNotFound) {
          continue;
        }
      }
    ...
    NSUInteger startIndex =
        (NSUInteger)MIN(offset, (NSInteger)filteredAccounts.count);
    NSUInteger endIndex =
        MIN(startIndex + (NSUInteger)limit, filteredAccounts.count);
    NSMutableArray<NSDictionary *> *views =
    ...
    if (endIndex < filteredAccounts.count) {
      result[@"cursor"] =
          [NSString stringWithFormat:@"%lu", (unsigned long)endIndex];
    }
  ...
  [dispatcher registerComAtprotoAdminSendEmail:^(HttpRequest *request,
                                                 HttpResponse *response) {
    if (!authorizeAdminRequest(request, response, serviceDatabases, jwtMinter,
                               adminController)) {
      return;
    ...
    if (![recipientDid isKindOfClass:[NSString class]] ||
        ...
        ![content isKindOfClass:[NSString class]] || content.length == 0 ||
        ![ATProtoValidator validateDID:recipientDid error:&didError] ||
        ![ATProtoValidator validateDID:senderDid error:&didError]) {
      response.statusCode = HttpStatusBadRequest;
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
        @"message" : didError.localizedDescription
            ?: @"Missing or invalid senderDid, recipientDid, or content"
      }];
      return;
    }
    ...
    PDSDatabaseAccount *recipientAccount =
        [serviceDatabases getAccountByDid:recipientDid error:&lookupError];
    if (!recipientAccount) {
      response.statusCode = HttpStatusNotFound;
      [response setJsonBody:@{
        @"error" : @"AccountNotFound",
        @"message" : lookupError.localizedDescription
            ?: @"Recipient account not found"
      }];
      return;
    }

    PDS_LOG_INFO(@"Admin sendEmail recipient=%@ sender=%@ subject=%@",
                 recipientDid, senderDid, subject ?: @"");
    response.statusCode = HttpStatusOK;
    [response setJsonBody:@{@"sent" : @YES}];
  }];

  [dispatcher registerComAtprotoAdminUpdateAccountEmail:^(
                  HttpRequest *request, HttpResponse *response) {
    if (!authorizeAdminRequest(request, response, serviceDatabases, jwtMinter,
                               adminController)) {
      return;
    ...
    NSString *did = nil;
    NSError *resolveError = nil;
    if (!resolveAccountIdentifierToDid(serviceDatabases, accountIdentifier,
                                       &did, &resolveError)) {
      if (resolveError.code == 404) {
        response.statusCode = HttpStatusNotFound;
        [response setJsonBody:@{
          @"error" : @"AccountNotFound",
          @"message" : resolveError.localizedDescription ?: @"Account not found"
        }];
      } else {
        response.statusCode = HttpStatusBadRequest;
        [response setJsonBody:@{
    ...
    PDSDatabaseAccount *existingAccount =
        [serviceDatabases getAccountByEmail:email error:&existingError];
    ...
    if (!updateAccountEmail(serviceDatabases, did, email, &updateError)) {
      if (updateError.code == 404) {
        ...
        [response setJsonBody:@{
          @"error" : @"AccountNotFound",
          @"message" : updateError.localizedDescription ?: @"Account not found"
        }];
      } else {
        response.statusCode = HttpStatusBadRequest;
        [response setJsonBody:@{
  ...
  [dispatcher registerComAtprotoAdminUpdateAccountHandle:^(
                  HttpRequest *request, HttpResponse *response) {
    if (!authorizeAdminRequest(request, response, serviceDatabases, jwtMinter,
                               adminController)) {
      return;
    ...
    if (![did isKindOfClass:[NSString class]] ||
        ![ATProtoValidator validateDID:did error:&didError]) {
      response.statusCode = HttpStatusBadRequest;
      [response setJsonBody:@{
        @"error" : @"InvalidDid",
    ...
    if (![ATProtoHandleValidator validateHandle:handle error:&handleError]) {
      ...
      [response setJsonBody:@{
        @"error" : @"InvalidHandle",
        @"message" : handleError.localizedDescription ?: @"Invalid handle"
      }];
      return;
    }
    NSString *normalizedHandle =
        [ATProtoHandleValidator normalizeHandle:handle];

    NSError *existingError = nil;
    PDSDatabaseAccount *existingAccount =
        [serviceDatabases getAccountByHandle:normalizedHandle
                                       error:&existingError];
    if (existingAccount && ![existingAccount.did isEqualToString:did]) {
      response.statusCode = HttpStatusBadRequest;
      [response setJsonBody:@{
        @"error" : @"HandleAlreadyInUse",
        @"message" : @"Handle is already used by another account"
      }];
      return;
    }
  ...
  [dispatcher registerComAtprotoAdminUpdateAccountPassword:^(
                  HttpRequest *request, HttpResponse *response) {
    if (!authorizeAdminRequest(request, response, serviceDatabases, jwtMinter,
                               adminController)) {
      return;
  ...
  [dispatcher registerComAtprotoAdminUpdateAccountSigningKey:^(
                  HttpRequest *request, HttpResponse *response) {
    if (!authorizeAdminRequest(request, response, serviceDatabases, jwtMinter,
                               adminController)) {
      return;
    ...
    if (![signingKey isKindOfClass:[NSString class]] ||
      ...
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
        @"message" : @"signingKey must be a did:key identifier"
      }];
      return;
    }

    NSError *updateError = nil;
    if (!updateAccountSigningKey(serviceDatabases, did, signingKey,
                                 &updateError)) {
      if (updateError.code == 404) {
        response.statusCode = HttpStatusNotFound;
        [response setJsonBody:@{
        ...
        response.statusCode = HttpStatusBadRequest;
        [response setJsonBody:@{
          @"error" : @"SigningKeyUpdateFailed",
          @"message" : updateError.localizedDescription
              ?: @"Failed to update signing key"
        }];
      }
      return;
    }
...
static void registerAdminAccountAndInviteMethods(
    XrpcDispatcher *dispatcher, PDSServiceDatabases *serviceDatabases,
    JWTMinter *jwtMinter, id<PDSAdminController> adminController) {
  [dispatcher registerComAtprotoAdminGetAccountInfo:^(HttpRequest *request,
                                                      HttpResponse *response) {
    if (!authorizeAdminRequest(request, response, serviceDatabases, jwtMinter,
                               adminController)) {
      return;
    ...
    if (!account) {
      ...
      [response setJsonBody:@{
        @"error" : @"AccountNotFound",
        @"message" : error.localizedDescription ?: @"Account not found"
      }];
      return;
    }
  ...
  [dispatcher registerComAtprotoAdminGetAccountInfos:^(HttpRequest *request,
                                                       HttpResponse *response) {
    if (!authorizeAdminRequest(request, response, serviceDatabases, jwtMinter,
                               adminController)) {
      return;
    ...
    if (dids.count == 0) {
      ...
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
        @"message" : @"Missing dids parameter"
      }];
      return;
    }

    NSMutableArray<NSDictionary *> *infos =
        [NSMutableArray arrayWithCapacity:dids.count];
    for (NSString *did in dids) {
      ...
      PDSDatabaseAccount *account =
          [serviceDatabases getAccountByDid:did error:nil];
      if (account) {
  ...
  [dispatcher registerComAtprotoAdminGetInviteCodes:^(HttpRequest *request,
                                                      HttpResponse *response) {
    if (!authorizeAdminRequest(request, response, serviceDatabases, jwtMinter,
                               adminController)) {
      return;
    ...
    if (![sort isEqualToString:@"recent"] && ![sort isEqualToString:@"usage"]) {
      ...
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
        @"message" : @"sort must be one of: recent, usage"
      }];
      return;
    }
    ...
    NSError *error = nil;
    NSArray<NSDictionary *> *codes =
        loadAdminInviteCodeViews(serviceDatabases, sort, limit, offset, &error);
    if (!codes) {
      response.statusCode = HttpStatusInternalServerError;
      [response setJsonBody:@{
        @"error" : @"QueryFailed",
    ...
    if (codes.count == (NSUInteger)limit) {
      result[@"cursor"] = [NSString
          stringWithFormat:@"%ld", (long)(offset + (NSInteger)codes.count)];
    }
  ...
  [dispatcher registerComAtprotoAdminDeleteAccount:^(HttpRequest *request,
                                                     HttpResponse *response) {
    if (!authorizeAdminRequest(request, response, serviceDatabases, jwtMinter,
                               adminController)) {
      return;
    ...
    if (!deleteAccountAsAdmin(serviceDatabases, did, &deleteError)) {
      if (deleteError.code == 404) {
        ...
        [response setJsonBody:@{
          @"error" : @"AccountNotFound",
          @"message" : deleteError.localizedDescription ?: @"Account not found"
        }];
      } else {
        response.statusCode = HttpStatusBadRequest;
        [response setJsonBody:@{
  ...
  [dispatcher registerComAtprotoAdminDisableAccountInvites:^(
                  HttpRequest *request, HttpResponse *response) {
    if (!authorizeAdminRequest(request, response, serviceDatabases, jwtMinter,
                               adminController)) {
      return;
    ...
    if (![accountDid isKindOfClass:[NSString class]] ||
        accountDid.length == 0) {
      response.statusCode = HttpStatusBadRequest;
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
    ...
    NSError *updateError = nil;
    if (!setInviteEnabledForAccount(serviceDatabases, accountDid, NO,
                                    &updateError)) {
      if (updateError.code == 404) {
        response.statusCode = HttpStatusNotFound;
        [response setJsonBody:@{
        ...
        response.statusCode = HttpStatusBadRequest;
        [response setJsonBody:@{
          @"error" : @"InviteUpdateFailed",
          @"message" : updateError.localizedDescription
              ?: @"Failed to disable account invites"
        }];
      }
      return;
    }
  ...
  [dispatcher registerComAtprotoAdminEnableAccountInvites:^(
                  HttpRequest *request, HttpResponse *response) {
    if (!authorizeAdminRequest(request, response, serviceDatabases, jwtMinter,
                               adminController)) {
      return;
    ...
    NSError *updateError = nil;
    if (!setInviteEnabledForAccount(serviceDatabases, accountDid, YES,
                                    &updateError)) {
      if (updateError.code == 404) {
        response.statusCode = HttpStatusNotFound;
        [response setJsonBody:@{
        ...
        [response setJsonBody:@{
          @"error" : @"InviteUpdateFailed",
          @"message" : updateError.localizedDescription
              ?: @"Failed to enable account invites"
        }];
      }
      return;
    }
  ...
  [dispatcher registerComAtprotoAdminDisableInviteCodes:^(
                  HttpRequest *request, HttpResponse *response) {
    if (!authorizeAdminRequest(request, response, serviceDatabases, jwtMinter,
                               adminController)) {
      return;
    ...
    NSArray<NSString *> *codes = validatedUniqueStringArrayFromJSONValue(
        body[@"codes"], @"codes", &validationError);
    if (!codes) {
      response.statusCode = HttpStatusBadRequest;
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
        @"message" : validationError.localizedDescription ?: @"Invalid codes"
      }];
      return;
    }
    NSArray<NSString *> *accounts = validatedUniqueStringArrayFromJSONValue(
        body[@"accounts"], @"accounts", &validationError);
    if (!accounts) {
      response.statusCode = HttpStatusBadRequest;
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
        @"message" : validationError.localizedDescription ?: @"Invalid accounts"
      }];
      return;
    }

    NSError *disableError = nil;
    if (![adminController disableInviteCodesWithCodes:codes
                                             accounts:accounts
                                                error:&disableError]) {
      if (disableError.code == 404) {
        ...
        [response setJsonBody:@{
          @"error" : @"AccountNotFound",
          @"message" : disableError.localizedDescription ?: @"Account not found"
        }];
      } else {
        response.statusCode = HttpStatusBadRequest;
        [response setJsonBody:@{
          @"error" : @"InviteUpdateFailed",
          @"message" : disableError.localizedDescription
              ?: @"Failed to disable invite codes"
        }];
      }
      return;
    }
...
static NSDictionary *labelLookupParamsFromRequest(HttpRequest *request,
                                                  NSString **errorMessage) {
  NSInteger limit = 50;
  NSString *limitParam = [request queryParamForKey:@"limit"];
  if (limitParam.length > 0 && (!parseStrictIntegerString(limitParam, &limit) ||
                                limit < 1 || limit > 250)) {
    if (errorMessage) {
  ...
  NSMutableDictionary *params =
      [NSMutableDictionary dictionaryWithObject:@(limit) forKey:@"limit"];
...
static void registerAdminModerationAndLabelMethods(
    XrpcDispatcher *dispatcher, PDSServiceDatabases *serviceDatabases,
    JWTMinter *jwtMinter, id<PDSAdminController> adminController,
    BOOL enforceMethodChecks) {
  [dispatcher
      registerMethod:@"com.atproto.admin.takeDownAccount"
             handler:^(HttpRequest *request, HttpResponse *response) {
               if (!authorizeAdminRequest(request, response, serviceDatabases,
               ...
               if (!success) {
                 response.statusCode = HttpStatusBadRequest;
                 [response setJsonBody:@{
                   @"error" : @"TakedownFailed",
                   @"message" : error.localizedDescription
                       ?: @"Failed to take down account"
                 }];
                 return;
               }
  ...
  [dispatcher registerComAtprotoLabelCreateLabel:^(HttpRequest *request,
                                                   HttpResponse *response) {
    if (!authorizeAdminRequest(request, response, serviceDatabases, jwtMinter,
                               adminController)) {
      return;
    ...
    NSDictionary *body = [request.jsonBody isKindOfClass:[NSDictionary class]]
                             ? request.jsonBody
                             : nil;
    ...
    if (error) {
      response.statusCode = HttpStatusBadRequest;
      [response setJsonBody:@{
        @"error" : @"LabelCreationFailed",
        @"message" : error.localizedDescription ?: @"Failed to create label"
      }];
      return;
    }
  ...
  [dispatcher registerComAtprotoLabelGetLabels:^(HttpRequest *request,
                                                 HttpResponse *response) {
    if (!authorizeAdminRequest(request, response, serviceDatabases, jwtMinter,
                               adminController)) {
      return;
    ...
    NSDictionary *params = [request.jsonBody isKindOfClass:[NSDictionary class]]
                               ? request.jsonBody
                               : nil;
    if (!params || params.count == 0) {
      ...
      if (!params) {
        ...
        [response setJsonBody:@{
          @"error" : @"InvalidRequest",
          @"message" : paramError ?: @"Invalid query parameters"
        }];
        return;
      }
    }
    ...
    if (error) {
      ...
      [response setJsonBody:@{
        @"error" : @"QueryFailed",
        @"message" : error.localizedDescription ?: @"Failed to fetch labels"
      }];
      return;
    }
...
static void registerPhase1IdentityAndAccountMethods(
    XrpcDispatcher *dispatcher, JWTMinter *jwtMinter,
    id<PDSAdminController> adminController,
    PDSServiceDatabases *serviceDatabases, PDSDatabasePool *userDatabasePool,
    PDSConfiguration *config, id<PDSEmailProvider> emailProvider) {
  [dispatcher registerComAtprotoIdentityRefreshIdentity:^(
                  HttpRequest *request, HttpResponse *response) {
    NSDictionary *body = request.jsonBody ?: @{};
    NSString *identifier =
        body[@"identifier"] ?: [request queryParamForKey:@"identifier"];
    ...
    NSDictionary *result =
        [XrpcIdentityHelper resolveIdentityInfoForIdentifier:identifier
                                            serviceDatabases:serviceDatabases
                                                   errorName:&errorName
                                                       error:&error];
    if (!result) {
  ...
  [dispatcher registerComAtprotoIdentityResolveHandle:^(
                  HttpRequest *request, HttpResponse *response) {
    ...
    PDSDatabaseAccount *account =
        [serviceDatabases getAccountByHandle:handle error:nil];
    if (account) {
      response.statusCode = HttpStatusOK;
    ...
    [handleResolver
        resolveHandle:handle
           completion:^(NSString *_Nullable did, NSError *_Nullable error) {
             resolvedDid = did;
  ...
  [dispatcher registerComAtprotoIdentityResolveIdentity:^(
                  HttpRequest *request, HttpResponse *response) {
  ...
  [dispatcher registerComAtprotoIdentityGetRecommendedDidCredentials:^(
                  HttpRequest *request, HttpResponse *response) {
  ...
  [dispatcher registerComAtprotoIdentityRequestPlcOperationSignature:^(
                  HttpRequest *request, HttpResponse *response) {
    ...
    for (int i = 0; i < 8; i++) {
      [token appendFormat:@"%C",
                          [alphabet
                              characterAtIndex:arc4random_uniform(
                                                   (uint32_t)alphabet.length)]];
    }
    ...
    if (emailProvider && account.email.length > 0) {
      ...
      NSString *body = [NSString
          stringWithFormat:@"Your confirmation code for updating your PLC "
                           @"identity is: %@\n\nIf you did not request this "
                           @"change, you can safely ignore this email.",
                           token];

      NSError *emailError = nil;
      if (![emailProvider sendEmailTo:account.email
                              subject:subject
                                 body:body
                                error:&emailError]) {
        PDS_LOG_ERROR(@"Failed to send PLC operation email to %@: %@",
                      account.email, emailError);
        response.statusCode = HttpStatusInternalServerError;
        [response setJsonBody:@{
          @"error" : @"EmailFailed",
  ...
  [dispatcher registerComAtprotoIdentitySignPlcOperation:^(
                  HttpRequest *request, HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
    ...
    if (!signingDidKey) {
      ...
      [response setJsonBody:@{
        ...
        @"message" : @"Unable to determine signing key for PLC operation"
      }];
      return;
    }
    ...
    if (![keyManager loadOrGenerateKeyWithError:&keyLoadError]) {
      ...
      [response setJsonBody:@{
        @"error" : @"KeyUnavailable",
        @"message" : keyLoadError.localizedDescription
            ?: @"Failed to load PLC rotation key"
      }];
      return;
    }
    NSString *serverRotationKey = keyManager.rotationKeyDidKey;

    NSArray *rotationKeys = [rotationKeysValue isKindOfClass:[NSArray class]]
                                ? rotationKeysValue
                                : nil;
    ...
    NSDictionary *verificationMethods =
        [verificationMethodsValue isKindOfClass:[NSDictionary class]]
            ? verificationMethodsValue
            : nil;
    if (verificationMethods.count == 0) {
      verificationMethods = @{@"atproto" : signingDidKey};
    }

    NSArray *alsoKnownAs = [alsoKnownAsValue isKindOfClass:[NSArray class]]
                               ? alsoKnownAsValue
                               : nil;
    if (alsoKnownAs.count == 0) {
      ...
      if (account.handle.length > 0) {
        alsoKnownAs =
            @[ [NSString stringWithFormat:@"at://%@", account.handle] ];
      } else {
        alsoKnownAs = @[];
      }
    }

    NSDictionary *services = [servicesValue isKindOfClass:[NSDictionary class]]
                                 ? servicesValue
                                 : nil;
    if (services.count == 0) {
      services = [XrpcIdentityHelper defaultPdsServiceForConfig:config];
    }
    if (!services) {
      services = @{};
    }
    ...
    DIDPLCResolver *plcResolver =
        [[DIDPLCResolver alloc] initWithPlcUrl:plcUrl];
    NSError *auditError = nil;
    NSArray *auditLog =
        [plcResolver resolveAuditLogForDID:did error:&auditError];
    ...
    if (auditLog && auditLog.count > 0) {
      ...
      for (id opDict in auditLog) {
        if ([opDict isKindOfClass:[NSDictionary class]]) {
          PLCOperation *op =
              [PLCOperation operationFromDictionary:opDict error:nil];
          if (op)
            [ops addObject:op];
        }
      }
      ...
      if (ops.count > 0) {
        ...
        PLCDIDState *state =
            [PLCStateReplayer replayHistory:ops error:&replayError];
        if (state && state.tombstoned) {
          response.statusCode = HttpStatusBadRequest;
          [response setJsonBody:@{
            @"error" : @"AccountTombstoned",
        ...
        if (lastOp) {
          NSString *lastCid =
              [PLCOperation calculateCIDForOperation:[lastOp toDictionary]
                                               error:nil];
          if (lastCid) {
            prev = lastCid;
          }
    ...
    NSError *cborError = nil;
    NSData *cborData =
        [ATProtoCBORSerialization encodeDataWithJSONObject:operationData
                                                     error:&cborError];
    if (!cborData) {
      response.statusCode = HttpStatusBadRequest;
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
        @"message" : cborError.localizedDescription
            ?: @"Invalid PLC operation payload"
      }];
      return;
    }
  ...
  [dispatcher registerComAtprotoIdentitySubmitPlcOperation:^(
                  HttpRequest *request, HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
    ...
    if (operationDid.length > 0 && ![operationDid isEqualToString:did]) {
      ...
      [response setJsonBody:@{
        @"error" : @"Forbidden",
        @"message" : @"Operation DID does not match authenticated account"
      }];
      return;
    }
    ...
    if (![opType isEqualToString:@"plc_operation"]) {
      ...
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
        @"message" : @"Operation must be type plc_operation"
      }];
      return;
    }
    ...
    if (![keyManager loadOrGenerateKeyWithError:&keyLoadError]) {
      ...
      [response setJsonBody:@{
        @"error" : @"KeyUnavailable",
        @"message" : @"Server rotation key not available"
      }];
      return;
    }
    NSString *serverRotationKey = keyManager.rotationKeyDidKey;
    ...
    if (![rotationKeys isKindOfClass:[NSArray class]] ||
      ...
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
        @"message" : @"Operation must include rotationKeys"
      }];
      return;
    }
    if (![rotationKeys containsObject:serverRotationKey]) {
      ...
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
        @"message" : @"Server rotation key must be included in rotationKeys"
      }];
      return;
    }
      ...
      if ([atprotoPds isKindOfClass:[NSDictionary class]]) {
        ...
        if (endpoint && ![endpoint isEqualToString:expectedEndpoint]) {
          ...
          [response setJsonBody:@{
            ...
            @"message" : [NSString
                stringWithFormat:
                    @"services.atproto_pds.endpoint must match server URL %@",
                    expectedEndpoint]
          }];
          return;
        }
    ...
    PDSDatabaseAccount *account =
        [serviceDatabases getAccountByDid:did error:nil];
    if (account.handle.length > 0) {
      NSString *expectedAka =
          [NSString stringWithFormat:@"at://%@", account.handle];
      if (![alsoKnownAs isKindOfClass:[NSArray class]] ||
          ![alsoKnownAs containsObject:expectedAka]) {
        response.statusCode = HttpStatusBadRequest;
        [response setJsonBody:@{
          @"error" : @"InvalidRequest",
          @"message" : [NSString
              stringWithFormat:@"alsoKnownAs must include %@", expectedAka]
        }];
        return;
      }
    }
    ...
    if (auditLog && auditLog.count > 0) {
      ...
      if (ops.count > 0) {
        PLCOperation *lastOp = ops.lastObject;
        NSString *expectedPrev =
            [PLCOperation calculateCIDForOperation:[lastOp toDictionary]
                                             error:nil];
        id submittedPrev = opData[@"prev"];
        if (expectedPrev && submittedPrev != [NSNull null] &&
            ![submittedPrev isEqualToString:expectedPrev]) {
          response.statusCode = HttpStatusBadRequest;
          [response setJsonBody:@{
            @"error" : @"InvalidRequest",
            @"message" : @"prev does not match last operation CID"
          }];
          return;
        }
      }
    ...
    if (config.debugSkipPlcOperations ||
        [config.plcURL isEqualToString:@"mock"]) {
    ...
    NSData *postData = [NSJSONSerialization dataWithJSONObject:opToSubmit
                                                       options:0
                                                         error:&auditError];
    if (!postData) {
      response.statusCode = HttpStatusInternalServerError;
      [response setJsonBody:@{
        @"error" : @"InternalError",
        @"message" : @"Failed to serialize operation"
      }];
      return;
    }

    NSURL *submitUrl =
        [NSURL URLWithString:[NSString stringWithFormat:@"%@/%@", plcUrl, did]];
    NSMutableURLRequest *submitRequest =
        [NSMutableURLRequest requestWithURL:submitUrl];
    submitRequest.HTTPMethod = @"POST";
    [submitRequest setValue:@"application/json"
    ...
    [[[NSURLSession sharedSession]
        dataTaskWithRequest:submitRequest
    ...
    dispatch_semaphore_wait(
        sema, dispatch_time(DISPATCH_TIME_NOW, 10 * NSEC_PER_SEC));
    ...
    if (statusCode != 200 && statusCode != 202) {
      NSString *bodyStr =
          responseData ? [[NSString alloc] initWithData:responseData
      ...
      [response setJsonBody:@{
        @"error" : @"UpstreamError",
        @"message" :
            [NSString stringWithFormat:@"PLC directory returned %ld: %@",
                                       (long)statusCode, bodyStr]
      }];
      return;
    }
  ...
  [dispatcher registerComAtprotoIdentityUpdateHandle:^(HttpRequest *request,
                                                       HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
    ...
    if (![ATProtoHandleValidator validateHandle:handle error:&validateError]) {
      ...
      [response setJsonBody:@{
        @"error" : @"InvalidHandle",
        @"message" : validateError.localizedDescription ?: @"Invalid handle"
      }];
      return;
    }
    NSString *normalizedHandle =
    ...
    if (![XrpcIdentityHelper updateAccountHandle:serviceDatabases
      ...
      [response setJsonBody:@{
        @"error" : @"HandleUpdateFailed",
        @"message" : error.localizedDescription ?: @"Failed to update handle"
      }];
      return;
    }
  ...
  [dispatcher registerComAtprotoServerGetAccountInviteCodes:^(
                  HttpRequest *request, HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
    ...
    NSError *error = nil;
    NSString *code =
        [serviceDatabases getInviteCodeForAccount:did error:&error];
    if (error) {
      response.statusCode = HttpStatusInternalServerError;
      [response setJsonBody:@{
        @"error" : @"InviteCodeLookupFailed",
        @"message" : error.localizedDescription
  ...
  [dispatcher registerComAtprotoServerRequestEmailConfirmation:^(
                  HttpRequest *request, HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
  ...
  [dispatcher registerComAtprotoServerRequestEmailUpdate:^(
                  HttpRequest *request, HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
  ...
  [dispatcher registerComAtprotoServerConfirmEmail:^(HttpRequest *request,
                                                     HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
    ...
    if (!isLikelyEmail(email) ||
        (account.email.length > 0 &&
         ![[account.email lowercaseString]
             isEqualToString:[email lowercaseString]])) {
      response.statusCode = HttpStatusBadRequest;
      [response setJsonBody:@{
        @"error" : @"InvalidEmail",
        @"message" : @"Provided email does not match account"
      }];
      return;
    }
  ...
  [dispatcher registerComAtprotoServerRequestAccountDelete:^(
                  HttpRequest *request, HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
  ...
  [dispatcher registerComAtprotoServerRequestPasswordReset:^(
                  HttpRequest *request, HttpResponse *response) {
    NSDictionary *body = request.jsonBody ?: @{};
    NSString *email = body[@"email"];
    if (email.length == 0 || !isLikelyEmail(email)) {
  ...
  [dispatcher registerComAtprotoServerReserveSigningKey:^(
                  HttpRequest *request, HttpResponse *response) {
    NSDictionary *body = request.jsonBody ?: @{};
    NSString *did = body[@"did"];
  ...
  [dispatcher registerComAtprotoServerResetPassword:^(HttpRequest *request,
                                                      HttpResponse *response) {
    NSDictionary *body = request.jsonBody ?: @{};
    NSString *token = body[@"token"];
    NSString *password = body[@"password"];
    ...
    NSError *accountError = nil;
    PDSDatabaseAccount *account =
        [serviceDatabases getAccountByDid:token error:&accountError];
    if (!account) {
      response.statusCode = HttpStatusBadRequest;
      [response setJsonBody:@{
        @"error" : @"InvalidToken",
    ...
    NSError *hashError = nil;
    NSData *newHash =
        pbkdf2HashPassword(password, account.passwordSalt, &hashError);
    if (!newHash) {
      response.statusCode = HttpStatusInternalServerError;
      [response setJsonBody:@{
        @"error" : @"PasswordResetFailed",
        @"message" : hashError.localizedDescription
    ...
    if (![serviceDatabases updateAccount:account error:&updateError]) {
      ...
      [response setJsonBody:@{
        @"error" : @"PasswordResetFailed",
        @"message" : updateError.localizedDescription
            ?: @"Failed to persist new password"
      }];
      return;
    }
  ...
  [dispatcher registerComAtprotoServerUpdateEmail:^(HttpRequest *request,
                                                    HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
...
static BOOL validateDidWebServiceAuthForAccountCreation(
    HttpRequest *request, HttpResponse *response, NSString *did,
    PDSConfiguration *config) {
  PDSConfiguration *effectiveConfig =
      config ?: [PDSConfiguration sharedConfiguration];
  ...
  if (!authHeader || ![authHeader hasPrefix:@"Bearer "]) {
    ...
    [response setJsonBody:@{
      @"error" : @"InvalidToken",
      @"message" : @"Missing service auth token"
    }];
    return NO;
  }
  ...
  if (!jwt || parseError) {
    ...
    [response setJsonBody:@{
      @"error" : @"InvalidToken",
      @"message" : @"Unable to parse service auth token"
    }];
    return NO;
  }
  ...
  if (!payloadDict) {
    ...
    [response setJsonBody:@{
      @"error" : @"InvalidToken",
      @"message" : @"Unable to decode service auth payload"
    }];
    return NO;
  }
  ...
  if (!lxm || ![lxm isEqualToString:kServiceAuthLxmCreateAccount]) {
    ...
    [response setJsonBody:@{
      @"error" : @"InvalidToken",
      @"message" : @"Service auth token has invalid lxm"
    }];
    return NO;
  }
  ...
  NSDictionary *atprotoData =
      [resolver resolveAtprotoDataForDID:did error:&resolveError];
  NSString *signingKey = atprotoData[@"signingKey"];
  if (!signingKey) {
    response.statusCode = HttpStatusUnauthorized;
    [response setJsonBody:@{
      @"error" : @"InvalidToken",
      @"message" : @"DID document missing signing key"
    }];
    return NO;
  }
  ...
  NSData *signingKeyBytes =
      [XrpcMethodRegistry publicKeyBytesFromMultibase:signingKey
                                                error:&decodeError];
  if (!signingKeyBytes) {
    response.statusCode = HttpStatusUnauthorized;
    [response setJsonBody:@{
      @"error" : @"InvalidToken",
      @"message" : @"Unable to decode signing key"
    }];
    return NO;
  }
  ...
  NSError *keyError = nil;
  NSData *publicKey =
      [[Secp256k1 shared] normalizedPublicKey:signingKeyBytes error:&keyError];
  if (!publicKey) {
    response.statusCode = HttpStatusUnauthorized;
    [response setJsonBody:@{
      @"error" : @"InvalidToken",
      @"message" : @"Unable to normalize signing key"
    }];
    return NO;
  }
  ...
  if (!iss || ![iss isEqualToString:did]) {
    ...
    [response setJsonBody:@{
      @"error" : @"InvalidToken",
      @"message" : @"Service auth token has invalid issuer"
    }];
    return NO;
  }

  NSString *aud = jwt.payload.aud;
  NSArray<NSString *> *expectedAudiences =
      serviceAuthExpectedAudiences(effectiveConfig);
  ...
  if (!aud || ![expectedAudiences containsObject:audBase]) {
    ...
    [response setJsonBody:@{
      @"error" : @"InvalidToken",
      @"message" : @"Service auth token has invalid audience"
    }];
    return NO;
  }
...
static void registerServerAccountAndSessionMethods(
    XrpcDispatcher *dispatcher, JWTMinter *jwtMinter,
    id<PDSAdminController> adminController,
    id<PDSAccountService> accountService,
    PDSRepositoryService *repositoryService,
    PDSServiceDatabases *serviceDatabases, PDSDatabasePool *userDatabasePool,
    PDSConfiguration *config, BOOL enforceDidWebServiceAuth) {
  [dispatcher registerComAtprotoServerCreateAccount:^(HttpRequest *request,
                                                      HttpResponse *response) {
    NSDictionary *body = request.jsonBody;
    NSString *email = body[@"email"];
    NSString *handle = body[@"handle"];
    ...
    if (!email || !password || !handle) {
      ...
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
        @"message" : @"Missing email, handle, or password"
      }];
      return;
    }
    ...
    if (did.length > 0) {
      ...
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
        @"message" : @"Cannot specify DID during account creation. Import is "
    ...
    if (config.inviteCodeRequired) {
      ...
      // Assuming serviceDatabases provides a way to check codes, or we need to
      // access the store directly. Based on previous research, ServiceDatabases
      // has useInviteCode:error:
      if (![serviceDatabases useInviteCode:inviteCode error:&inviteError]) {
        response.statusCode = HttpStatusBadRequest;
        [response setJsonBody:@{
          @"error" : @"InvalidInviteCode",
          @"message" : inviteError.localizedDescription
    ...
    if (enforceDidWebServiceAuth && did.length > 0 &&
        [did hasPrefix:@"did:web:"]) {
      if (!validateDidWebServiceAuthForAccountCreation(request, response, did,
                                                       config)) {
        return;
      }
    }
    ...
    NSError *error = nil;
    NSDictionary *result = [accountService
    ...
    if (createdDid && repositoryService) {
      ...
      if (![repositoryService initializeRepoForDid:createdDid
                                             error:&initError]) {
        PDS_LOG_ERROR(@"Failed to initialize repo for DID %@: %@", createdDid,
                      initError);
      }
    }
  ...
  [dispatcher registerComAtprotoServerGetSession:^(HttpRequest *request,
                                                   HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
    ...
    if (error || !account) {
      ...
      [response setJsonBody:@{
        @"error" : @"AccountNotFound",
        @"message" : @"Account not found for session"
      }];
      return;
    }
  ...
  [dispatcher registerComAtprotoServerRefreshSession:^(HttpRequest *request,
                                                       HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    ...
    NSDictionary *session =
        [accountService refreshAccessToken:refreshToken error:&error];
  ...
  [dispatcher registerComAtprotoServerDeleteSession:^(HttpRequest *request,
                                                      HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
    ...
    if (!success) {
      ...
      [response setJsonBody:@{
        ...
        @"message" : error.localizedDescription ?: @"Failed to delete session"
      }];
      return;
    }
  ...
  [dispatcher registerComAtprotoServerCreateInviteCode:^(
                  HttpRequest *request, HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
    ...
    if (![targetDid isEqualToString:did]) {
      ...
      [response setJsonBody:@{
        @"error" : @"Forbidden",
        @"message" : @"Cannot create invite codes for other accounts"
      }];
      return;
    }
    ...
    NSError *error = nil;
    NSString *code = nil;
    if (!createInviteCodeInDatabase(serviceDatabases, targetDid, useCount,
                                    &code, &error)) {
      response.statusCode = HttpStatusBadRequest;
      [response setJsonBody:@{
        @"error" : @"InviteCodeCreateFailed",
        @"message" : error.localizedDescription
            ?: @"Failed to create invite code"
      }];
      return;
  ...
  [dispatcher registerComAtprotoServerCreateInviteCodes:^(
                  HttpRequest *request, HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
    ...
    if (![forAccounts isKindOfClass:[NSArray class]] ||
        forAccounts.count == 0) {
      forAccounts = @[ did ];
    }
    ...
    if (codeCount <= 0 || codeCount > 100) {
      ...
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
        @"message" : @"codeCount must be between 1 and 100"
      }];
      return;
    }
    ...
    for (NSString *accountDid in forAccounts) {
      if (![accountDid isKindOfClass:[NSString class]] ||
          ![accountDid isEqualToString:did]) {
        response.statusCode = HttpStatusForbidden;
        [response setJsonBody:@{
          @"error" : @"Forbidden",
          @"message" : @"Cannot create invite codes for other accounts"
    ...
    for (NSString *accountDid in forAccounts) {
      NSMutableArray<NSString *> *codes =
          [NSMutableArray arrayWithCapacity:(NSUInteger)codeCount];
      for (NSInteger i = 0; i < codeCount; i++) {
        NSError *error = nil;
        NSString *code = nil;
        if (!createInviteCodeInDatabase(serviceDatabases, accountDid, useCount,
                                        &code, &error)) {
          response.statusCode = HttpStatusBadRequest;
          [response setJsonBody:@{
            @"error" : @"InviteCodeCreateFailed",
  ...
  [dispatcher registerComAtprotoServerCreateAppPassword:^(
                  HttpRequest *request, HttpResponse *response) {
    if (request.method != HttpMethodPOST) {
      response.statusCode = HttpStatusMethodNotAllowed;
      [response setHeader:@"POST" forKey:@"Allow"];
    ...
    NSError *error = nil;
    NSDictionary *result =
        [serviceDatabases createAppPasswordForAccount:did
                                                 name:name
  ...
  [dispatcher registerComAtprotoServerListAppPasswords:^(
                  HttpRequest *request, HttpResponse *response) {
    if (request.method != HttpMethodGET) {
      response.statusCode = HttpStatusMethodNotAllowed;
      [response setHeader:@"GET" forKey:@"Allow"];
    ...
    NSError *error = nil;
    NSArray<NSDictionary *> *passwords =
        [serviceDatabases listAppPasswordsForAccount:did error:&error];
    if (error) {
      response.statusCode = HttpStatusInternalServerError;
      [response setJsonBody:@{
        @"error" : @"AppPasswordListFailed",
        @"message" : error.localizedDescription
  ...
  [dispatcher registerComAtprotoServerRevokeAppPassword:^(
                  HttpRequest *request, HttpResponse *response) {
    if (request.method != HttpMethodPOST) {
      response.statusCode = HttpStatusMethodNotAllowed;
      [response setHeader:@"POST" forKey:@"Allow"];
    ...
    NSError *error = nil;
    BOOL success = [serviceDatabases revokeAppPasswordForAccount:did
                                                            name:name
                                                           error:&error];
    if (!success) {
      response.statusCode = HttpStatusBadRequest;
      [response setJsonBody:@{
        @"error" : @"AppPasswordRevokeFailed",
        @"message" : error.localizedDescription
  ...
  [dispatcher registerComAtprotoServerGetServiceAuth:^(HttpRequest *request,
                                                       HttpResponse *response) {
    ...
    if (lxm.length > 0) {
      ...
      if (![ATProtoValidator validateNSID:lxm error:&lxmError]) {
        ...
        [response setJsonBody:@{
          @"error" : @"InvalidRequest",
          @"message" : lxmError.localizedDescription ?: @"Invalid lxm parameter"
        }];
        return;
      }
    }
    ...
    if (![ATProtoValidator validateDID:audDid error:&audError]) {
      ...
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
        @"message" : audError.localizedDescription ?: @"Invalid aud DID"
      }];
      return;
    }
    ...
    if (hasRequestedExp && requestedExp <= nowSeconds) {
      ...
      [response setJsonBody:@{
        @"error" : @"BadExpiration",
        @"message" : @"exp must be in the future"
      }];
      return;
    }
    ...
    payload[@"exp"] =
        @(hasRequestedExp ? requestedExp : (long long)(nowSeconds + 60));
    ...
    NSString *token = [minter signPayload:payload
    ...
    if (!token) {
      ...
      [response setJsonBody:@{
        ...
        @"message" : mintError.localizedDescription
            ?: @"Failed to mint service auth token"
      }];
      return;
    }
...
static void registerRepoCoreMethods(XrpcDispatcher *dispatcher,
                                    ...
                                    PDSRepositoryService *repositoryService,
                                    PDSServiceDatabases *serviceDatabases) {
  [dispatcher registerComAtprotoRepoListRecords:^(HttpRequest *request,
                                                  HttpResponse *response) {
    ...
    if (!collection) {
      ...
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
        @"message" : @"Missing collection parameter"
      }];
      return;
    }
    ...
    if ([repo hasPrefix:@"did:"]) {
      ...
      PDSDatabaseAccount *account =
          [serviceDatabases getAccountByHandle:repo error:nil];
      if (account) {
        did = account.did;
      } else {
        HandleResolver *handleResolver = [[HandleResolver alloc] init];
        __block NSString *resolvedDid = nil;
        dispatch_semaphore_t semaphore = dispatch_semaphore_create(0);
        [handleResolver resolveHandle:repo
                           completion:^(NSString *_Nullable resolved,
    ...
    if (!did) {
      ...
      [response setJsonBody:@{
        @"error" : @"RepoNotFound",
        @"message" :
            [NSString stringWithFormat:@"Could not find repo: %@", repo]
      }];
      return;
    }
  ...
  [dispatcher registerComAtprotoRepoGetRecord:^(HttpRequest *request,
                                                HttpResponse *response) {
    NSString *repo = [request queryParamForKey:@"repo"];
    NSString *collection = [request queryParamForKey:@"collection"];
    ...
    if (!collection || !rkey) {
      ...
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
        @"message" : @"Missing collection or rkey parameter"
      }];
      return;
    }
    ...
    NSError *error = nil;
    NSDictionary *record =
        [recordService getRecord:uri forDid:did error:&error];
  ...
  [dispatcher registerComAtprotoRepoCreateRecord:^(HttpRequest *request,
                                                   HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
    ...
    if (repo && ![repo isEqualToString:did]) {
      ...
      [response setJsonBody:@{
        @"error" : @"Forbidden",
        @"message" : @"Cannot create record for another user"
      }];
      return;
    }
    ...
    NSString *uri =
        [NSString stringWithFormat:@"at://%@/%@/%@", did, collection, rkey];
    NSDictionary *createdRecord =
        [recordService getRecord:uri forDid:did error:nil];

    response.statusCode = HttpStatusOK;
    [response setJsonBody:createdRecord ?: @{@"uri" : uri}];
  }];

  [dispatcher registerComAtprotoRepoDeleteRecord:^(HttpRequest *request,
                                                   HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
    ...
    if (repo && ![repo isEqualToString:did]) {
      ...
      [response setJsonBody:@{
        @"error" : @"Forbidden",
        @"message" : @"Cannot delete record for another user"
      }];
      return;
    }
    ...
    if (!success) {
      response.statusCode = HttpStatusBadRequest;
      [response setJsonBody:@{
        @"error" : @"RecordDeletionFailed",
        @"message" : error.localizedDescription ?: @"Failed to delete record"
      }];
      return;
    }
  ...
  [dispatcher registerComAtprotoRepoUploadBlob:^(HttpRequest *request,
                                                 HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
    ...
    if (contentType &&
        [contentType isEqualToString:@"application/x-msdownload"]) {
      response.statusCode = HttpStatusBadRequest;
      [response setJsonBody:@{
        @"error" : @"InvalidMimeType",
        @"message" : @"Forbidden MIME type"
      }];
      return;
    }
    NSError *error = nil;
    NSDictionary *result =
        [blobService uploadBlob:blobData
                         forDid:did
  ...
  [dispatcher registerComAtprotoRepoDeleteBlob:^(HttpRequest *request,
                                                 HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
  ...
  [dispatcher registerComAtprotoRepoListMissingBlobs:^(HttpRequest *request,
                                                       HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
  ...
  [dispatcher registerComAtprotoRepoDescribeRepo:^(HttpRequest *request,
    ...
    if ([identifier hasPrefix:@"did:"]) {
      ...
      NSString *normalizedHandle =
          [ATProtoHandleValidator normalizeHandle:identifier];
      localAccount =
          [serviceDatabases getAccountByHandle:normalizedHandle error:nil];
    ...
    NSString *handleFromDidDoc =
        normalizedAtHandleFromAlsoKnownAs(doc.alsoKnownAs);
    NSString *accountHandle = localAccount.handle.length > 0
                                  ? [localAccount.handle lowercaseString]
                                  : @"handle.invalid";
    BOOL handleIsCorrect = (handleFromDidDoc.length > 0 &&
                            [handleFromDidDoc isEqualToString:accountHandle]);
    ...
    if ([stats[@"collections"] isKindOfClass:[NSArray class]]) {
      for (NSDictionary *col in stats[@"collections"]) {
        if ([col isKindOfClass:[NSDictionary class]] &&
            [col[@"collection"] isKindOfClass:[NSString class]]) {
  ...
  [dispatcher registerComAtprotoRepoPutRecord:^(HttpRequest *request,
                                                HttpResponse *response) {
    NSString *authHeader = [request headerForKey:@"Authorization"];
    NSString *did = [XrpcMethodRegistry extractDIDFromAuthHeader:authHeader
                                                       jwtMinter:jwtMinter
    ...
    if (repo && ![repo isEqualToString:did]) {
      ...
      [response setJsonBody:@{
        @"error" : @"Forbidden",
        @"message" : @"Cannot update record for another user"
      }];
      return;
    }
    ...
    if (!collection || !rkey || !record) {
      ...
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
        @"message" : @"Missing collection, rkey, or record"
      }];
      return;
    }
    ...
    if (!success) {
      response.statusCode = HttpStatusBadRequest;
      [response setJsonBody:@{
        @"error" : @"RecordUpdateFailed",
        @"message" : error.localizedDescription ?: @"Failed to update record"
      }];
      return;
    }
  ...
  [dispatcher registerComAtprotoRepoApplyWrites:^(HttpRequest *request,
                                                  HttpResponse *response) {
    NSDictionary *body = request.jsonBody;
    if (!body) {
      response.statusCode = HttpStatusBadRequest;
    ...
    if (repo && ![repo isEqualToString:did]) {
      ...
      [response setJsonBody:@{
        @"error" : @"Forbidden",
        @"message" : @"Cannot apply writes for another user"
      }];
      return;
    }
    ...
    if (!writes || ![writes isKindOfClass:[NSArray class]]) {
      ...
      [response setJsonBody:@{
        @"error" : @"InvalidRequest",
        @"message" : @"Missing or invalid writes array"
      }];
      return;
    }
    ...
    if (!result) {
      ...
      [response setJsonBody:@{
        @"error" : @"WriteFailed",
        @"message" : error.localizedDescription ?: @"Failed to apply writes"
      }];
      return;
    }
...
static void registerSyncCoreMethods(
    XrpcDispatcher *dispatcher, JWTMinter *jwtMinter,
    id<PDSAdminController> adminController,
    PDSServiceDatabases *serviceDatabases, PDSDatabasePool *userDatabasePool,
    PDSRecordService *recordService, PDSBlobService *blobService,
    PDSRepositoryService *repositoryService, PDSConfiguration *config) {
  ...
  [dispatcher registerComAtprotoSyncListRepos:^(HttpRequest *request,
    ...
    while (scanIndex < (NSInteger)accounts.count &&
      ...
      if (account.did.length > 0) {
        ...
        if (!latest) {
          // Ensure empty repos are visible by creating the initial empty commit
          // if needed.
  ...
  [dispatcher registerComAtprotoSyncNotifyOfUpdate:^(HttpRequest *request,
    ...
    PDS_LOG_INFO(@"Received notifyOfUpdate for hostname: %@ (deprecated, use "
                 @"requestCrawl)",
                 hostname);
