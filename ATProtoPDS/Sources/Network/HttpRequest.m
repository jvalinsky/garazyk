...
- (NSDictionary *)parseMultipartFormData:(NSData *)body headers:(NSDictionary *)headers {
    if (!body || body.length == 0) {
        return nil;
    }
    ...
    NSData *boundaryData = [boundaryMarker dataUsingEncoding:NSUTF8StringEncoding];
    ...
    while (position < body.length) {
        // Find next boundary
        NSRange boundaryRange = [body rangeOfData:boundaryData options:0 range:NSMakeRange(position, body.length - position)];
        ...
        if (position + 2 <= body.length) {
            ...
            if ([endCheck isEqualToData:[@"--" dataUsingEncoding:NSUTF8StringEncoding]]) {
                break; // End of multipart data
            }
        }

        // Find headers/body separator
        NSRange separatorRange = [body rangeOfData:crlfCrlfData options:0 range:NSMakeRange(position, body.length - position)];
        ...
        NSData *headersData = [body subdataWithRange:NSMakeRange(headersStart, headersEnd - headersStart)];
        ...
        NSRange nextBoundaryRange = [body rangeOfData:boundaryData options:0 range:NSMakeRange(position, body.length - position)];
        ...
        if (bodyEnd > position) {
            ...
            NSData *fieldData = [body subdataWithRange:NSMakeRange(position, bodyLength)];

            // Remove trailing CRLF if present
            if (bodyLength >= 2) {
                NSData *trailingCrlf = [fieldData subdataWithRange:NSMakeRange(bodyLength - 2, 2)];
                if ([trailingCrlf isEqualToData:[@"\r\n" dataUsingEncoding:NSUTF8StringEncoding]]) {
                    fieldData = [fieldData subdataWithRange:NSMakeRange(0, bodyLength - 2)];
                }
            }
            ...
            if ([fieldName isEqualToString:@"blob"]) {
                ...
                NSString *textValue = [[NSString alloc] initWithData:fieldData encoding:NSUTF8StringEncoding];
                if (textValue) {
                    formData[fieldName] = [textValue stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
                }
            }
        }
...
- (instancetype)parseFromData:(NSData *)data remoteAddress:(NSString *)remoteAddress {
    ...
    if (!requestString) {
        requestString = [[NSString alloc] initWithData:data encoding:NSISOLatin1StringEncoding];
    }
    ...
    NSArray<NSString *> *lines = [requestString componentsSeparatedByString:@"\r\n"];
    if (lines.count == 0) {
        return nil;
    }
    ...
    NSDictionary<NSString *, NSString *> *queryParams = [self parseQueryParams:queryString];
...
- (NSDictionary<NSString *, NSString *> *)parseHeaders:(NSArray<NSString *> *)lines {
...
- (NSData *)parseBody:(NSArray<NSString *> *)lines fromData:(NSData *)data {
    ...
    NSString *headerText = [[lines subarrayWithRange:NSMakeRange(0, headerEndLine)] componentsJoinedByString:@"\r\n"];
    ...
    NSRange bodyRange = [data rangeOfData:headerData options:0 range:NSMakeRange(0, data.length)];
    if (bodyRange.location == NSNotFound) {
        return [NSData data];
    }

    NSUInteger bodyStart = bodyRange.location + bodyRange.length + 4;
    if (bodyStart >= data.length) {
        return [NSData data];
    }

    return [data subdataWithRange:NSMakeRange(bodyStart, data.length - bodyStart)];
}
