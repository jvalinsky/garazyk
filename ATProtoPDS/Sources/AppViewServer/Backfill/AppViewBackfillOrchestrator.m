...
- (void)_scheduleNextWorkers {
    while (!_stopping
        ...
        AppViewBackfillWorker *worker = [[AppViewBackfillWorker alloc]
            initWithDID:did database:_database indexers:_indexers];
...
- (void)worker:(AppViewBackfillWorker *)worker didCompleteForDID:(NSString *)did lastRev:(NSString *)lastRev {
    ...
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_BACKGROUND, 0), ^{
        NSError *err = nil;
        NSArray<AppViewPendingDelta *> *deltas =
            [self.database dequeuePendingDeltasForDID:did error:&err];
        if (deltas.count > 0) {
            PDS_LOG_INFO(@"[AppView Backfill] Replaying %lu pending deltas for %@",
                         (unsigned long)deltas.count, did);
            for (AppViewPendingDelta *delta in deltas) {
        ...
        if ([delegate respondsToSelector:@selector(orchestrator:didCompleteBackfillForDID:)]) {
            [delegate orchestrator:self didCompleteBackfillForDID:did];
        }
    });
}
