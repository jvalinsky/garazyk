...
@property (nonatomic, strong) PDSAppleKeyManager *keyStore;
...
- (instancetype)initWithKeyStore:(PDSAppleKeyManager *)keyStore {
    self = [super init];
    if (self) {
        _keyStore = keyStore;
...
- (SecKeyRef _Nullable)currentSigningKey {
    ...
    dispatch_sync(self.accessQueue, ^{
        PDSAppleKeyPair *activePDSAppleKeyPair = [self.keyStore getActiveKeyPair:nil];
        if (activePDSAppleKeyPair) {
            keyRef = activePDSAppleKeyPair.privateKey;
            CFRetain(keyRef);
        }
    });
...
- (NSArray *)allValidPublicKeys {
    ...
    dispatch_sync(self.accessQueue, ^{
        NSArray<PDSAppleKeyPair *> *allKeyPairs = [self.keyStore allKeyPairs:nil];
        for (PDSAppleKeyPair *keyPair in allKeyPairs) {
            if (keyPair.isActive) {
...
- (BOOL)rotateKeys {
    ...
    dispatch_sync(self.accessQueue, ^{
        ...
        NSError *error = nil;
        PDSAppleKeyPair *newPDSAppleKeyPair = [self.keyStore generateKeyPairWithAlgorithm:@"ECDSA" keySize:256 error:&error];
        
        if (newPDSAppleKeyPair) {
            // Set the new key as active
            success = [self.keyStore setKeyPairActive:newPDSAppleKeyPair.keyID error:&error];
            
            if (success) {
                // Optionally, deactivate old keys after a grace period
                // For now, keep all keys active during transition
                PDS_LOG_AUTH_INFO(@"Key rotation completed successfully (new key ID: %@)", newPDSAppleKeyPair.keyID);
            }
        } else {
...
- (nullable NSData *)signData:(NSData *)data error:(NSError **)error {
    ...
    dispatch_sync(self.accessQueue, ^{
        PDSAppleKeyPair *active = [self.keyStore getActiveKeyPair:&localError];
        if (active) {
