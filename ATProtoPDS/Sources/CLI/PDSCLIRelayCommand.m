...
- (NSString *)usage {
    return @"pds relay serve|start|stop|status|upstream [options]";
}
...
- (NSString *)helpText {
    return @"ATProto Relay (Sync v1.1) - Full network relaying.\n\n"
           ...
           @"  --help             Show this help\n\n"
           @"Examples:\n"
           @"  pds relay serve --upstream pds1.com --upstream pds2.com\n"
           @"  pds relay serve --port 3000 --retention 48\n"
           @"  pds relay status";
}
...
- (int)executeUpstream:(NSArray<NSString *> *)args
               context:(PDSCLICommandContext *)context {
    if (args.count == 0) {
        [context printInfo:@"Usage: pds relay upstream add|remove|list [url]"];
        return 1;
    }
