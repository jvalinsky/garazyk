...
- (NSString *)usage {
    return @"pds oauth <subcommand> [options]";
}
...
- (NSString *)helpText {
    return @"Manage OAuth clients for ATProto authentication.\n\n"
           ...
           @"Examples:\n"
           @"  pds oauth client register --client-id my-app --redirect-uri http://localhost:3000/callback\n"
           @"  pds oauth client list\n"
           @"  pds oauth client delete my-app";
}
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:dbPath]) {
        if (error) {
            *error = [NSError errorWithDomain:@"PDSCLIOAuthCommand"
                                         code:-1
                                     userInfo:@{NSLocalizedDescriptionKey: @"Database not found. Run 'pds serve' first."}];
        }
        return nil;
    }
