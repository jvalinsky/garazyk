...
NSString *const WebSocketServerErrorDomain =
    @"com.atproto.pds.websocket.server";
...
@property(nonatomic, strong)
    NSMutableSet<WebSocketConnection *> *mutableConnections;
...
- (BOOL)start:(NSError **)error {
  ...
  nw_listener_set_state_changed_handler(
      listener, ^(nw_listener_state_t state, _Nullable nw_error_t error) {
        // Dispatch to a queue to handle state changes, but signal semaphore
        // immediately if needed Note: nw_listener callbacks are on
        // listenerQueue
        ...
        case nw_listener_state_failed:
          ...
          dispatch_semaphore_signal(
              strongSelf.stopSemaphore); // Also signal stop if we were stopping
          dispatch_semaphore_signal(
              readySemaphore); // Signal ready semaphore to unblock start
          break;
