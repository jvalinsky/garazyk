...
/*!
 ...
 @abstract Bug condition exploration test for ATProto OAuth client_metadata support.
 
 @discussion This test verifies that ATProto clients (bsky.app, witchsky.app, native apps)
 can authenticate using the client_metadata parameter without database pre-registration,
 as required by the ATProto OAuth specification.
 ...
 The test failure confirms the bug exists. After implementing the fix, this test will pass.
...
- (void)setUp {
    ...
    // NOTE: We deliberately DO NOT register bsky.app or witchsky.app in the database
    // This is the bug condition - ATProto clients should work via client_metadata
}
...
- (void)testBskyAppAuthorizationWithClientMetadata {
    NSString *codeVerifier = @"high-entropy-random-string-that-is-long-enough-for-pkce-43-chars-minimum";
    ...
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:clientMetadata options:0 error:&jsonError];
    XCTAssertNil(jsonError, @"Failed to serialize client_metadata: %@", jsonError);
    NSString *clientMetadataJSON = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
    ...
    for (NSString *key in authParams) {
        NSString *value = authParams[key];
        NSString *encodedValue = [value stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
        [parts addObject:[NSString stringWithFormat:@"%@=%@", key, encodedValue]];
    }
    ...
    XCTAssertNotEqual(authResp.statusCode, 400, @"Authorization should not return 400 error");
    
    // Check that we don't get "unauthorized_client" error
    if (authResp.body && authResp.body.length > 0) {
        NSString *bodyString = [[NSString alloc] initWithData:authResp.body encoding:NSUTF8StringEncoding];
        XCTAssertFalse([bodyString containsString:@"unauthorized_client"], 
                      @"Should not return unauthorized_client error for valid client_metadata");
    }
    ...
    XCTAssertTrue(isSuccess, @"Authorization should succeed with status 200 or 302, got %ld", (long)authResp.statusCode);
}
...
/*!
 ...
 @discussion This test simulates witchsky.app providing client_id=https://witchsky.app
 with valid client_metadata JSON. Per ATProto OAuth spec, this should succeed
 without database pre-registration.
...
- (void)testWitchskyAppAuthorizationWithClientMetadata {
    NSString *codeVerifier = @"another-high-entropy-random-string-for-pkce-with-minimum-43-characters";
    NSString *codeChallenge = [self generateCodeChallenge:codeVerifier];
...
/*!
 ...
 @discussion This test simulates a native app using http://127.0.0.1:8080/callback
 as redirect_uri. Per RFC 8252 and ATProto OAuth spec, loopback redirects should
 be allowed for native apps.
...
- (void)testNativeAppWithLoopbackRedirectIPv4 {
    NSString *codeVerifier = @"native-app-pkce-verifier-with-sufficient-entropy-for-security-requirements";
    NSString *codeChallenge = [self generateCodeChallenge:codeVerifier];
    ...
    // EXPECTED BEHAVIOR (after fix): Authorization succeeds with loopback redirect
    XCTAssertNotEqual(authResp.statusCode, 400, @"Authorization should not return 400 error");
...
- (void)testNativeAppWithLoopbackRedirectIPv6 {
    NSString *codeVerifier = @"ipv6-native-app-pkce-verifier-with-sufficient-entropy-for-security";
    NSString *codeChallenge = [self generateCodeChallenge:codeVerifier];
    ...
    // EXPECTED BEHAVIOR (after fix): Authorization succeeds with IPv6 loopback redirect
    XCTAssertNotEqual(authResp.statusCode, 400, @"Authorization should not return 400 error");
