...
- (void)testApplicationSyncGetRepoSocketStreamingUsesChunkedTransferEncoding {
    ...
    HttpServer *server = [PDSHttpTestUtilities startSocketServerWithDispatcher:self.dispatcher error:&startError];
    if (!server) {
    ...
    NSData *rawResponse = [PDSHttpTestUtilities rawHTTPResponseForPath:path port:(uint16_t)server.port error:&requestError];
    [server stop];
    ...
    NSError *parseError = nil;
    NSDictionary *parsed = [PDSHttpTestUtilities parseRawHTTPResponse:rawResponse error:&parseError];
    XCTAssertNil(parseError);
    ...
    NSDictionary *chunked = [PDSHttpTestUtilities decodeChunkedBody:parsed[@"body"] error:&parseError];
    XCTAssertNil(parseError);
...
- (void)testApplicationSyncGetBlobSocketRangeUsesChunkedPartialContent {
    ...
    NSError *requestError = nil;
    NSData *rawResponse = [PDSHttpTestUtilities rawHTTPResponseForPath:path
