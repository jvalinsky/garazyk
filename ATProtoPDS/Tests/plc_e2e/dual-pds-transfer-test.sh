...
"$PLC_BIN" serve --port $PLC_PORT --data "$PLC_DATA" &
PLC_PID=$!
sleep 2

echo "-> Starting PDS Server A on port $PDS_A_PORT..."
"$CLI_BIN" --data-dir "$PDS_A_DATA" --config "$PDS_A_DATA/config.json" serve --port $PDS_A_PORT &
PDS_A_PID=$!
sleep 2

echo "-> Starting PDS Server B on port $PDS_B_PORT..."
"$CLI_BIN" --data-dir "$PDS_B_DATA" --config "$PDS_B_DATA/config.json" serve --port $PDS_B_PORT &
PDS_B_PID=$!
sleep 2

echo "-> Creating account 'alice.test' on PDS A..."
# We explicitly do NOT use interactive prompt by providing all arguments
ATPROTO_INTERACTIVE=0 "$CLI_BIN" --data-dir "$PDS_A_DATA" --config "$PDS_A_DATA/config.json" account create \
...
DID=$("$CLI_BIN" --data-dir "$PDS_A_DATA" --config "$PDS_A_DATA/config.json" account info alice.test | grep 'DID:' | awk '{print $2}')
...
"$CLI_BIN" --verbose --data-dir "$PDS_A_DATA" --config "$PDS_A_DATA/config.json" account update-plc-endpoint "$DID" "http://127.0.0.1:8002" || { echo "ERROR: failed to update PLC endpoint"; exit 1; }
